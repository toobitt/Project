$(document).ready(function(){  
	...
	replyComment = function(sid,commid,type){
		//var userid = $("#rp_"+commid+'_'+type).val().split("_")[0];
		var username = $("#rp_"+commid+'_'+type).val().split("_")[1]; 
		$("#co_"+type+'_'+commid).attr("className","first");
		var replyTextArea = '<div class="hui_show" id="comm_div_'+commid+'_'+type+'"><div class="hui_bor" id="status_item_'+commid+'"><p><span><a href="javascript:void(0)" onclick="close_Comm('+commid+','+type+');">&nbsp;</a></span><input style="margin-top: 4px;margin-left: 5px;vertical-align: middle;" type="checkbox" name="transmit_to_ms" id="transmit_to_mt'+sid+'" value="0"  onclick="changevalue('+sid+')" /><label for="transmit_to_mt'+sid+'" style="cursor:pointer;font-size:12px;margin-left:3px;">同时转发到我的点滴</label></p>';
		replyTextArea +='<p id="text_'+commid+'"><input type="text" class="hui_detail" name="comm_'+commid+'_'+type+'"  id="reply_'+commid+'_'+type+'" value="回复@'+username+':"><a href="javascript:void(0);" class="reply_comm" style="color: #A0A0A0;" onclick="replyAction('+commid+','+type+')">回复</a></p></div><p class="hui_show_bot"></p>';
		replyTextArea +='</div>';
	...
	close_Comm = function(commid,type)
	{
		$("#comm_div_"+commid+"_"+type).remove();
	}
	replyAction = function(commid,type)
	{
		var sid = $("input[name=myself]").val().split('_')[2];
