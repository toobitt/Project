...
$(document).ready(function(){
				...
				$.ajax({
					...
					{ 
						...
						{ 
							...
							li_new += '</span>'; 
							li_new += '<a style="margin-left:10px;" href="user.php?user_id=' + json.user.id +'">'+json.user.username +'</a> &nbsp;&nbsp;'+json.text;
						 	 
							li_new += '<span style="font-size:12px;color:silver;" >&nbsp;&nbsp;'+json.create_at;
			...
			else
			{
				...
				$.ajax({
					...
					{ 		 
						...
						var url = SNS_MBLOG + 'show.php?id='+ sid +'#'+json.id;
						var li_new = '<dd id="co_' + json.id + '_' + json.user.id + '" onmouseover="report_show(' + json.id + ',' + json.user.id + ');" onmouseout="report_hide(' + json.id + ',' + json.user.id + ');">';
						li_new += li_new += '<a id="' + json.id + '" name="' + json.id + '"></a>'+
						'<div style="display:none;" id="cons_' + json.id + '_' + json.user.id + '">'+json.text+'</div>'+
						'<div style="display:none;" id="ava_' + json.id + '_' + json.user.id + '">'+json.user.small_avatar+'</div>'+
						'<div style="display:none;" id="user_' + json.id + '_' + json.user.id + '">'+json.user.username +'</div>'+
						'<div style="display:none;" id="url_' + json.id + '_' + json.user.id + '">'+ url +'</div>'+
						...
						li_new += '<span style="font-size:12px;color:silver;">&nbsp;&nbsp;'+json.create_at+'</span></dd>';
						$("#text_"+ sid).append(li_new);
						$("#comm_text_"+sid).val('');  
	...
	{
		//删除评论时添加下面的div
		var dd = '<div id="box_'+commentid+'" style="width:200px;height:80px;text-align: center; background-color: white; padding: 5px; border: 5px solid rgb(223, 223, 223); position: absolute; z-index: 9999; font-size: 12px;left:40%top:50%;"><p style="color: rgb(102, 102, 102);">确定删除该条评论吗?</p><br><input type="button" value="确定" name="confirm" onclick="deleteComment('+commentid+');" style="margin: 0pt 5px; background-color: rgb(108, 187, 74); padding: 3px 10px; border: 0px none; color: rgb(255, 255, 255);"><input type="button" value="取消" name="canel" onclick="closeBox('+commentid+');" style="margin: 0pt 5px; background-color: rgb(108, 187, 74); padding: 3px 10px; border: 0px none; color: rgb(255, 255, 255);"></div>';
		$("#tips_"+commentid).before(dd);
	...
	if($("#Mymap").html()!=null)	
	{
		var Mymap = new AlertBox("Mymap"),locks = false;
		function lockup(e){ e.preventDefault(); };
		function lockout(e){ e.stopPropagation(); };
		Mymap.onShow = function(){
			if ( locks ) {
				OverLay.show();
			}
		}
		Mymap.onClose = function(){	
			OverLay.close();
		}
		$$("mapClose").onclick = function(){ Mymap.close(); }
		$$("mapClose").onclick = function(){ Mymap.close(); }	
	...
	$("#status").keyup(function(e) {
		...
		switch(e.keyCode)
		{
			case 8:
				params.keycode_e = 0;
				params.key_length = keyWords.lastIndexOf("@")+1;
				keys = keyWords.substr(params.key_length);
				var pattern = /\s{1,}/g;
			...
			case 13://
				if(params.move >= 0)
				{
					insert_name(parseInt(params.key));
			...
			case 39: //right
				...
				break;
			case 40: //down
				if(params.keycount&&params.key<(params.keycount-1))
				{
					params.move = 1;
					$("#cur_"+params.key).removeClass('cur');
					params.key ++;
					$("#cur_"+params.key).addClass('cur');
				}
				break;
			case 38: //up
				if(params.keycount>=0&&params.key>0)
				{
					params.move = 1;
					$("#cur_"+params.key).removeClass('cur');
					params.key --;
		...
		var hang = $(this).val().split("\n"); 
		if(hang.length>1)
		{
			params.top_s = hang.length-1;
		}
		for(var i = 0;i<hang.length;i++){ 
			params.left_s = hang[i].length;
		} 
		var offset = $("#status").offset(); 
		var position = keyWords.indexOf('@');
		if(!keyWords)
		{
			$("#friends").hide();
			$("#friends_name").remove();
		...
		if(position >= 0)
		{
			if(e.keyCode == 50)
			{
				params.keycode_s = e.keyCode;
				params.key_length = keyWords.lastIndexOf("@")+1;
			}
			$("#friends").css({
				'left': offset.left + params.left_s*8 - 130,
				'top':offset.top + params.top_s*12 - 50});
//			alert(e.keyCode);
			
			keys = keyWords.substr(params.key_length);
			if(keys&&params.keycode_s == 50)
			{
				var keyc = "@"+ keys;
				var pattern = /\@{1}.*\s{1,}/g;
				if(pattern.test(keyc))
				{
					$("#friends_name").remove();
					$("#friends").hide();
					params.keycode_e = 32;
			...
			if(params.keycode_s&&!params.keycode_e&&!params.move)
			{
				if(keyWords.substr(params.key_length))
				{
					$.ajax({
						url:"index.php",
						type:"POST",
						...
						success:function(json)
						{
							if(json)
							{
								...
								$("#friends").show();
								$("#friends_name").remove();
								$("#friends").append(obj.html);
								params.key = 0;
								params.keycount = obj.count;
							}
						}
					});
				}
	...
	change_css = function(total,id)
	{
		for(i = 0;i<total;i++)
		{
			...
			{
				$("#cur_"+i).removeClass('cur');
			}
		}
	}
	
	insert_name = function(id)
	{
		if (!$("#cur_"+id).html())
		{
			return;
		...
		values = $("#status").val();
		val = values.substr(0,params.key_length);
		$("#status").val(val+$("#cur_"+id).html()+" ");
		$("#friends").hide();
		$("#friends_name").remove();
		params.keycode_s = 0;
		params.keycode_e = 0;
...
// 表情开始	
	...
	$("#publish_face").bind("click",function(){
		global_face('status','face');
		$("#uploadvideo").hide();
		$("#uploadimg").hide();
	});
	...
	$("#facelistS").bind("click",function(){
		global_face('status','faceS');
	});
