...
*
* $Id: ui.base.php 4194 2011-07-26 05:26:45Z lijiaying $
***************************************************************************/
/**
 * 初始化抽象类，完成程序的初始化工作
 * @author develop_tong
...
//				echo $url;
			...
			//$this->page_title = '正在转向...';
					
			//include(hg_load_template("redirect"));
			$this->tpl->setTemplateTitle('正在转向...');
			$this->tpl->addVar('_mGuide', $this->mGuide);
			$this->tpl->addVar('message', $message);
			$this->tpl->addVar('url', $url);
			$this->tpl->addVar('guide', $guide);
		...
		if (!$this->input['ajax'])
		{
			...
			if(!$this->user['id'] && $need_login)
			{
				if(!SNS_UCENTER)
				{
					//$this->page_title = '登录';
					//$message = '请先登录…';
					//include(hg_load_template("login", $tpldir));
					$message = '请先登录…';
			...
			{
				//$this->page_title = '出错了';
				//include(hg_load_template($templates)); 
