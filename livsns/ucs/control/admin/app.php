...
/*
	...
	$Id: app.php 2096 2011-02-17 09:53:12Z chengqing $
*/
...
class control extends adminbase {
	...
	function onls() {
		...
		$applist = $_ENV['app']->get_apps(); //获取所有应用信息
				
		$this->view->assign('status', $status);
		$this->view->assign('a', $a);
		$this->view->assign('applist', $applist);
		//$this->view->assign('cp_list', $this->app_list);
		
		//print_r($this->view->vars);
	 ...
	 * 添加应用后台管理
	 */
	/*function onadd_app_cp()
	{
		$is_sadd_cp = getgpc('add_cp', 'P'); //检测是否是添加后台
		
		if($is_sadd_cp)  
		{
			$cp_name = getgpc('cp_name', 'P');
			$cp_url = getgpc('cp_url', 'P');
			$type = 'CP';
			
			if($cp_name & $cp_url)
			{
				$sql = "INSERT INTO " . UC_DBTABLEPRE . "applications SET name = '" . $cp_name . "' , url = '" . $cp_url . "' , is_cp =1";
				$this->db->query($sql);
				header("location: admin.php?m=app&a=ls");	
			}
							
		}
		else //显示添加后台表单
		{
			$this->view->display('admin_cp');	
		} 
	}*/
