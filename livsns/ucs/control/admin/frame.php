...
/*
	...
	$Id: frame.php 3106 2011-03-28 08:25:16Z chengqing $
*/
...
class control extends adminbase {
	...
	function __construct() {
		...
		require (ROOT_DIR . 'lib/class/groups.class.php');
		$this->mGroup = new Group();
		
		require (ROOT_DIR . 'lib/class/albums.class.php');
		$this->mAlbums = new albums();
		
		require (ROOT_DIR . 'lib/video/video.class.php');
	...
	function onindex() {
		 ...
		 * 获取微博数目
		 */
		$status_nums = $this->mStatus->getStatusNum();
		
		/**
		 * 获取地盘数目
		 */
		$group_nums = $this->mGroup->getGroupNum();
		$thread_nums = $this->mGroup->getThreadNum();
				
		/**
		 * 获取相册数目
		 */
		$albums_nums = $this->mAlbums->getAlbumsNum();
		$pics_nums = $this->mAlbums->getPicNum();
		
		/**
		 * 获取网台数目
		 */
		$station_nums  = $this->mVideo->getStationNum();
		$video_nums  = $this->mVideo->getVideoNum();
		$program_nums  = $this->mVideo->getProgramNum();
				
		$this->view->assign('sid', $this->view->sid);
		$mainurl = getgpc('mainurl');
		$mainurl = $mainurl  ? $mainurl : 'admin.php?m=frame&a=main&sid='.$this->view->sid;
		...
		$this->view->assign('status_nums', $status_nums);
		$this->view->assign('group_nums', $group_nums);
		$this->view->assign('thread_nums', $thread_nums);
		$this->view->assign('albums_nums', $albums_nums);
		$this->view->assign('pics_nums', $pics_nums);
		$this->view->assign('station_nums', $station_nums);
		$this->view->assign('video_nums', $video_nums);
		$this->view->assign('program_nums', $program_nums);
	...
	function onapplist() {
		...
		//print_r($applist);
		$this->view->assign('applist', $applist);
		$this->view->display('admin_applist');
	}
	...
	function _get_uc_members() {
		if(!$this->members) {
			$this->members = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."member");
		}
		return $this->members;
	}
