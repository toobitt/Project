...
/*
	...
	$Id: admin.php 2994 2011-03-22 05:13:07Z zdl $
*/
...
class control extends adminbase {
	...
	function onls() {
		...
		if(!empty($_POST['addname']) && $this->submitcheck()) {
			...
			$uid = $this->db->result_first("SELECT id FROM ".UC_DBTABLEPRE."member WHERE username='$addname'");
			if($uid) {
				...
				if($adminuid) {
					...
					$this->db->query("UPDATE " .UC_DBTABLEPRE ."member SET user_group_id=1 WHERE id='$uid'");//设置member表的group_id为1
		...
		$userlist = $this->db->fetch_all("SELECT a.*,m.* FROM ".UC_DBTABLEPRE."admins a LEFT JOIN ".UC_DBTABLEPRE."member m ON m.id=a.uid LIMIT $start, $ppp");
