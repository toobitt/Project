...
/*
	...
	$Id: admin.php 2096 2011-02-17 09:53:12Z chengqing $
*/
...
class adminbase extends base {
	...
	function check_priv() {
		...
		if(empty($username)) {
			...
			if(!$this->user['isfounder']) {
				...
				$admin = $this->db->fetch_first("SELECT a.*, m.* FROM ".UC_DBTABLEPRE."admins a LEFT JOIN ".UC_DBTABLEPRE."member m ON a.uid = m.id  WHERE a.username='$username'");
				if(empty($admin)) {
					header('Location: '.UC_API.'/admin.php?m=user&a=login&iframe='.getgpc('iframe', 'G').($this->cookie_status ? '' : '&sid='.$this->view->sid));
					exit;
				} else {
	...
	{		
		$sql = "SELECT name , url FROM " . UC_DBTABLEPRE . "applications WHERE is_cp=1";
		$r = $this->db->query($sql);
