...
/*
	...
	$Id: user.php 3438 2011-04-08 03:22:19Z wang $
*/
...
class usermodel {
	...
	function get_user_by_uid($uid) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."member WHERE id='$uid'");
		return $arr;
	}

	function get_user_by_username($username) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."member WHERE username='$username'");
		return $arr;
	}

	function get_user_by_email($email) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."member WHERE email='$email'");
		return $arr;
	}
	...
	function check_usernameexists($username) {
		$data = $this->db->result_first("SELECT username FROM ".UC_DBTABLEPRE."member WHERE username='$username'");
		return $data;
	}
	...
	function check_emailexists($email, $username = '') {
		...
		$email = $this->db->result_first("SELECT email FROM  ".UC_DBTABLEPRE."member WHERE email='$email' $sqladd");
		return $email;
	}
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		...
		$sqladd = $uid ? "id='".intval($uid)."'," : '';
		//$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."member SET $sqladd username='$username', password='$password', email='$email', join_time='".$this->base->time."', salt='$salt'");
		$uid = $this->db->insert_id();
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."member_extra SET member_id ='$uid', ip='$regip', last_activity=" . $this->base->time);
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."memberfields SET uid='$uid'");
		return $uid;
	}

	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
		$data = $this->db->fetch_first("SELECT username, id, password, salt FROM ".UC_DBTABLEPRE."member WHERE username='$username'");
		...
		if($sqladd || $emailadd) {
			$this->db->query("UPDATE ".UC_DBTABLEPRE."member SET $sqladd WHERE username='$username'");
	...
	function delete_user($uidsarr) {
		...
		if($uids) {
			$this->db->query("DELETE FROM ".UC_DBTABLEPRE."member WHERE id IN($uids)");
			$this->db->query("DELETE FROM ".UC_DBTABLEPRE."member_extra WHERE member_id IN($uids)");
	...
	function get_total_num($sqladd = '') {
		$data = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."member $sqladd");
		return $data;
	}

	function get_list($page, $ppp, $totalnum, $sqladd) {
		$start = $this->base->page_get_start($page, $ppp, $totalnum);
		$data = $this->db->fetch_all("SELECT * FROM ".UC_DBTABLEPRE."member m LEFT JOIN ".UC_DBTABLEPRE."member_extra mx ON m.id=mx.member_id $sqladd order by m.id desc LIMIT $start, $ppp");
		return $data;
	}
	...
	function name2id($usernamesarr) {
		...
		$query = $this->db->query("SELECT id FROM ".UC_DBTABLEPRE."member WHERE username IN($usernames)");
		$arr = array();
