...
class developer_auth extends outerUpdateBase
{
    ...
    public function create()
    {
        ...
        //判断类型
        $sub_type = trim($this->input['sub_type']);
        if($sub_type == 'push')
        {
        	$data['push_create_time'] = TIMENOW;
        }
        elseif ($sub_type == 'developer')
        {
        	$data['develop_create_time'] = TIMENOW;
        }
        ...
        else 
        {
            ...
            $push_api_config = $company->getPushApiConfig($user_id);
            if(!$push_api_config && !$push_api_config['app_id'] && !$push_api_config['app_key'])
            {
