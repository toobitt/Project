...
class attribute_value_mode extends InitFrm
{
	...
	/******************************************************针对角标的listUI相关配置的操作**********************************************************/
    //获取针对角标的前台的属性
	public function getFrontCornerAttributeData($id = '',$mod_corner_id = 0,$role_id = 1 , $type = 0)
	{
        if(!$id)
	    {
	        return false;
	    ...
	    {
	        $sql = "SELECT * FROM " .DB_PREFIX. "superscript_ui_attr_value WHERE mod_corner_id = '" .$mod_corner_id. "' AND type =".$type;
	    }
	...
	public function setFrontCornerListUIValue($data = array())
	{
		...
		//判断该属性的值是否存在
		$sql = "SELECT * FROM " .DB_PREFIX. "superscript_ui_attr_value WHERE mod_corner_id = '" .$data['mod_corner_id']. "' AND ui_attr_id = '" .$data['ui_attr_id']. "' AND type = ".$data['type'];
		$pre_data = $this->db->query_first($sql);
		if($pre_data)
		{
    ...
    //对角标前台属性关联的后台属性统一设置值
	public function setFrontCornerAttrSameToRelate($ui_attr_id = '',$value = '',$mod_corner_id = '' , $type = 0)
	{
	...
	public function setFrontCornerAttrEachToRelate($ui_attr_id = '',$value = '',$mod_corner_id = '' , $type = 0)
	{
	...
	public function setCornerListUIValue($data = array())
	{
		...
		//判断该属性的值是否存在
		$sql = "SELECT * FROM " .DB_PREFIX. "superscript_attr_value WHERE mod_corner_id = '" .$data['mod_corner_id']. "' AND relate_id = '" .$data['relate_id']. "' AND type = ".$data['type'];
		$pre_data = $this->db->query_first($sql);
		if($pre_data)
		{
	 ...
	 * 获取后台针对用户使用的角标属性数据
	 */
	public function getCornerAttributeData($mod_corner_id = 0 , $type = 0)
	{
	    /***********************************************获取用户自己设置的属性值***************************************/
	    if ($mod_corner_id)//如果存在$mod_corner_id，取出该$mod_corner_id相应的属性值
	    {
	        $sql = "SELECT * FROM " .DB_PREFIX. "superscript_attr_value WHERE mod_corner_id = '" .$mod_corner_id. "' AND type =".$type;
	    }
	 ...
	 */
	public function getALListAttrByUiId($ui_id = 0 , $role_id = 0)
	{		 
