...
define('MOD_UNIQUEID','buy_type');//模块标识
define('SCRIPT_NAME', 'buyTypeUpdate');
require_once(ROOT_PATH . 'frm/node_frm.php');
class buyTypeUpdate extends nodeFrm
{
	public function __construct()
	{
        ...
        $this->setNodeTable('buy_type');
        $this->setNodeVar('buy_type');
	}
	...
	public function update() 
	{
        ...
        $this->addLogs('修改购买类型','',$data,$data['name']);
        //更新方法
        $this->updateNode();
		$this->addItem($data);
		$this->output();
	...
	public function delete()
	{
		...
		{
			$this->addLogs('删除购买类型','','','删除购买类型+' . $this->input['id']);
	...
	public function create()
	{
        ...
        if($nid = $this->addNode())
        {
			...
			$this->addLogs('创建购买类型',$data,'','创建购买类型' . $nid);
