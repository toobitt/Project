...
class  vod_collect extends adminReadBase
{
	...
	public function show()
	{
		...
		$sql = "SELECT vc.*,vs.name AS sort_name,vf.starttime FROM ".DB_PREFIX."vod_collect as vc  LEFT JOIN ".DB_PREFIX."vod_media_node as vs ON  vc.vod_sort_id = vs.id LEFT JOIN ".DB_PREFIX."vodinfo as vf ON vf.id = vc.vodinfo_id  WHERE  1 ". $condition ."  ORDER BY collect_order_id DESC ".$limit;
		$q  = $this->db->query($sql);
	...
	public function detail()
	{
		...
		$sql = "SELECT id,name as sort_name FROM ".DB_PREFIX."vod_media_node WHERE id = '".$return['vod_sort_id']."'";
