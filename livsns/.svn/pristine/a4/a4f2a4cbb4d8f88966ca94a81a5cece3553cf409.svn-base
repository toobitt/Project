...
define('MOD_UNIQUEID','tv_interact_node');//模块标识
require_once(ROOT_PATH . 'frm/node_frm.php');
class tv_interact_node_update extends nodeFrm
{
	public function __construct()
	{
        ...
        $this->setNodeTable('tv_interact_node');
        $this->setNodeVar('tv_interact_node');
	}
	...
	public function delete()
	{
		...
		//非管理员不能删除顶级分类
		$sql = 'SELECT id FROM '.DB_PREFIX.'tv_interact_node WHERE fid=0';
		$q = $this->db->query($sql);
		...
		$sql = "SELECT count(*) as total FROM " . DB_PREFIX . "tv_interact WHERE sort_id = " . intval($this->input['id']); 
		...
		//查询分类
		$sql = 'SELECT * FROM '.DB_PREFIX.'tv_interact_node WHERE id IN ('.$this->input['id'].')';
		...
		while($row = $this->db->fetch_array($q))
		{
			...
			$data2[$row['id']]['content']['tv_interact_node'] = $row;
		}
		 $this->initNodeData();
		//判断是否成功删除
	...
	public function drag_order()
	{
		...
		if(!empty($sort))
		{
			foreach($sort as $key=>$val)
			{
				...
				{
					$sql ="UPDATE " . DB_PREFIX . "tv_interact_node SET";
...
$out = new tv_interact_node_update();
