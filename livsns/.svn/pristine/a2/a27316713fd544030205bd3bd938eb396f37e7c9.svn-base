...
class SurveyPublish extends adminUpdateBase implements publish
{
	...
	function get_content()
	{
		...
		{
			$sql = "SELECT * FROM " . DB_PREFIX ."survey WHERE node_id = {$sort_id}";
		}
 	...
 	function update_content()
 	{
		...
		$sql = "SELECT * FROM " . DB_PREFIX ."survey WHERE id = " . $data['from_id'];
		$ret = $this->db->query_first($sql);
		if($ret['status'] != 1)
		{
			$sql = "UPDATE " . DB_PREFIX ."survey SET expand_id = 0, column_url = '' WHERE id = " . $data['from_id'];
		}
		else 
		{
			...
			$sql = "UPDATE " . DB_PREFIX . "survey SET expand_id = " . $data['expand_id'] . ", column_url = '" .serialize($url). "' where id = " . $data['from_id'];
		}
		$this->db->query($sql);
 	...
 	function delete_publish()
 	{
		...
		{
			$sql = "SELECT column_id,column_url FROM " . DB_PREFIX ."survey WHERE id = " . $data['from_id'];
			$ret = $this->db->query_first($sql);
			...
			$sql = "UPDATE " . DB_PREFIX ."survey SET column_id = '".addslashes(serialize($column_id))."', column_url = '".addslashes(serialize($column_url))."' WHERE id = " . $data['from_id'];
			$this->db->query($sql);						
		}	
		else		//全部删除
		{
			$sql = "UPDATE " . DB_PREFIX . "survey  
