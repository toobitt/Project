...
class Oauth{
	 ...
	 * @return array
	 */
	function getAuthorizeURL($authorizeurl,$callbackurl, $state = NULL, $display = NULL ) {
		$params = array();
		...
		return trim($authorizeurl,'/'). "?" . http_build_query($params);
	}
	 ...
	 * @return array
	 */
	function getAccessToken($accessTokenURL, $keys ) {
		$params = array();
		$params['client_id'] = $this->client_id;
		...
		} elseif ( $this->response_type === 'password' ) {
	 ...
	 * @return mixed
	 */
	function get($url, $parameters = array(), $format = false) {
		$response = $this->oAuthRequest($url, 'GET', $parameters, $format);
	 ...
	 * @return mixed
	 */
	function post($url, $parameters = array(), $format = false, $multi = false) {
		$response = $this->oAuthRequest($url, 'POST', $parameters, $format, $multi );
		if ($this->format === 'json' && $this->decode_json) {
			return json_decode($response, true);
		}
		return $response;
	 ...
	 * @ignore
	 */
	function oAuthRequest($url, $method, $parameters, $format = false, $multi = false) {
		if($format === 'tx')
		{
			$url = "{$url}";
		}
		else if($format && $format !== 'tx')
		{
			$url = "{$url}.{$this->format}";
		}
		switch ($method) {
...
//		print_r($url);exit;
		...
		$this->url = $url;
		$str = date('Y-m-d H:i:s',time()).'-------------'.$url.'=====post data======'.$postfields.'=====$response====='.$response."\r\n\r\n";
		//$this->logsIn('api/share/logs','share.txt',$str,$type=false);
		if ($this->debug) {
			echo "=====post data======\r\n";
			var_dump($postfields);
	...
	function logsIn($dir,$filename,$strings,$type=false)
	{  
		$subdir = date('Y-m',time()).'/'.date('d',time());
		$path = ROOT_PATH.trim($dir,'/').'/'.$subdir;
	    if(!is_dir($path))
	    {
