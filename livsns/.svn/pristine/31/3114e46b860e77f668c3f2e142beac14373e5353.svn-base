$(document).ready(function(){	
	if($("#Box").html()!=null)	
	{
			        	...
			        	else
			        	{
				        	...
				        	if(id)
	        				{
					        	...
					        	else
				        		{
					        		if($("#collect_"+id).html())
				        			{
					        			li = '<a class="plays" href="'+SNS_VIDEO+'station_play.php?sta_id='+id+'"></a><a class="gz_get" href="javascript:void(0);" onclick="del_concern('+ obj.id +','+ id +','+ uid +',1);">取消关注</a>';
					        			$("#collect_"+id).html(li);
				        			}
					        		if($("#gz_"+id).html())
				        			{
				...
				$.ajax({
			        ...
			        success: function(json) {
			        	...
			        	else
			        	{
			        		...
			        		{
				        		li = '<a class="plays" href="'+SNS_VIDEO+'station_play.php?sta_id='+id+'"></a><a class="gz_get" href="javascript:void(0);" onclick="add_concern('+ cid +',1,'+ uid +');">关注该频道</a>';
		...
		cell_phone = function(){
			...
			if(!tel)
			{
				$("#error_tip").html('手机号码不为空！');
			}
			else
			{
				if(tel.length != 11 )
				{
					$("#error_tip").html('请输入正确的手机格式，这涉及到您手机发送微博的功能是否能够成功实现！');
				}
				else
				{
					$("#error_tip").html('');
					$.ajax({
				        ...
				        success: function(json) {
				        	...
				        	else
				        	{
				        		...
				        		if(obj == 'have')
				        		{
				        			$("#error_tip").html('此号码已被绑定，请重新填写！');
				        		}
				        		else
				        		{
				        			if(obj.id)
				        			{
					        			$("#error_tip").attr('class','success_tip').html('绑定成功！').fadeIn(1000);
					        			location.href = SNS_UCENTER+"cellphone.php";
				        			}
				        			else
			        				{
				        				$("#error_tip").html('绑定失败，请联系网站网管理员！');
		...
		unbind_phone = function()
		{
			$.ajax({
		        ...
		        success: function(json) {
		        	...
		        	{
		        		$("#error_tip").attr('class','success_tip').html('解除成功！').fadeIn(1000);
		        		location.href = SNS_UCENTER+"cellphone.php";
		        	}
		        }
		    });
