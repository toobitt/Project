...
class crash_report extends adminReadBase
{
	private $mode;
	...
	public function show()
	{
		...
		//删除一周前的崩溃报告
		$limit_time = TIMENOW - 3600*24*7;
		$condition = ' UNIX_TIMESTAMP(create_time) < '.$limit_time;
		$sql = " DELETE FROM " .DB_PREFIX. "crash_report WHERE " . $condition;
		$this->db->query($sql);
	...
	public function get_condition()
	{
		...
		{
			$condition .= ' AND  app_name  LIKE "%'.trim(($this->input['k'])).'%" OR ddversion LIKE "%'.trim(($this->input['k'])).'%" ';
		}
