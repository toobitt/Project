...
class road extends InitFrm
{
    ...
    public function create($data)
    {
        ...
        //取得图片数组
        $piccount = count($_FILES['Filedata']['name']);
        ...
        if ($photos)
        {
            ...
            for($i = 0; $i < $piccount; $i++)
            {
                $material = $this->material->addMaterial($photos[$i], 0, 0, 0); //插入各类服务器
                if ($material['id']){
                    $mat[$i] = array(
                    'host' => $material['host'],
                    'dir' => $material['dir'],
        ...
        if ($photos[0])
        {
            $material = $this->material->addMaterial($photos[0], 0, 0, 0); //插入各类服务器
            if ($material['id'])
            {
                $mat = array(
                    'host' => $material['host'],
        ...
        if ($photos[1])
        {
            $material = $this->material->addMaterial($photos[1], 0, 0, 0); //插入各类服务器
            if ($material['id'])
            {
                $mat = array(
                    'host' => $material['host'],
                ...
                $sql = "UPDATE " . DB_PREFIX . "road SET pic2_id=" . $material['id'] . ",pic2='" . json_encode($mat) . "' ,local_img = 1 WHERE id=" . $data['id'];
                $this->db->query($sql);
            }
        }*/        
