<?php
class copywriting extends InitFrm
{
	...
	public function show($condition,$offset,$count,$field='*',$leftjoin='')
	{
		$limit 	 = " LIMIT " . $offset . " , " . $count;
		$sql = "SELECT {$field} FROM " . DB_PREFIX . "copywriting as c 
		".$leftjoin."  WHERE 1 ".$condition.$limit;
		$q = $this->db->query($sql);
		$return = array();
	...
	public function detail($id)
	{
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "copywriting WHERE id = " . $id;
		$row = $this->db->query_first($sql);
		if(is_array($row) && $row)
		{
			if($row['name'])
			{
				$row['name']=str_ireplace('<br/>', "\n", html_entity_decode($row['name']));
			}
			if($row['value'])
			{
				$row['value']=str_ireplace('<br/>', "\n", html_entity_decode($row['value']));
			}
			if(!empty($row['icon']))
			{
				$row['icon']=unserialize($row['icon']);
	...
	{
		$sql = 'UPDATE '.DB_PREFIX.'copywriting SET is_on = '.$opened.' WHERE id = '.$ids;
		$this->db->query($sql);
		$arr = array(
			'id'=>$ids,
			'opened'=>$opened,
