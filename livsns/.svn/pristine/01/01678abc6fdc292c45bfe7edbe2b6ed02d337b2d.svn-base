...
class special_sort_update extends nodeFrm
{
	...
	public function delete()
	{
		...
		//查询分类
		$sql = 'SELECT * FROM '.DB_PREFIX.'special_sort WHERE id IN ('.$this->input['id'].')';
		...
		while($row = $this->db->fetch_array($q))
		{
			...
			$data2[$row['id']]['content']['special_sort'] = $row;
		}
		 $this->initNodeData();
		//判断是否成功删除
	...
	public function drag_order()
	{
		...
		if(!empty($sort))
		{
			foreach($sort as $key=>$val)
			{
				...
				{
					$sql ="UPDATE " . DB_PREFIX . "special_sort SET";
