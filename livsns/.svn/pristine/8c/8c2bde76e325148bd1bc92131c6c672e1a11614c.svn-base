...
class column extends InitFrm
{
	...
	{
		$sql = "SELECT ".$field." FROM ".DB_PREFIX."column WHERE id in(".$id.")";
		$info = $this->db->query_first($sql);
		return $info;
	}
...
//		$site = $this->get_site_by_id($site_id);
	...
	public function get_site_column_first($field='*',$column_id)
	{
		$column = $this->get_column_first($field,$column_id);
		if(!empty($column['site_id']))
		{
			$site = $this->get_site_by_id($column['site_id'],' id,site_name,sub_weburl,weburl,custom_content_dir ');
			$column['site_data'] = $site;
			$column['column_url'] = mk_column_url($column+array('sub_weburl'=>$site['sub_weburl'],'weburl'=>$site['weburl'],'custom_content_dir'=>$site['custom_content_dir']));
		}
		return $column;
	}
	...
	public function getColumnsByfid($fid = 0)
	{
		$sql = "select * from ".DB_PREFIX."column where fid = ".$fid." order by order_id asc";
		$info = $this->db->fetch_all($sql);
		return $info;
