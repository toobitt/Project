<?php if($value['medias'])
{
	...
	<div id="prev_<?php echo $value['id'];?>"  style="display:<?php echo $show = $is_details ? 'none' : 'block';?>">
	<?php 
		...
		else//微博中含有视频
		{
			...
			{?>
				...
				$(document).ready(function (){
					...
					scaleVideo(<?php echo $value['id'];?>,<?php echo $mv['id'];?>,<?php echo $mv['self'];?>);
...
<?php 
	if(is_array($transmit_info['medias']))
	{
		...
		<div id="prev_<?php echo $transmit_info['id'] + $value['id'];?>" style="display:<?php echo $show = $is_details ? 'none' : 'inline-block';?>">
		<?php 
