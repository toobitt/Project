<?php
class order_mode extends InitFrm
{
	...
	{
		$sql = "SELECT * FROM " . DB_PREFIX . "order  WHERE 1 " . $condition . $orderby . $limit;
		$q = $this->db->query($sql);
		$info = array();
		while($r = $this->db->fetch_array($q))
		{
		//	$r['create_time'] = date('Y-m-d H:s',$r['create_time']);
			$info[] = $r;
		}
		return $info;
	}
	...
	{
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order WHERE 1 " . $condition;
	...
	public function audit($id = '')
	{
		...
		$sql = " UPDATE " .DB_PREFIX. "order SET status = '" .$status. "' WHERE id = '" .$id. "'";
		$this->db->query($sql);
