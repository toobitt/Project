...
class  videopointApi extends outerReadBase
{
    var $curl,$obj;
    ...
    public function is_pointed()
    {
        if(!isset($this->input['videoid']))
            return false;
        $ret = $this->obj->detail('point',$this->input['videoid']);
        if(empty($ret))
        {
            $ret = -1;//给前端做判断
            //$this->errorOutput('videoid不正确');
        }
        $this->addItem($ret);
        $this->output();
    ...
    public function show_points()
    {
        if(!isset($this->input['videoid']))
            return false;
        $ret = $this->obj->show('point',$this->input['videoid']);
        if(empty($ret))
            $this->errorOutput('videoid不正确');
    ...
    public function show_onepoint()
    {
        if(!isset($this->input['id']))
            return false;
        $ret = $this->obj->detail('point',$this->input['id'],'id');
        if(empty($ret))
            $this->errorOutput('id不正确');
        $this->addItem($ret);
        $this->output();
    }
    ...
    public function get_node_videos()
    {
        $this->curl->setSubmitType('get');
        $this->curl->initPostData();
        $this->curl->addRequestData('vod_sort_id',$this->input['id']);
