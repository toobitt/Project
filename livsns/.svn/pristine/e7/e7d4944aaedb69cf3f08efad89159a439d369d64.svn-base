...
class ServerAPI{
     ...
     * @param array $params
     */
    public function __construct($appKey,$appSecret,$action,$params=array(),$format = 'json'){
        $this->appKey = $appKey;
    ...
    public  function curl($action,$params,$httpHeader) {
        ...
        for($i=0;$i<1000;$i++)
        {
            if(strstr($data, "%5B".$i."%5D"))
            {
                $data = str_replace("%5B".$i."%5D",'', $data,$count);
            }
            else
            {
                break;
    ...
    private function getAllowMethods() {
        return array(
            '/user/refresh'=>array('userId','name','portraitUri'),
         ...
         //   '/message/broadcast'=>array('fromUserId','objectName','content'), 暂时未开放
            ...
            '/user/blacklist/add'=>array('userId','blackUserId'),
            '/user/blacklist/remove'=>array('userId','blackUserId'),
        );
    }
