...
class district_node_update extends nodeFrm
{
	...
	public function __construct()
	{
        ...
        $this->setNodeTable('district_node');
        $this->setNodeVar('district_node');
	...
	public function delete()
	{
		...
		//查询分类
		$sql = 'SELECT * FROM '.DB_PREFIX.'district_node WHERE id IN ('.urldecode($this->input['id']).')';
		...
		while($row = $this->db->fetch_array($q))
		{
			...
			$data2[$row['id']]['content']['district_node'] = $row;
		}
		 $this->initNodeData();
		//判断是否成功删除
	...
	public function drag_order()
	{
		...
		if(!empty($sort))
		{
			foreach($sort as $key=>$val)
			{
				...
				{
					$sql ="UPDATE " . DB_PREFIX . "district_node SET";
