...
/*自定义开始*/	
	function onget_total_num() {
		$this->init_input();
		$sqladd = $this->input('sqladd');
		$status = $_ENV['user']->get_total_num($sqladd);
		return $status;
	}
	...
	function onget_list() {
		...
		$ppp = $this->input('ppp');
		$totalnum = $this->input('totalnum');
		$sqladd = $this->input('sqladd');
		$status = $_ENV['user']->get_list($page, $ppp, $totalnum, $sqladd);
		return $status;
	}
