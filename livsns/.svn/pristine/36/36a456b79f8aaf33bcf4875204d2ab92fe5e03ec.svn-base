...
class vod_upload_update extends adminUpdateBase
{
    private $local_curl;
    private $curl;
    ...
    public function create()
    {
        ...
        if($this->user['group_type'] > MAX_ADMIN_TYPE)
        {
            ...
            {
                case 0 : $this->input['status'];break; //默认
                case 1 : $this->input['status'] = 1;break; //待审核
                case 2 : $this->input['status'] = 2;break; //已审核
            }
        }
        ...
        $data = array(
            ...
            'create_time'   =>  $this->input['create_time'] ? intval($this->input['create_time']) : TIMENOW,
            'update_time'   =>  TIMENOW,
            'ip'            =>  hg_getip(),
            ...
            'is_praise' => $this->input['is_praise'] ? 1 : 0,
        	'source_path'=>$this->input['source_path'] ? trim($this->input['source_path']) :'',
        	'source_filename'=>$this->input['source_filename'] ? trim($this->input['source_filename']):'',
    ...
    function update_praise_count()
    {
    	...
    	$info = array();
    	$sql  = "UPDATE " . DB_PREFIX . "vodinfo  SET  praise_count=praise_count" . $type . $num . "  WHERE id =" . $id;
    	$q    = $this->db->query($sql);
    	//发布
    	$sql  = "SELECT id, status, expand_id, title, column_id, pub_time,addperson FROM " . DB_PREFIX . "vodinfo WHERE id =" . $id;
    	$info = $this->db->query_first($sql);
