...
class superscript_mode extends InitFrm
{
	...
	public function getModCornerByCond($cond = '',$fields = '*',$group_id = '')
	{
	    ...
	    $sql = "SELECT " . $fields . " FROM " .DB_PREFIX. "module_corner WHERE 1 " . $cond;
	...
	public function createModCorner($data = array())
	{
		...
		$sql = " INSERT INTO " . DB_PREFIX . "module_corner SET ";
		foreach ($data AS $k => $v)
		{
			$sql .= " {$k} = '{$v}',";
		}
		...
		$this->db->query($sql);
		$vid = $this->db->insert_id();
		$sql = " UPDATE ".DB_PREFIX."module_corner SET order_id = {$vid}  WHERE id = {$vid}";
		$this->db->query($sql);
		return $vid;
	}
	
	//按照条件删除模块角标关系数据
	public function deleteModCornerByCond($cond = '')
	{
        if(!$cond)
        {
            return FALSE;
        ...
        $sql = "DELETE FROM " .DB_PREFIX. "module_corner WHERE 1 " . $cond;
        $this->db->query($sql);
        return TRUE;
	}
	...
	public function getUseCornerInfoByModId($module_id = '')
	{
	    if(!$module_id)
	    {
	        return FALSE;
	    ...
	    $sql = "SELECT s.*,mc.id AS mod_corner_id FROM " .DB_PREFIX. "module_corner mc LEFT JOIN " . DB_PREFIX . "superscript s ON mc.superscript_id = s.id WHERE mc.module_id = '" .$module_id. "' ";
	    $q = $this->db->query($sql);
	    $arr = array();
	    while ($r = $this->db->fetch_array($q))
	    {
	...
	{
		$sql = "SELECT * FROM ".DB_PREFIX."module_corner AS mc LEFT JOIN ".DB_PREFIX."app_module AS 
				m ON m.id = mc.module_id WHERE mc.superscript_id = ".$superscript_id." AND m.user_id = ".$user_id;
		$q   = $this->db->query($sql);
        $ret = array();
        while ($r = $this->db->fetch_array($q))
        {
	 ...
	 * 获取组建角标使用信息
	 */
	public function cornerIsUseInComp($superscript_id = 0 , $user_id = 0)
	{
		$sql = "SELECT * FROM ".DB_PREFIX."components_new_corner AS mc LEFT JOIN ".DB_PREFIX."components AS
				m ON m.id = mc.comp_id WHERE mc.superscript_id = ".$superscript_id." AND m.user_id = ".$user_id;
		$q   = $this->db->query($sql);
		$ret = array();
		while ($r = $this->db->fetch_array($q))
		{
	...
	public function getInfoByCata($cond_key = '' , $user_id = 0)
	{
		$sql = "SELECT * FROM ".DB_PREFIX."superscript AS s LEFT JOIN ".DB_PREFIX."superscript_cond AS
				 c ON s.id = c.superscript_id WHERE c.cond_key =  '".$cond_key."' AND c.user_id =".$user_id;
		$q = $this->db->query($sql);
		$ret = array();
		while ($r = $this->db->fetch_array($q))
		{
