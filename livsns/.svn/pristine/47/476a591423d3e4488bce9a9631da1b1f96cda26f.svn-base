...
class getFollowsInfoApi extends BaseFrm
{
	...
	{	
		...
		$sql = "SELECT r.member_id AS id , r.follow_time , m.member_name as username, m.host, m.dir, m.filepath, m.filename, e.* 
		...
		{
			$condition = " WHERE r.fmember_id = " . $this->user['user_id'] . " 
						   ORDER BY r.follow_time DESC 
						   LIMIT " . $offset . ', ' . $count;
