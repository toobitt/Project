...
class grade_style extends adminReadBase
{
	private $mode;
	...
	public function show()
	{
		...
		if(!empty($ret))
		{
			foreach($ret as $k => $v)
			{
				...
				$v['default_pic'] =  unserialize($v['default_pic']);
				$v['index_pic'] = $index_pic['host'].$index_pic['dir'].$index_pic['filepath'].$index_pic['filename'];
				//$v['default_pic'] = $v['host'].$v['dir'].$v['filepath'].$v['filename'];
				$this->addItem($v);
				$re[] = $v;
			}
			//file_put_contents('222.txt', var_export($re,1));
			$this->output();
		}
	}
