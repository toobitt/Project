<?php
define('MOD_UNIQUEID','app_sms_count');
require_once('global.php');
require_once(CUR_CONF_PATH . 'lib/app_sms_count_mode.php');
class app_sms_count_update extends adminUpdateBase
{
	private $mode;
    public function __construct()
	{
		parent::__construct();
		$this->mode = new app_sms_count_mode();
	}
	...
	public function update()
	{
        ...
        $update_data = array();
        if($recharge = $this->input['recharge'])
        {
            $update_data = array(
                'recharge' => $recharge,
            );
        }
		$ret = $this->mode->update($this->input['app_id'],$update_data);
		if($ret)
		{
