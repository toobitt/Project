<?php
define('MOD_UNIQUEID','member_ip_blacklist');
require_once('global.php');
require_once ROOT_PATH . 'lib/class/applant.class.php';
class member_ip_blacklist extends adminReadBase
{
    private $Blacklist;
    private $applant;
	...
	public function show()
	{
		...
		$ret = $this->Blacklist->showIpBlacklist($condition,$orderby,$limit);
	...
	{
		$condition = $this->get_condition();
		$info = $this->Blacklist->ip_count($condition);
	...
	public function get_condition()
	{
		...
		{
            $condition .= ' AND  (member_id  LIKE "%'.trim(($this->input['k'])).'%" OR member_name  LIKE "%'.trim(($this->input['k'])).'%" OR ip  LIKE "%'.trim(($this->input['k'])).'%" OR identifier  LIKE "%'.trim(($this->input['k'])).'%")';
		}
