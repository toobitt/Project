...
class apps extends appCommonFrm
{
	...
	public function limitNum()
	{
	    ...
	    $result = $nums['total'] >= APP_LIMIT_NUM ? true : false;
	    $this->addItem($result);
	    $this->output();
	}
	...
	private function filter_data()
	{
		...
		if (empty($app_name)) $this->errorOutput(PARAM_WRONG);
		return array(
			'name' => $app_name
		);
	}
	...
	public function config()
	{
		...
		if (!USE_EFFECT) unset($app_info['transition_effects']);
		//获取APP引导图
