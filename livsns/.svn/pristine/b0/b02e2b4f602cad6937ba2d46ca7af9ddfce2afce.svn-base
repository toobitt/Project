...
class verify_font extends adminReadBase
{
	private $mode;
	...
	public function show()
	{
		...
		$ret = $this->mode->show($condition,$orderby,$limit,'font');
		if(!empty($ret))
		{
			foreach($ret as $k => $v)
			{
	...
	{
		$condition = $this->get_condition();
		$info = $this->mode->count($condition,'font');
	...
	public function detail()
	{
		if($this->input['id'])
		{
			...
			*/
			$ret = $this->mode->detail($this->input['id'],$condition,'font');
			if($ret)
			{
				$this->addItem($ret);
				$this->output();
