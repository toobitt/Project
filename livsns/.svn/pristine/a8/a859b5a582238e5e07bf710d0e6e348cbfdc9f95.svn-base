...
class attentionUpdateApi extends outerUpdateBase
{
	...
	public function create()
	{
		...
		$this->setXmlNode('attention', 'create');
		...
		else 
		{
			...
			$result =  $this->action->insert('option_attention', $data);
			if($result)
			{
				//更新统计表
	...
	public function delete()
	{
		...
		$this->setXmlNode('attention', 'delete');
		$total = $this->action->get('option_attention','count(id) as total', $data, 0, 1, array());
		if($total)
		{
			$result =  $this->action->delete('option_attention', $data);
			if($result)
			{
				//更新统计表
				$this->updateSignData($post, -1);
	...
	function update()
	{
		...
		if($total)
		{
			if($this->action->update('option_attention', array('state'=>$state), $data, array()))
			{
