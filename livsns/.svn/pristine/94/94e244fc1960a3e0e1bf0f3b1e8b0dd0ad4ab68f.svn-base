...
//		$sendto 			= explode(',',trim($this->input['sendto']));
//		
//		$sendto_org_id 		= $sendto[0];
//		$sendto_org_name 	= $sendto[1];
//		
//		if($sendto_org_id==-1)
//		{
//			$this->errorOutput("没有选择组织机构");
//		}
	...
	public function update() 
	{
		...
		if(!$this->obj->update_notice($data,$cond))
		{
	...
	public function delete() 
	{
		...
		$sql = "DELETE FROM " . DB_PREFIX . "notice_content WHERE id=".($this->input['id']);
		if(!$this->db->query($sql))
		{
			$this->errorOutput('删除不成功');
		}
		
		$sql = "DELETE FROM " . DB_PREFIX . "notice WHERE notice_id=".($this->input['id']);
		if(!$this->db->query($sql))
		{
			$this->errorOutput('删除不成功');
		}
