...
class content_insert_xsApi extends adminBase
{
    ...
    public function content_insert_xs($offset = 0, $count = 100)
    {
        ...
        if ($content_ids)
        {
            ...
            if ($column_ids)
            {
                $column_details = $this->column->get_column_by_id(' id,name,parents ', implode(',', $column_ids), 'id');
            }
    ...
    public function opration_xunsearch($data, $content_columns, $opration, $column_details)
    {
        ...
        if(is_array($column_datas = unserialize($content_columns['column_datas'])))
        {
            ...
            {
                //$content_columns['column_datas'][$v['id']]['childs'] = $column_details[$v['id']]['childs'];
                $column_idstr .= $tag.$column_details[$v['id']]['parents'];
                $tag=',';
            }
