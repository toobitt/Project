...
require_once(CUR_CONF_PATH . 'lib/usercenter_cards_mode.php');
class app_cards extends outerReadBase
{
	private $mode;
	private $ucard;
    public function __construct()
	{
		...
		$this->ucard = new usercenter_cards_mode();
	}
	...
	public function getAppCards()
	{
		...
		$orderby = '  ORDER BY id DESC ';
		$limit = ' LIMIT ' . $offset . ' , ' . $count;
		$appCards = $this->mode->show($condition,$orderby,$limit);
		
		$cardInfo = $this->ucard->show();
		foreach ($cardInfo as $k=>$v)
		{
		    $cardInfo[$k]['is_choose'] = 0;
		    if($appCards)
		    {
		        foreach ($appCards as $ko=>$vo)
		        {
    		        if($v['id'] == $vo['card_id'])
    		        {
    		            $cardInfo[$k]['is_choose'] = 1;
    		        }    		            
		        }
		    }
		}    
		$this->addItem($cardInfo);
		$this->output();
	}
	...
	public function get_condition()
	{
		...
		{
		    $condition .= " AND app_id IN (".($this->input['id']).")";
		}
