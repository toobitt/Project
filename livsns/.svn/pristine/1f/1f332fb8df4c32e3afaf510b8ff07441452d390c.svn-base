...
//		}
	...
	public function get_template_sort($id,$search='')
	{
		if($search)
		{
			$sql = "SELECT id,name FROM ".DB_PREFIX."site_tem_sort WHERE name like '%".$search."%'";
		...
		else
		{
			if($id)
			{
				$sql = "SELECT id,name FROM ".DB_PREFIX."site_tem_sort WHERE fid = ".$id;
				$sort_info = $this->db->fetch_all($sql);
			...
			{
				$sql = "SELECT id,name FROM ".DB_PREFIX."site_tem_sort WHERE fid=0";
				$sort_info = $this->db->fetch_all($sql);
	...
	{			
		$sql = "SELECT fid FROM ".DB_PREFIX."site_tem_sort WHERE id = ".$id;
