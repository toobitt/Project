...
class destroy_batch extends BaseFrm
{
	private $mUser;
	...
	*/
	...
	{	
		...
		$sql = "SELECT sta.* , exl.transmit_count,exl.reply_count,exl.comment_count FROM ".DB_PREFIX."status sta 
		 LEFT JOIN ".DB_PREFIX."status_extra exl ON sta.id = exl.status_id 
		where sta.id in(".$this->input['id'].") AND sta.status=0 ORDER BY sta.id DESC ";
		...
		//删除收藏表中的数据
		$sql = "delete  from ".DB_PREFIX."status_favorites where status_id in  (".$this->input['id'].") and member_id =".$userinfo['id'];
