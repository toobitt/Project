	...
	public function detail($id = '',$cond = '')
	{
		...
		if($info)
		{
    		if(empty($info['package_name']))
    		{
    		    ...
    		    $info['android_package_name'] = $this->settings['package'] ? $this->settings['package']['android'].$info['id'] : 'com.hoge.android.app'.$info['id'];
    		}
    		else
    		{
    		    $info['ios_package_name'] = $info['package_name'];
    		    $info['android_package_name'] = $info['package_name'];
    		}
