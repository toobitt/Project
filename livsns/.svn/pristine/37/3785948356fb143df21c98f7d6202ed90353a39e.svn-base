<?php
require('./global.php');
define('SCRIPT_NAME', 'ftplog');
define('MOD_UNIQUEID', 'ftplog');
require_once(ROOT_PATH . 'lib/class/curl.class.php');
class ftplog extends adminBase
{
	...
	function show()
	{
		...
		$sql = 'SELECT * FROM '.DB_PREFIX.'ftplog WHERE 1'.$condition.$orderby.$limit;
		$q = $this->db->query($sql);
	...
	{
		$sql = 'SELECT COUNT(*) AS total FROM '.DB_PREFIX.'ftplog '.$this->get_condition();
