<?php
/***************************************************************************
* $Id: appClient.class.php 17481 2013-04-19 09:36:46Z yaojian $
***************************************************************************/
class appClient extends InitFrm
{
	...
	public function show($data)
	{
		...
		$sql = 'SELECT ' . $fields . ' FROM ' . DB_PREFIX . 'app_client WHERE 1';
		//获取查询条件
	...
	public function count($data)
	{
		...
		$sql = 'SELECT COUNT(id) AS total FROM ' . DB_PREFIX . 'app_client WHERE 1';
	 ...
	 * @param Array $data
	 */
	public function get_client_relation($data)
	{
	    $condition = '';
	    ...
	    $sql = 'SELECT * FROM ' . DB_PREFIX . 'client_relation WHERE 1' . $condition;
	    $query = $this->db->query($sql);
	    $info = array();
	    while ($rows = $this->db->fetch_array($query))
	    {
	 ...
	 * @param Int $client_id
	 */
	public function check($app_id, $client_id)
	{
	    $fields = array('download_url', 'file_size', 'update_desc', 'version_name', 'publish_time', 'package');
	    if ($client_id == 1) $fields[] = 'version_code';
	    $sql = 'SELECT ' . implode(',', $fields) . ' FROM ' . DB_PREFIX . 'client_relation 
	    WHERE state > 0 AND app_id = ' . $app_id . ' AND client_id = ' . $client_id;
	    $result = $this->db->query_first($sql);
	    if (!$result)
	    {
	        $sql = 'SELECT ' . implode(',', $fields) . ' FROM ' . DB_PREFIX . 'publish_log 
