...
class browseApi extends adminBase
{
	...
	public function show()
	{
		...
		{
			//查询数据源对应的参数
			$datasource_info = $this->data_source->get_datasource_info($block_info['datasource_id']);
		}
...
//		print_r($serach_data);exit;
		//根据数据源获取内容
		$content_data = $this->data_source->query($block_info['datasource_id'],$serach_data);
		//根据数据源获取内容条数
		$serach_data['is_count'] = 1;
		$serach_data['offset'] = $offset;
		$serach_data['count'] = $count;
		$count = $this->data_source->query($block_info['datasource_id'],$serach_data);
