 ...
 *
 */
class showGroupList extends uiBaseFrm
{
	private $curl;
	private $status;
	...
	//	$group_nums = array_pop($group);
		...
		if(empty($group) || !is_array($group))                  	 		//该用户没有加入地盘
		{
			$have_group = false;				
		}
		else
		...
		$this->tpl->outTemplate('group');   		            		                   //数据写入粉丝模板		
	}
