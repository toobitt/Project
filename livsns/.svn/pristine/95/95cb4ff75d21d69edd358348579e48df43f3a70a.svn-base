...
class apps extends appCommonFrm
{
	...
	public function detail()
	{
		...
		if ($app_info)
		{
			...
			//获取APP引导图
			$guide_pic = $this->api->app_pic(array('app_id' => $app_info['id']));
	...
	public function global_config()
	{
	    ...
	    $data = array(
	        ...
	        'module_english_limit' => MODULE_ENGLISH_LIMIT,
	...
	public function create()
	{
		...
		*/
		...
		$app_icon_info = $this->material->get_material_by_ids($app_icon);
		if (!$app_icon_info[0]) $this->errorOutput(APP_ICON_ERROR);
		
		//APP启动画面
		$app_start_pic = intval($this->input['app_start_pic']);
		$app_start_pic_info = $this->material->get_material_by_ids($app_start_pic);
		...
		if ($this->input['guide_pic'])
		{
		    $pic_id = trim(urldecode($this->input['guide_pic']));
		...
		if ($validate)
		{
		    $this->api->update('app_pic', array('app_id' => $result['id']), array('id' => $validate));
		}
		$this->addItem($result);
		$this->output();
	}
	...
	public function update()
	{
		...
		if (isset($this->input['app_start_pic']))
		{
    		//APP启动画面
    		$app_start_pic = intval($this->input['app_start_pic']);
    		$app_start_pic_info = $this->material->get_material_by_ids($app_start_pic);
    		if (!$app_start_pic_info[0]) $this->errorOutput(APP_STARTPIC_ERROR);
    		$app_start_pic_info = serialize($app_start_pic_info[0]);
    		if ($app_start_pic_info != $app_info['startup_pic'])
    		{
    		    $validate['startup_pic'] = $app_start_pic_info;
    		}
		}
		if ($this->input['drop_ids'])
		{
		    $pic_id = trim(urldecode($this->input['drop_ids']));
    		$id_arr = explode(',', $pic_id);
    		$id_arr = array_filter($id_arr, 'filter_arr');
    		if ($id_arr)
    		{
    		    $pic_id = implode(',', $id_arr);
    		    $this->api->delete('app_pic', array('id' => $pic_id));
    		}
		}
		//更新数据
		if ($this->input['guide_pic'])
		{
    		...
    		if ($id_arr)
    		{
    		    ...
    		    if ($info)
    		    {
    		        ...
    		        $this->api->update('app_pic', array('app_id' => $app_id), array('id' => $validate_ids));
    		    }
    		}
		}
		if ($validate)
	...
	public function config()
	{
    		    ...
    		    if ($this->settings['guide_size'])
    			{
    				...
    				if ($this->settings['guide_size']['ios'])
    				{
    				    foreach ($this->settings['guide_size']['ios'] as $ios)
    				    {
    				        ...
    				        if ($key) $guide_pic_arr[$k]['ios'][$key] = $url;
    				    }
    				}
    			}
		    }
		...
		if ($temp_info)
		{
			...
			if ($temp_attr)
			{
				...
				foreach ($temp_attr as $k => $attr)
				{
					...
					$attr_ids_extends_str .= ','.$attr['attr_ids_extends'];
				}
			...
			if ($ui_id)
			{
				...
				if ($ui_info)
				{
					foreach ($module_info as $k => $v)
					{
						...
						$attr_ids_extends_str .= ','.$ui_attr['attr_ids_extends'];
					}
				}
			}
		...
		/**
		if($attr_ids_extends_str)
		{
			$attr_ids_extends_arr = explode(',',$attr_ids_extends_str);
			if($attr_ids_extends_arr = @array_unique($attr_ids_extends_arr))
			{
				if($attr_ids_extends_arr = @array_filter($attr_ids_extends_arr))
				{
					if($_REQUEST['debug'])
					{
						print_r($attr_ids_extends_arr);exit;
					}
					$sql = "select id,flag from ".DB_PREFIX."app_attribute where id in (".implode(',', $attr_ids_extends_arr).")";
					$attr_ids_extends_query = $this->db->query($sql);
					$attr_ids_extends_info = array();
					while ($attr_ids_extends_rows = $this->db->fetch_array($attr_ids_extends_query))
					{
						if($attr_ids_extends_rows['flag']==1)
						{
							$attr_ids_extends_tem[] = $attr_ids_extends_rows['id'];
						}
						else
						{
							$attr_ids_extends_module[] = $attr_ids_extends_rows['id'];
						}
					}
				}
			}
...
//             		                $updateData['version_code'] = $client['version_code'] + 1;
            		                $updateData['version_name'] = $client['version_name'];
            		                $updateData['version_code'] = $client['version_code'];
        		                }
...
//     		    }
	...
	private function compare_data($info, $cache)
	{
	    ...
	    if ($info['icon']['web'] != $cache['icon']['web']) return false;
	    if ($info['startup_pic']['web'] != $cache['startup_pic']['web']) return false;
	    ...
	    if ($info['module'] != $cache['module'])
	    {
	        ...
	        foreach ($info['module'] as $infoModule)
	        {
	            ...
	            if ($infoModule['type'] != $modules[$infoModule['id']]['type']) return false;
	            if ($infoModule['solidify_id'] != $modules[$infoModule['id']]['solidify_id']) return false;
