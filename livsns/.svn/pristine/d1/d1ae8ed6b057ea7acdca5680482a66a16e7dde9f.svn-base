...
function commentAdd(obj){	
	...
	ajaxdata = { cmid :  $('#cmid').val(), content : cval, user_name : author , user_id : uid };
				...
				success:function(data){					
					if(!data){
						...
						$.each(data.data,function(index, item){
							...
							item.avatar = item.avatar ? item.avatar : "/m2o/images/avatar.png";
							item.content = (typeof AnalyticEmotion ==='function') ? AnalyticEmotion(item.content) : item.content;
							$(options.target).append('<li class="comment-item comment-item-'+item.id+' clearfix"><div class="comment-item-avatar fl"><a href="#"><img src="'+item.avatar+'" width="50" height="50"></a></div><div class="comment-item-main"><div class="comment-item-bar"><span class="comment-item-author"><a href="#">'+item.username+'</a></span><span class="comment-item-ip">'+item.ip+'</span><span class="comment-item-pubdate">'+item.pub_time+'</span></div><div class="comment-item-content">'+AnalyticEmotion(item.content)+'</div><!--<div class="comment-item-action"><a class="comment-item-digg" onclick="comment.digg(1236, 0);return false;" href="#">支持<span class="digg-num">(0)</span><span class="comment-item-increase">+1</span></a><a class="comment-item-reply" onclick="comment.reply(1236);return false;" href="#">回复</a><a href="#" class="comment-item-share" onclick="comment.share(1236);return false;">分享</a></div>--></div></li>');
						});
					}
...
function xcodeupdate(jqselector, typeid){

	if($(jqselector).size() ==0){return}

	$.ajax({
		url : '/m2o/xcode/get.php',
		data : {'type' : typeid, 'random' : Math.random()},
		dataType : 'json',
		timeout : 5000,
		success : function(data){
			if(data===null || data.length==0){return}
			$(jqselector).attr('src', data[0].img);
		}
	});
}
