...
class tuji_tuji_tuji extends adminBase
{
	public $field = 'id,expand_id,content_fromid,title,brief,tuji_sort_id,default_comment,user_name,create_time,ip,update_time,status,total_pic,keywords,catalog,total_visit,total_comment,cover_url,path,order_id,auto_cover,is_namecomment,is_orderby_name,is_add_water,water_id,tuji_source,url,weight';

	public $tablename = 'tuji_tuji_tuji';
	
	public $child_tablename = 'tuji_tuji_tuji_tuji_pics';
	
	public $child_table = 'tuji_pics';
	
	public $array_field = 'cover_url,latest';
	
	public $array_child_field = 'pic,img_info';
	...
	public function get_content_detail()
	{
		...
		if($id)
		{
			...
			if($need_child_detail && $this->child_tablename)
			{
				...
				$condition = " AND expand_id='".$id."' ORDER BY order_id ASC";
				//查询数据
