...
class site extends InitFrm
{
	...
	public function get_all_site($field = '*',$limit='', $condition = '')
	{
		$sql = "SELECT ".$field." FROM ".DB_PREFIX."site WHERE 1 " . $condition . " ORDER BY id ".$limit;
		$info = $this->db->fetch_all($sql);
		return $info;
	}
	...
	public function get_client($field = ' * ',$ids = '',$key = '')
	{
