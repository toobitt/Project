<?php
/***************************************************************************
* $Id: iconManage.class.php 17481 2013-04-19 09:36:46Z yaojian $
***************************************************************************/
class iconManage extends InitFrm
{
	...
	public function show($data)
	{
		...
		$sql = 'SELECT ' . $fields . ' FROM ' . DB_PREFIX . 'app_material WHERE 1';
		//获取查询条件
		...
		while ($rows = $this->db->fetch_array($query))
		{
		    if($rows['material_id'])
		    {
		        $rows['id'] = $rows['material_id'];
		    }
		    else 
		    {
		        $rows['material_id'] = $rows['id'];
		    }
	...
	public function count($data)
	{
		...
		$sql = 'SELECT COUNT(id) AS total FROM ' . DB_PREFIX . 'app_material WHERE 1';
	...
	public function getCategoryIdByTag($mark)
	{
	    $sql = 'SELECT id FROM ' . DB_PREFIX . 'icon_category WHERE 1';
	    if ($mark) $sql .= ' AND mark = "' . $mark . '"';
	    $q = $this->db->query($sql);
	    $info = array();
	    while ($rows = $this->db->fetch_array($q))
	    {
	        $info[$rows['id']] = $rows['id'];
	    }
	    return $info;
	}
	...
	private function get_condition($data)
	{
		...
		{
		    $condition .= " AND cate_id = " . $data['category_id'];
		}
		else
		{
		    $sql = 'SELECT id FROM ' . DB_PREFIX . 'icon_category';
		    $q = $this->db->query($sql);
		    $cate_id = '';
		    while ($rows = $this->db->fetch_array($q))
		    {
