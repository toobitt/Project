...
define('MOD_UNIQUEID','service_ggfw_create');//模块标识
require_once CUR_CONF_PATH.'lib/service.class.php';
class serviceGGFWCreateApi extends outerUpdateBase
{
	...
	/**
	 * 
	 * @Description  提交公共服务信息
	 * @author Kin
	 * @date 2013-10-26 上午10:00:49 
	...
	public function create()
	{
		$data = array(
				...
				'create_time'		=> TIMENOW,
				'is_secret'			=> intval($this->input['is_secret']),
				'password'			=> trim($this->input['password']),
				'id_type'			=> trim($this->input['id_type']),
				'cutomer_id'		=> intval($this->input['cutomer_id']),
				'type'				=> 2,
		);
		...
		if ($id)
		{
			$this->service->forward_common($data);
		}
		$this->addItem($data);
		$this->output();
	}
...
$ouput= new serviceGGFWCreateApi();
