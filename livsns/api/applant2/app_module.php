...
class app_module extends appCommonFrm
{
	...
	public function show()
	{
		...
		$count = isset($this->input['count']) ? intval($this->input['count']) : 1000;
		$data = array(
			'offset' => $offset,
			'count' => $count,
	 ...
	 */
	private function limitNum($app_id)
	{
	    $queryData = array(
	        'app_id' => $app_id
	    );
	    $nums = $this->api->count($queryData);
	    return $nums['total'] >= MODULE_LIMIT_NUM ? true : false;
	}
	...
	private function filter_data()
	{
		...
		*/
		$data = array(
		    ...
		    'is_sub' => $subscribe,
		    'icon_bg' => $icon_bg,
			'sign' => trim(urldecode($this->input['sign'])),
		);
		if ($ui_id > 0)
		{
		    $data['ui_id'] = $ui_id;
