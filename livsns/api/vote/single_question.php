...
class singleQuestionApi extends outerReadBase
{
	...
	//	$condition .= ' AND start_time < ' . TIMENOW . ' AND end_time > ' . TIMENOW;
		...
		$single_question = $this->mVoteQuestion->show2($condition,$device_token,$data_limit,'');
	...
	public function detail()
	{
		...
		$info = $this->mVoteQuestion->detail(' AND status=1',$offset,$count);
		if(!$info)
		{
			$this->errorOutput('未找到指定的数据');
		}
		if($device_token && $info['is_device'])
		{
	...
	public function pagedetail()
	{
		...
		$total=json_decode($this->pagecount(),1);
		$optionlist['page_info']['total_page']=ceil($total['total'] / $count);
		$optionlist['page_info']['total_num']=$total['total'];
		$optionlist['page_info']['page_num']=$count;
		$optionlist['page_info']['current_page']=$this->input['page'];
		$optionlist['info'] = $info['options'];
		$this->addItem($optionlist);
		$this->output();
	}
	...
	public function pagecount() {
		...
		$sql = 'select COUNT(*) AS total from '.DB_PREFIX.'question_option where vote_question_id = '.$this->input['id']." and title like '%$title%'";
