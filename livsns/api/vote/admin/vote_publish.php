...
class VotePublish extends adminUpdateBase implements publish
{
	...
	function get_content()
	{
		...
		{
			$sql = "SELECT * FROM " . DB_PREFIX ."vote_question WHERE node_id = {$sort_id}";
		}
		...
		while($row = $this->db->fetch_array($info))
		{
			...
			$row['struct_id'] = 'vote';
			$row['struct_ast_id'] = 'vote_option';
			$row['expand_id'] = '';
			$row['content_fromid'] = $row['id'];
			$row['indexpic'] = unserialize($row['pictures_info']);
			$row['ip'] = hg_getip();
			$row['user_id'] = $row['user_id'];
			$row['create_user'] = $row['user_name'];
			$row['brief'] = $row['describes'];
			unset($row['id']);
			$ret[] = $row;
		}
		$this->addItem($ret);
 	...
 	function update_content()
 	{
		...
		$sql = "SELECT * FROM " . DB_PREFIX ."vote_question WHERE id = " . $data['from_id'];
		$ret = $this->db->query_first($sql);
		if($ret['status'] != 1)
		{
			$sql = "UPDATE " . DB_PREFIX ."vote_question SET expand_id = 0, column_url = '' WHERE id = " . $data['from_id'];
		}
		else 
		{
			...
			$sql = "UPDATE " . DB_PREFIX . "vote_question SET expand_id = " . $data['expand_id'] . ", column_url = '" .serialize($url). "' where id = " . $data['from_id'];
		}
		$this->db->query($sql);
 	...
 	function delete_publish()
 	{
		...
		{
			$sql = "SELECT column_id,column_url FROM " . DB_PREFIX ."vote_question WHERE id = " . $data['from_id'];
			$ret = $this->db->query_first($sql);
			...
			$sql = "UPDATE " . DB_PREFIX ."vote_question SET column_id = '".addslashes(serialize($column_id))."', column_url = '".addslashes(serialize($column_url))."' WHERE id = " . $data['from_id'];
			$this->db->query($sql);						
		}	
		else		//全部删除
		{
			$sql = "UPDATE " . DB_PREFIX . "vote_question 
