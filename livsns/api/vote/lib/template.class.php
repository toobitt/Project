...
class template_mode extends InitFrm
{
	...
	public function generation($data,$template_file,$child_data)
	{
		...
		$forms = $this->generate_forms($child_data,$data['template_id']);
 		if($data['header_info'])
		{
			$header_info = $this->generation_info($data['header_info'],$data['template_id'],$content,'header_info');
		}
		if($data['footer_info'])
		{
			$footer_info= $this->generation_info($data['footer_info'],$data['template_id'],$content,'footer_info');
		}
	...
	public function generate_forms($form,$template_id)
	{
		...
		$comtemp = $this->get_component($template_id);
		if($form)
		{
			foreach ($form as $v)
			{
	...
	public function generation_info($info,$template_id,$content,$sign)
	{
		...
		if($info)
		{
			...
			foreach ($info as $v)
			{
				...
				$info_html .= $this->create_simple($comtemp[$sign], $v);
			}
		}
		return $info_html;
	}
		
	public function create_options($temp,$info,$need_other = 0)
	{
			...
			if($match_b && $info['options'])
			{
				if($info['is_other'] || $need_other)
				{
					$info['options'][] = array(
						...
						'title'			=> '其他',
						'is_other'		=> 1,
						'noplay'		=> !$info['is_other'] ? 1 : 0,
						'unique_name'	=> 'other_answer',
					);
				}
				foreach ($info['options'] as $key=>$op)
				{
					...
					if($op['is_other'])
					{
						...
						$op['unique_name'] = $op['unique_name'] ? $op['unique_name'] : 'other_answer['.$info['id'].']';
