...
class activate extends outerUpdateBase
{
	...
	{
		$activate_code 	= $this->input['activate_code'];//激活码
		...
		//判断有没有激活码，有的话就验证激活码，没有的话就默认是场外嘉宾
		$_code = array('id' => 0,'guest_type' => 1);
		if($activate_code)
		{
			//根据激活码查询有没有该激活码
			$code = $this->activate->isExistsActivateCode($activate_code);
			if(!$code)
			{
				//$code = $_code;
				$this->errorOutput(ACTIVATE_CODE_NOT_EXISTS_OR_USED);
			}
		}
		else //如果没有激活码默认嘉宾身份是场外嘉宾
		{
			$code = $_code;
		}
		...
		if($code['id'])
		{
			if(!$this->activate->update($code['id'],array('is_use' => 1)))
			{
				$commit_judge = 0;
			}
		}
		...
		//生成二维码,放在data目录
		$_vcard_data['id'] = $vid;
		$_vcard_data['guest_type_text'] = $this->settings['guest_type'][$code['guest_type']];
		...
		$data['id'] = $vid;
		$data['vcard_url'] = 'http://' . $this->settings['App_meeting']['host'] . '/' . $this->settings['App_meeting']['dir'] .'data/vcard/' . $_pic_name;  
		$data['avatar'] = $avatar;
		$data['guest_type'] = $code['guest_type'];
		$data['guest_type_text'] = $this->settings['guest_type'][$code['guest_type']];
		$this->addItem($data);
		$this->output();
	}
