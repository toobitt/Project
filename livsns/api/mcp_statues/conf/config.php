<?php
/***************************************************************************
* LivSNS 0.1
* (C)2004-2010 HOGE Software.
*
* $Id: config.php 4108 2011-06-27 06:33:52Z repheal $
***************************************************************************/
$gDBconfig = array(
	'host' => 'db.dev.hogesoft.com',
	'user' => 'root',
	'pass' => 'hogesoft',
	'database' => 'dev_mcp_mblog',
	'charset' => 'utf8',
	'pconncet' => '0',
);

define('DB_PREFIX', 'liv_');//定义数据库表前缀
define('WORDS_NUM', 140); //微博发表字数
define('TIME_SPACE', 60); //发布时间间隔单位为秒
define('TIME_NUM', 3); //发布次数限制
//define('DOMAIN','video.hcrt.cn/vod');

//define( "WB_AKEY" , '2613888264');
//define( "WB_SKEY" , '96a32662abb819e1ae64a03b470bb8cc');
define( "WB_AKEY", '2473706278');
define( "WB_SKEY", 'a113626eda457787b1911c59afe34863');

define('APP_UNIQUEID','weibo');

define('FRIENDS_MEM_PRE', 'friends'); //用户关注对象的memcache键前缀
define('FOLLOWERS_MEM_PRE', 'followers'); //用户粉丝的memcache键前缀
define('BLOCKERS_MEM_PRE', 'blockers');   //用户黑名单对象的memcache键前缀

define('INITED_APP', true);
?>