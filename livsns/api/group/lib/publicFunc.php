...
class publicFunc
{
	...
	//判断用户与讨论组的关系
	public function getLevelGroupAndUser($user_id, $group_id)
	{
		$this->curl = new curl($this->settings['App_group']['host'], $this->settings['App_group']['dir']);
		$this->curl->initPostData();
		$this->curl->addRequestData('user_id',$user_id);
		$this->curl->addRequestData('group_id',$group_id);
		$this->curl->addRequestData('a','level');
		$result = $this->curl->request('group.php');
		return $result['0'];
		var_dump($this->curl);exit;
	}
	...
	function spacecutstr($str, $length) {
		...
		$str = shtmlspecialchars(cutstr(strip_tags(preg_replace(array(
				"/\[hide=?\d*\](.+?)\[\/hide\]/is",
				"/\[quote](.*)\[\/quote]/siU",
				"/\[($bbcodes)=?.*\]/iU",
				"/\[\/($bbcodes)\]/i",
