...
class roadUpdateApi extends adminUpdateBase
{
	...
	public function create()
	{
		...
		if(is_array($this->input['area']))
		{
			$where = " AND id in (".implode(",",$this->input['area']).") ";
			$str = $this->obj->get_area($where);
		}
		else
		{
			$str = "";
		...
		$data = array(
			...
			"areas"					=> $str,
			'live_traffic'			=> $this->input['live_traffic'],
		);	
	...
	function update_comment_count()
	{
		...
		if($type)
		{
			$sql = "UPDATE " . DB_PREFIX . "road SET comm_num=comm_num" . $type . $comment_count . " WHERE id =" . $id ;
