...
define('MOD_UNIQUEID','road_update_user_queue');
class UpdateUserQueue extends cronBase
{
	...
	public function  show()
	{
		...
		if($user_info && $user_info['uid'] && !$user_info['error'])
		{
			...
			$sql = "UPDATE " . DB_PREFIX ."user SET  uid = '".$user_info['uid']."', avatar = '" . (serialize($avatar)) . "' WHERE id = " . $q['id'];
			$this->db->query($sql);
		}
	...
	public function batch_show() {
    	...
    	while ($row = $this->db->fetch_array($q)) {
			...
			if($user_info && $user_info['uid'] && !$user_info['error'])
			{
				$avatar = array('host'=>$user_info['avatar'],'dir' => '','filepath' =>'','filename' => '');
				$user_info['avatar'] = $avatar;
				$sql = "UPDATE " . DB_PREFIX ."user SET  uid = '".$user_info['uid']."', avatar = '" . (serialize($avatar)) . "' WHERE id = " . $row['id'];
				$this->db->query($sql);
			}
