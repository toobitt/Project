...
class road extends InitFrm
{
    ...
    public function show($con = "")
    {
        ...
        $sql = "SELECT r.*,g.title AS group_name,g.color,g.log AS glog, a.id as areaid ,a.name as areaname" .
                ...
                " ON r.group_id = g.id " .
                " WHERE 1  " . $con;
        //file_put_contents('sql1.txt',var_export($sql,1));
        $q    = $this->db->query($sql);
        $data = array();
    ...
    {
        $sql = "SELECT * FROM " . DB_PREFIX . "group  WHERE 1 ORDER BY order_id ASC";
        $q   = $this->db->query($sql);
        $ret = array();
        while ($row = $this->db->fetch_array($q))
        {
    ...
    public function get_area($cond)
    {
    	$sql  = "SELECT * FROM " . DB_PREFIX . "area WHERE 1 $cond";
        $q    = $this->db->query($sql);
        $data = "";
