<?php
define('MOD_UNIQUEID','app_extension');
require_once('global.php');
require_once(CUR_CONF_PATH . 'lib/app_extension_mode.php');
class app_extension extends outerReadBase
{
	private $mode;
    public function __construct()
	{
		parent::__construct();
		$this->mode = new app_extension_mode();
	}
	...
	public function SetExtenionField()
	{
	    $data = array(
				'app_id' => $this->input['app_id'] ? $this->input['app_id'] : 0,
				'user_id' => $this->input['user_id'],
				'user_name' => $this->input['user_name'],
				'create_time' => TIMENOW,
		);
        $extension_field = $this->input['extension_field'] ? $this->input['extension_field'] : array();
        $data['extension_field'] = serialize($extension_field);
		...
		$res = $this->mode->UpdateExtension($data['app_id'],$data);
