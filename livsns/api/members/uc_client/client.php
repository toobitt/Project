 ...
 */
function hg_utf8togbk($content,$is_stripslashes = false,$old_charset = 'utf-8',$new_charset = 'gbk',$is_enforce = false){
...
function uc_api_post($module, $action, $arg = array()) {
	...
	foreach($arg as $k => $v) {
		...
		$k=hg_utf8togbk($k);//iconv( "UTF-8", "gb2312",$k);
		$k = urlencode($k);
		if(is_array($v)) {
			$s2 = $sep2 = '';
			foreach($v as $k2 => $v2) {
				// 强制转换编码方式为gb2312，以兼容通行证
				$k2=hg_utf8togbk($k2);//iconv( "UTF-8", "gb2312",$k2);
				$k2 = urlencode($k2);
				// 强制转换编码方式为gb2312，以兼容通行证
				$tmp=hg_utf8togbk($v2,true);//iconv( "UTF-8", "gb2312",uc_stripslashes($v2));
			...
			// 强制转换编码方式为gb2312，以兼容通行证
			$v=hg_utf8togbk($v,true);//iconv( "UTF-8", "gb2312",uc_stripslashes($v));
	...
	return hg_gbktoutf8($result);//iconv( "gb2312", "UTF-8",$result);
}
