...
class appModule extends InitFrm
{
	...
	public function show($data)
	{
		...
		if ($solidify_info || $tpl_info)
		{
            foreach ($info as $k => $v)
            {
                ...
                {
                    $info[$k]['body_tpl'] = $tpl_info[$v['body_tpl_id']];
                }
            }
		}
		return $info;
	...
	{
	    $sql = 'SELECT id,body_html FROM ' . DB_PREFIX . 'body_tpl WHERE status = 2 AND id IN (' . $id . ')';
	    $query = $this->db->query($sql);
	    $info = array();
	    while ($rows = $this->db->fetch_array($query))
	    {
	        $rows['body_html'] = html_entity_decode($rows['body_html']);
	        $info[$rows['id']] = $rows['body_html'];
	    }
	    return $info;
	}
