...
class apps extends appCommonFrm
{
     ...
     * 更新扩展字段的相关数量
     */
    public function editCatalogNum()
    {
    	...
    	$radio_num = intval($this->input['radio_num']);
    	$time_num = intval($this->input['time_num']);
    	$price_num = intval($this->input['price_num']);
    	$list_ui_num = intval($this->input['list_ui_num']);
    	$content_ui_num = intval($this->input['content_ui_num']);
    	$main_num = intval($this->input['main_num']);
    	$minor_num = intval($this->input['minor_num']);
    	//根据user_id 拿到APP_INFO
    	$app_info = $this->api->getAppInfoByUserId($user_id);
    	$app_id = $app_info['id'];
    	
    	
    	//先查询有没有，，有update  没有则insert
    	$info = $this->api->detail('app_catalog_num', array(
    		'user_id'	=> $user_id,
    		'app_id'	=> $app_id,
    	));
    	if($info)
    	{
    		$this->api->update('app_catalog_num', array(
    			'list_ui_num'	=> $list_ui_num,
    			'content_ui_num'	=> $content_ui_num,
    			'time_num'		=> $time_num,
    			'price_num'		=> $price_num,
    			'radio_num'			=> $radio_num,
    			'main_num'		=> $main_num,
    	...
    	{
    		$this->api->create('app_catalog_num', array(
    				'list_ui_num'	=> $list_ui_num,
    				'content_ui_num'	=> $content_ui_num,
    				'time_num'		=> $time_num,
    				'price_num'		=> $price_num,
