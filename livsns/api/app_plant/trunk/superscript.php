...
//     	    }
    ...
    public function delete()
    {
        ...
        //看这个角标是否有模块在使用
        $module_use_info = $this->mode->cornerIsUse($id,$user_id);
        //看这个角标是否有组建在使用
        $comp_use_info = $this->mode->cornerIsUseInComp($id,$user_id);
        
        if($module_use_info || $comp_use_info)
        {
    ...
    //根据模块ID获取角标的条件
    public function getCornerCondByCompId()
    {
    	...
    	$comp_corner = $this->super_comp->getModCornerByCond(' AND comp_id ='.$comp_id);
...
//     	$mod_corner = $this->mode->getModCornerByCond(" AND module_id = '" .$module_id. "' ");
    	...
    	//查询出对应角标的条件，每个角标可能会有多个条件
    	foreach($comp_corner AS $k => $v)
    	{
    		//获取对应角标的条件
    		$corner_cond = $this->mode->getSuperscriptWithCondById($v['superscript_id'],'id,cond_type,is_show_text,field_type,text');
    		if($corner_cond)
    		{
    			$comp_corner[$k]['corner_info'] = $corner_cond;
    		}
    		else
    		{
    			$comp_corner[$k]['corner_info'] = array();
    		}
    	}
    
    	$this->addItem($comp_corner);
    	$this->output();
    }
    
    public function catalogIsInScriptCond()
    {
    	$user_id = intval($this->input['user_id']);
    	$cond_key = trim($this->input['catalog_field']);	
    	$info = $this->mode->getInfoByCata($cond_key,$user_id);
