...
class app extends InitFrm
{
	...
	public function getAddAppinfo($start_time = 0 , $end_time = 0)
	{
// 		$sql = "select count(*) as total from ".DB_PREFIX."app_info where create_time >".$start_time." and create_time < ".$end_time;
// 		$info = $this->db->query_first($sql);
// 		return $info;
		$sql = "SELECT * FROM ".DB_PREFIX."app_version where status = 1 and create_time > ".$start_time." and create_time < ".$end_time." group by app_id order by create_time asc";
	}
	
	public function getAddPush($start_time,$end_time)
	{
		$sql = "SELECT * FROM ".DB_PREFIX."promote_info where UNIX_TIMESTAMP(created_at) > ".$start_time." and UNIX_TIMESTAMP(created_at) < ".$end_time;
		$info = $this->db->query_first($sql);
		return $info;
	}
	
	public function getTodayPushInfo($zero_time = 0)
	{
		$sql = "select count(*) as total,t as hour from (SELECT id,user_name, DATE_FORMAT(created_At,'%H') as t,created_At FROM ".DB_PREFIX."promote_info where UNIX_TIMESTAMP(created_At) > ".$zero_time." ORDER BY `id` DESC) as new_table  group by new_table.t";
		$query = $this->db->query($sql);
		$info = array();
		while ($rows = $this->db->fetch_array($query))
		{
	...
	public function getTodayAddAppInfo($zero_time = 0)
	{
		$sql = "select count(*) as total,t as hour from (SELECT id,user_name, FROM_UNIXTIME(create_time,'%H') as t,create_time FROM ".DB_PREFIX."app_info where create_time > ".$zero_time." ORDER BY `id` DESC) as new_table  group by new_table.t";
		$query = $this->db->query($sql);
		$info = array();
		while ($rows = $this->db->fetch_array($query))
		{
	...
	public function getTodayBus_info($zero_time = 0)
	{
		$sql = "select count(*) as total,t as hour from (SELECT id,user_name, FROM_UNIXTIME(open_time,'%H') as t,create_time FROM ".DB_PREFIX."business_auth where open_time > ".$zero_time." ORDER BY `id` DESC) as new_table  group by new_table.t";
		$query = $this->db->query($sql);
		$info = array();
		while ($rows = $this->db->fetch_array($query))
		{
	...
	public function getTodayPushMsgInfo($zero_time = 0)
	{
		$sql = "select count(*) as total,t as hour from (SELECT id,user_name, FROM_UNIXTIME(create_time,'%H') as t,create_time FROM ".DB_PREFIX."push_msg where create_time > ".$zero_time." ORDER BY `id` DESC) as new_table  group by new_table.t";
		$query = $this->db->query($sql);
		$info = array();
		while ($rows = $this->db->fetch_array($query))
		{
	...
	public function getPushInfoInDate($start_time = 0 , $end_time = 0)
	{
		$sql = "select count(*) as total from ".DB_PREFIX."promote_info where UNIX_TIMESTAMP(created_At) > ".$start_time." and UNIX_TIMESTAMP(created_At) <".$end_time;
		$info = $this->db->query_first($sql);
		return $info;
	}
	...
	{
		$sql = "select count(*) as total from ".DB_PREFIX."app_info where pack_time != 0";
		$info = $this->db->query_first($sql);
		return $info;
		
	}
	
	public function appCountInDate($start_time , $end_time)
	{
		...
		{
			$sql = "SELECT count(distinct(app_id)) as total FROM ".DB_PREFIX."app_version where status = 1 and create_time > ".$start_time." and create_time < ".$end_time." order by create_time asc";
		}
		$info = $this->db->query_first($sql);
		return $info;
	}
	
	public function getAppNumsInDateBySource($start_time , $end_time , $client_type)
	{
		if($start_time == 0 && $end_time == 0)
		{
			$sql = "select count(*) as total from ".DB_PREFIX."app_version where status = 1 and client_type =".$client_type." group by app_id";
		}
		else
		{
			$sql = "SELECT count(distinct(app_id)) as total FROM ".DB_PREFIX."app_version where client_type = ".$client_type." and status = 1 and create_time > ".$start_time." and create_time < ".$end_time."  order by create_time asc";
		}
		$info = $this->db->query_first($sql);
		return $info;
	}
	
	public function getFirstCreateAndroidAppIndate($start_time = 0 , $end_time = 0 , $client_type = 0)
	{
		$sql = "SELECT * FROM ".DB_PREFIX."app_version where client_type = ".$client_type." and create_time > ".$start_time." and create_time < ".$end_time."  group by  app_id order by create_time asc";
		$query = $this->db->query($sql);
		$info = array();
		while ($rows = $this->db->fetch_array($query))
		{
