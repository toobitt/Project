...
class superscript_ui_attr_mode extends InitFrm
{
	...
	{
		$sql = "SELECT * FROM " . DB_PREFIX . "superscript_ui_attr  WHERE 1 " . $condition . $orderby . $limit;
		$q = $this->db->query($sql);
		$info = array();
	...
	public function create($data = array())
	{
		...
		$sql = " INSERT INTO " . DB_PREFIX . "superscript_ui_attr SET ";
		foreach ($data AS $k => $v)
		{
			$sql .= " {$k} = '{$v}',";
		}
		...
		$sql = " UPDATE ".DB_PREFIX."superscript_ui_attr SET order_id = {$vid}  WHERE id = {$vid}";
		$this->db->query($sql);
		return $vid;
	}
	...
	public function update($id,$data = array())
	{
		...
		//查询出原来
		$sql = " SELECT * FROM " .DB_PREFIX. "superscript_ui_attr WHERE id = '" .$id. "'";
		...
		//更新数据
		$sql = " UPDATE " . DB_PREFIX . "superscript_ui_attr SET ";
		foreach ($data AS $k => $v)
		{
			$sql .= " {$k} = '{$v}',";
		}
	...
	{
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "superscript_ui_attr WHERE 1 " . $condition;
	...
	public function delete($id = '')
	{
		...
		//查询出原来
		$sql = " SELECT * FROM " .DB_PREFIX. "superscript_ui_attr WHERE id IN (" . $id . ")";
		$q = $this->db->query($sql);
		$pre_data = array();
		while ($r = $this->db->fetch_array($q))
		{
		...
		//删除主表
		$sql = " DELETE FROM " .DB_PREFIX. "superscript_ui_attr WHERE id IN (" . $id . ")";
		$this->db->query($sql);
		return $pre_data;
	}
	...
	public function audit($id = '')
	{
		...
		$sql = " UPDATE " .DB_PREFIX. "superscript_ui_attr SET status = '" .$status. "' WHERE id = '" .$id. "'";
		$this->db->query($sql);
