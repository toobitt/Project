...
require_once(CUR_CONF_PATH . 'lib/appAttr.class.php');
...
class app_attribute extends appCommonFrm
{
    ...
    public function detail()
    {
        ...
        //多图显示图片处理
        if($appAttr_info['def_val'] && $appAttr_info['type'] == 'multiplefiles')
        {
            $_image = $this->_upYunOp->getPicInfoById($appAttr_info['def_val']);
            if($_image && isset($_image[0]) && $_image[0])
            {
                $appAttr_info['more_file_value'] = $_image[0];
            }
        }
        ...
        if ($table)
        {
            ...
            if ($relation)
            {
                if ($relation['name'])
                {
                    $appAttr_info['name'] = $relation['name'];
                }

                if ($relation['brief'])
                {
                    $appAttr_info['brief'] = $relation['brief'];
                }
    ...
    private function filter_data()
    {
        ...
        if (empty($attr_name))
        {
            $this->errorOutput(ATTR_NAME_NOT_EXISTS);
        }

        if (empty($attr_mark))
        {
            $this->errorOutput(ATTR_MARK_NOT_EXISTS);
        }

        if (empty($attr_type))
        {
            $this->errorOutput(ATTR_TYPE_NOT_EXISTS);
        }
        ...
        elseif($attr_type == 'multiplefiles')
        {
            if($_FILES['more_files_default'] && !$_FILES['more_files_default']['error'])
            {
                $_FILES['Filedata'] = $_FILES['more_files_default'];
                unset($_FILES['more_files_default']);
                $uploadRet = $this->upload();
                if($uploadRet)
                {
                    $data['def_val'] = $uploadRet['id'];
                }
            }
        }
        return $data;
