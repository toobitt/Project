...
class attribute_relate_update extends adminUpdateBase
{
	private $mode;
	private $_upYunOp;
	...
	{
	    $name         = $this->input['name'];//名称
	    $attr_id      = $this->input['attr_id'];//基础属性id
	    $ui_id        = $this->input['ui_id'];//ui的id
	    $role_type_id = $this->input['role_type_id'];//角色id 
	    $group_id     = $this->input['group_id'];//分组id 
	    
	    if(!$attr_id)
	    {
	        $this->errorOutput(NO_SELECT_BASE_ATTR);//未选择基础属性
	    }
	    ...
	    if(!$group_id)
	    {
	     ...
	     //判断同一个UI下同一个分组下同一种角色是否已经存在
	    if($role_type_id == -1)
	    {
	        $cond = " AND ar.ui_id = '" .$ui_id. "' 
    	    	      AND ar.attr_id = '" .$attr_id. "' 
	    ...
	    else 
	    {
	        $cond = " AND ar.ui_id = '" .$ui_id. "' 
    	    	      AND ar.attr_id = '" .$attr_id. "' 
    	    	      AND ar.group_id = '" .$group_id. "' 
    	    	      AND (ar.role_type_id = '" .$role_type_id. "' OR ar.role_type_id = -1) ";
	    }

	    if($this->mode->detail('',$cond))
	    {
	        $this->errorOutput(THIS_UI_GROUP_ROLE_HAS_EXISTS);
	    }
		...
		{
			$data['id'] = $vid;
			$this->addLogs('创建属性',$data,'','创建属性' . $vid);
	...
	public function delete()
	{
		...
		if($ret)
		{
			$this->addLogs('删除属性',$ret,'','删除属性' . $this->input['id']);
			$this->addItem('success');
			$this->output();
		}
	}
	...
	public function do_set_value()
	{
	    ...
	    $attr_type_id = $this->input['attr_type_id'];
	    if(!$attr_type_id)
	    {
	    ...
	    $vid = $this->mode->update($id,$update_data);
		if($vid)
		{
			$data['id'] = $vid;
			$this->addLogs('设置属性值',$data,'','设置属性值' . $vid);
			$this->addItem('success');
			$this->output();
		}
	}
...
$out = new attribute_relate_update();
