...
define('MOD_UNIQUEID','attribute_group');//模块标识
require_once(ROOT_PATH . 'frm/node_frm.php');
class attribute_group_update extends nodeFrm
{
	...
	public function delete()
	{
		...
		//查询分类
		$sql = 'SELECT * FROM '.DB_PREFIX.'attribute_group WHERE id IN ('.$this->input['id'].')';
		...
		while($row = $this->db->fetch_array($q))
		{
			...
			$data2[$row['id']]['content']['attribute_group'] = $row;
		}
		 $this->initNodeData();
		//判断是否成功删除
...
$out = new attribute_group_update();
