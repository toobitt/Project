...
class ui_attribute_update extends adminUpdateBase
{
	private $mode;
    private $_upYunOp;
	...
	public function create()
	{
	    ...
	    $ui_id          = $this->input['ui_id'];//所属ui的id
	...
	//绑定后台关联属性
	public function bind_relate_attr()
	{
	    $ui_attr_id = $this->input['ui_attr_id'];//前台属性的id
	    $attr_ids = $this->input['relate_ids'];//后台关联的属性id，可以是多个用逗号分隔
        if(!$ui_attr_id || !$attr_ids)
        {
            $this->errorOutput(NOID);
        }
        
        $attrIdsArr = explode(',', $attr_ids);
        if(!$attrIdsArr || !is_array($attrIdsArr))
        {
            $this->errorOutput(ATTR_IDS_ERROR);
        }
        
        $ret = $this->mode->bindAttr($ui_attr_id,$attrIdsArr);
        if($ret)
        {
            $this->addItem(array('error' => 0,'msg' => '绑定成功'));
            $this->output();
        }
	}
	...
	{
	    $this->drag_order('ui_attribute', 'order_id');
