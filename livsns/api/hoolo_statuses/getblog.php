...
***************************************************************************/
		...
		else
		{
			...
			else 
			{
				//取得固定用户的点滴信息
				$sql = "SELECT sta.* , exl.transmit_count,exl.reply_count,exl.comment_count FROM ".DB_PREFIX.
				"status sta  LEFT JOIN ".DB_PREFIX."status_extra exl ON sta.id = exl.status_id where sta.member_id=$u_id " . $newest_id_condition .  $ex_sta . "$extra ORDER BY sta.create_at ". $order . $this->end;
