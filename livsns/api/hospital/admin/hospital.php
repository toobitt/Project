...
class hospital extends adminReadBase
{
	private $mode;
    public function __construct()
	{
		$this->mPrmsMethods = array(
		'manage_hospital'	=>'管理医院',
		'manage_depart'		=>'管理科室',
		'audit'				=>'审核',
		'_node'=>array(
			'name'=>'医院名称',
			'filename'=>'hospital_node.php',
			'node_uniqueid'=>'hospital_node',
			),
		);
	...
	public function get_condition()
	{
		...
		if($this->user['group_type'] > MAX_ADMIN_TYPE)
		{
			...
			if($authnode = $this->user['prms']['app_prms'][MOD_UNIQUEID]['nodes'])
			{
				...
				if($authnode_str)
				{
					$condition .= ' AND t1.id IN (' . $authnode_str . ')'; 
				}
			}
			else 
			{
		...
		if($this->input['hospital_level'])
		{
			$condition .= " AND t1.level = " . intval($this->input['hospital_level']);
		}
