...
class {$class_name} extends BaseFrm
{
	protected $hg_extend_api	= '{$extend_api}';
	protected $hg_agruments 	= '{$args}';
	protected $hg_settings 		= '{$settings}';
	protected $hg_map_val 		= '{$map_val}';
	protected $hg_maps 			= '{$maps}';
	...
	function show()
	{
		$token = trim($this->hg_settings['token']);
		if ($token)
		{
			$token = explode(',', $token);
			if ($token[0] == 'exit')
			{
				...
				$stime = strtotime(date('Y-m-d ') . $time[0]);
		...
		else 
		{
			...
			if($this->hg_agruments)
			{
				foreach($this->hg_agruments['ident'] as $k=>$v)
				{
					...
					{
						$default_val = $this->hg_agruments['value'][$k];
						$len = strlen($v);
						$gv = substr($default_val, 0, $len);
						if ($gv == $v)
						{
							$tmpval = substr($default_val, $len);
							$op = substr($tmpval, 0, 1);
							if (in_array($op, array(' ', '+', '-', '*', '/', '%', '.')))
							{
								$default_val = substr($tmpval, 1);
							}
						}
						...
						elseif (isset($this->input[$v]))
						{
							switch ($op) 
							{
								case ' ':
								case '+':
									$getv = intval($this->input[$v]) + $default_val;
									break;
								case '-':
									$getv = intval($this->input[$v]) - $default_val;
									break;
			...
			//请求结果
			$data = $this->curl->request("{$file_name}");
