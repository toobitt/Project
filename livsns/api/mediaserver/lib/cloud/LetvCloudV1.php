...
define('FAILED',20);
define('WAIT', 30);
include(CUR_CONF_PATH . 'lib/video_cloud.php');   
    
/**
  * 提供访问乐视云视频开放平台的接口
  */
class LetvCloudV1 extends  videoCloud {
        ...
        public function upload()
        {
        	...
        	if (!$title)
        	{
        		$title = rawurldecode($this->mFiles['name']);
        	}
        	$ret = $this->videoUploadInit($title, hg_getip(), $this->mFiles['size']);
        	$ret = json_decode($ret, 1);
        	file_put_contents(CACHE_DIR . 'le.txt',var_export($ret, 1));
        	if ($ret['code'])
        	{
        		return $ret['message'];
        	}
        	$video = array(
        		'content_id' => $ret['data']['video_id'],
        		'extend_data' => $ret['data']['video_unique'],
        		'notranscode' => 1,
        	);
        	$this->setVideoInfo($video);
        	$upload_url = $ret['data']['upload_url'];
        	$ret = $this->videoUpload($this->mFiles['tmp_name'], $upload_url);
        	$ret = json_decode($ret, 1);        
        	if ($ret['code'])
        	{
        		return $ret['message'];
        ...
        public function getCloudVideo($video_id = 0)
        {
        	if (!$video_id)
        	{
        	...
        	$pu = $this->input['pu'];
        	$autoplay = intval($this->input['autoplay']);
        	...
        	$sql = 'SELECT * FROM ' . DB_PREFIX . 'vod_extend WHERE vodinfo_id=' . $video_id;
			...
			$this->user_unique = $this->settings['cloud_user'];
			$this->secret_key = $this->settings['cloud_secret_key'];
			$videoinfo = $this->videoGetPlayinterface($this->user_unique,$local_video['extend_data'], $type, $pu,$autoplay, $width, $height);
...
//				exit($swf);
				$data = array(
					'totalsize' => $videoinfo['data']['video_duration'] * 1000,
					'duration' => $videoinfo['data']['initial_size'],
					'update_time' => time(),
					'is_link' => 1,
					'swf' => $swf
				);
        ...
        public function videoGetPlayinterface($uu,$vu,$type,$pu="",$auto_play=null,$width=0,$height=0){
        	...
        	if($type == "url"){
        		...
        		$res = '<script type="text/javascript">var letvcloud_player_conf = '.$jsonString.';</script>';
        	}elseif ($type == "flash"){
        		$res = "http://yuntv.letv.com/bcloud.swf?".$queryString;
        	}elseif ($type == "html"){
        		$res = '<embed src="http://yuntv.letv.com/bcloud.swf" allowFullScreen="true" quality="high" width="' . $args['width'] . '" height="' . $args['height'] . '" align="middle" allowScriptAccess="always" flashvars="'.$queryString.'" type="application/x-shockwave-flash"></embed>';
        	}
        	else
        	{
        		$res = '<script id="autoJs' . $vu . '" type="text/javascript">var pNode=document.getElementById("autoJs' . $vu . '").parentNode,pnd=pNode.style,dWidth=pnd.width,pHeight=pnd.height,dHeight=ReCallHeight(pHeight,pNode);function ReCallHeight(e,t){if(e==""||e=="100%"){var n=t.parentNode,r=t.parentNode.style.height;if(r)return r;var i=ReCallHeight(r,n);return i}return e} var letvcloud_player_conf = {"uu":"' . $uu . '","vu":"' . $vu . '","pu":"' . $pu . '","auto_play":' . $auto_play . ',"gpcflag":1,"width": dWidth,"height": dHeight};</script><script type="text/javascript" src="http://yuntv.letv.com/bcloud.js"></script>';
        	}
        	return $res;
        }
