...
class jpush 
{
	...
	function __construct($masterSecret = '',$appkeys = '') 
	{
		...
		if(!$this->_masterSecret)
		{
			if(defined('MASTER_SECRET') && MASTER_SECRET)
			{
				$this->_masterSecret = MASTER_SECRET;
			}
		}
		...
		if(!$this->_appkeys)
		{
			if(defined('JPUSH_APP_KEY') && JPUSH_APP_KEY)
			{
				$this->_appkeys = JPUSH_APP_KEY;
			}
		}
	}
	/**
