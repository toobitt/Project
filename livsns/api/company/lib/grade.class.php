...
class grade extends InitFrm
{
	...
	public function show($data)
	{
		...
		$sql = 'SELECT * FROM ' . DB_PREFIX . 'grade WHERE 1';
		//获取查询条件
	...
	public function count($data)
	{
		...
		$sql = 'SELECT COUNT(id) AS total FROM ' . DB_PREFIX . 'grade WHERE 1';
	...
	{
		$sql = 'SELECT * FROM ' . DB_PREFIX . 'grade WHERE id = ' . intval($id);
		return $this->db->query_first($sql);
	}
	...
	{
		$sql = 'SELECT DISTINCT role_id, role_name FROM ' . DB_PREFIX . 'grade ORDER BY role_id';
		$query = $this->db->query($sql);
		$info = array();
		while ($rows = $this->db->fetch_array($query)) {
			$info[] = $rows;
