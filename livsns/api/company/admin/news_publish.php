...
class NewsPublish extends adminUpdateBase implements publish
{
    ...
    public function get_content()
    {
        ...
        $this->curl = new curl($this->settings['App_news']['host'], $this->settings['App_news']['dir']);
        ...
        $ret = $this->curl->request('admin/news_publish.php');
        $ret = $ret[0];
        $this->addItem($ret);
        $this->output();
    }
    ...
    {    	
        ...
        $this->curl->request('admin/news_publish.php');
        //更新叮当数据
        $sql = "SELECT * FROM " . DB_PREFIX . "content WHERE source = 'news' AND source_id = " . $data['from_id'];
        $ret = $this->db->query_first($sql);
      	if ($ret['state'] != 1)
        {
            $sql = "UPDATE " . DB_PREFIX . "content SET publish_id = 0, order_id = 0  WHERE source = 'news' AND source_id = " . $data['from_id'];
        ...
        else 
        {
        	if (!empty($data['content_url']) && is_array($data['content_url']))
            {
            	...
            	$sql = "UPDATE " . DB_PREFIX . "content SET publish_id = " . $publish_id . ",order_id = ".$publish_id." WHERE source = 'news' AND  source_id = " . $data['from_id'];
        		$this->db->query($sql);
            }
        }
