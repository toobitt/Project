...
class TujiPublish extends adminUpdateBase implements publish
{
	private $curl;
    ...
    public function get_content()
    {
        ...
        $this->curl = new curl($this->settings['App_tuji']['host'], $this->settings['App_tuji']['dir']);
        ...
        $ret = $this->curl->request('admin/tuji_publish.php');
        $ret = $ret[0];
        $this->addItem($ret);
        $this->output();
    }
    ...
    public function update_content()
    {
        ...
        $this->curl->request('admin/tuji_publish.php');
        //更新叮当数据
        $sql = "SELECT * FROM " . DB_PREFIX . "content WHERE source = 'photo' AND source_id = " . $data['from_id'];
        $ret = $this->db->query_first($sql);
      	if ($ret['state'] != 1)
        {
            $sql = "UPDATE " . DB_PREFIX . "content SET publish_id = 0, order_id = 0  WHERE source = 'photo' AND source_id = " . $data['from_id'];
        ...
        else 
        {
        	if (!empty($data['content_url']) && is_array($data['content_url']))
            {
            	...
            	$sql = "UPDATE " . DB_PREFIX . "content SET publish_id = " . $publish_id . ",order_id = ".$publish_id."  WHERE source = 'photo' AND  source_id = " . $data['from_id'];
        		$this->db->query($sql);
            }
        }
