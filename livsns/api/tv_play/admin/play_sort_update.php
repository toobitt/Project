...
define('MOD_UNIQUEID','play_sort');//模块标识
require_once(ROOT_PATH . 'frm/node_frm.php');
class play_sort_update extends nodeFrm
{
	public function __construct()
	{
		parent::__construct();
        $this->setNodeTable('play_sort');
        $this->setNodeVar('play_sort');
	...
	public function delete()
	{
	    ...
	    //删除分类之前先判断该分类下面有没有电视剧，如果有的话就不能删除
	    $sql = "SELECT * FROM ".DB_PREFIX."tv_play WHERE play_sort_id = '" .$this->input['id']. "'";
	    $_data = $this->db->query_first($sql);
	    if($_data)
	    {
	    	$this->errorOutput(THIS_SORT_EXISTS_TV_PLAY);
	    }

		//查询分类
		$sql = 'SELECT * FROM '.DB_PREFIX.'play_sort WHERE id IN ('.$this->input['id'].')';
		...
		while($row = $this->db->fetch_array($q))
		{
			...
			$data2[$row['id']]['content']['play_sort'] = $row;
		}
		 $this->initNodeData();
		//判断是否成功删除
	...
	public function drag_order()
	{
		...
		if(!empty($sort))
		{
			foreach($sort as $key=>$val)
			{
				...
				{
					$sql ="UPDATE " . DB_PREFIX . "play_sort SET";
