...
class feedback_feedback_feedback extends adminBase
{
	public $field = 'id,content_fromid,title,brief,node_id,column_id,status,counts,is_login,is_login,order_id,org_id,user_id,user_name,create_time,update_user_id,update_user_name,update_time,audit_user_id,audit_user_name,audit_time,ip,appid,appname,expand_id,column_url,pub_time';

	public $tablename = 'feedback_feedback_feedback';
	
	public $child_tablename = '';
	
	public $child_table = '';
	
	public $array_field = 'column_id';
		...
		if($id)
		{
			...
			if($result == 'no_file_dir' || !$this->obj->is_mk_cache($need_pages,$need_child_detail,$result['need_pages'],$result['need_child'],$need_process,$result['need_process'],$need_separate,$result['need_separate']))
			{
				...
				if(!empty($con_detail['content']))
				{
					...
					$child_data1 = $after_process['content_pics'];
				}
...
$out = new feedback_feedback_feedback();
$action = $_INPUT['a'];
