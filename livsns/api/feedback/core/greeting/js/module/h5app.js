	...
	H5App.prototype = {
		...
		init : function(){
			...
			if( !!op.needSystemInfo ){
				...
				self.callWebviewMethod('callSystemInfo', 'getSystemInfo');
			}
		...
		gainElemParam : function( event ){
			...
			if( params ){
				...
				this.callWebviewMethod( operation, operation );
			}
		},
			...
			if( callfunc == 'sharePlatsAction' ){
				if( !this.wificz ){
					...
					if (t.length > 0) {
						...
						t = $('<div id="tip-share" class="tip-share-wechat"><img src="./images/tip-share-wechat.png" width="100%" alt="分享提示" /></div>').css({
							position:'absolute',
							width:'100%',
							top:0,
							left:0,
		...
		getSystemInfo : function( json ){
			var self = $.my,
				op = self.op;
			self.appSystemInfo = json.device_token ? json : json.deviceInfo;
