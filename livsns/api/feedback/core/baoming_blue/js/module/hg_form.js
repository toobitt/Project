(function(){
	...
	$.extend( hg_Submit.prototype, {
		...
		init : function(){
			var opt = this.opt;
			if( !opt.form.is('form') ){
				opt.errorCallback && opt.errorCallback.call( this, opt.errorMsg.form_error );
			}
		...
		ajaxform : function( dom ){
			...
			if( strTip ){
				opt.errorCallback && opt.errorCallback.call( this, strTip );
				return false;
			}
			...
			$.ajax({
				...
				error : function(){
	        		opt.errorCallback && opt.errorCallback.call( this, '接口访问错误，请稍候再试' );
	        		utils.spinner && utils.spinner.close();
	        	}
			});
		},
