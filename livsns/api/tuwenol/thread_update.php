<?php
define('ROOT_DIR', './../../');
define('MOD_UNIQUEID','thread_update');
define('SCRIPT_NAME', 'thread_update');
...
class thread_update extends adminBase
{
	...
	function create()
	{
		...
		'content'=>$content,
		'aid'=>$aid,
		'client'=>$this->input['client'] ? $this->input['client'] : $this->user['user_name'],
		'status'=>0,
		'create_time'=>TIMENOW,
	...
	function get_transcode_progress()
	{
		...
		if($data)
		{
			...
			foreach ($data as $v)
			{
				 ...
				 {
				 	 $info = array('transcode_percent' => 100,'id' => $ret['id']);
				 }
				 else 
				 {
				 ...
				 if($info['transcode_percent'] == 100)
				 {
				 	...
				 	$livmedia->addRequestData('id',$ret['id']);
			...
			if($return)
			{
				foreach($return as $val)
				{
					if($val['m3u8'])
					{
						...
						$sql = 'UPDATE ' . DB_PREFIX . 'attach set uri="'.addslashes(serialize($uri)).'",type="'.$type.'" where id='.$val['aid'];
						$this->db->query($sql);
					}
					$this->addItem($val);
				}
