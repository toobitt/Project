...
class thread extends adminBase
{
	...
	function show_comment()
	{
		...
		{
			$where .= ' AND thid = ' . intval($this->input['thread_id']);
		}
		...
		{
			$where .= ' AND create_time > '.intval($this->input['create_time']);
		}
	...
	{
		$sql = 'SELECT * FROM '  . DB_PREFIX . 'thread WHERE id = '.intval($this->input['id']) . ' AND status=1';
		$data = $this->db->query_first($sql);
		if(!$data)
		{
			$this->errorOutput("帖子不存在或被删除");
		}
		if($data['aid'])
		{
