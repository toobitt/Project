...
class  CategoryUpdateAPI extends outerUpdateBase
{
    private $tbname = 'cate';
	...
	public function create()
	{
		...
		if($createCategoryNumMax = $this->settings['createCategoryNumMax'])
		{
			$sql = 'select count(*) as total from '.DB_PREFIX.'cate where 1 and type = 2 and user_id = '.$this->user['user_id'];
			$total = $this->db->query_first($sql);
			if($total['total'] > $createCategoryNumMax)
			{
				$this->errorOutput('对不起,您创建的专辑数已达上限');
			}
		}
	...
	public function delete()
	{
		...
		$cond = " where id in ($id) AND `user_id`=".$this->user['user_id'];
