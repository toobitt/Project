...
class control extends adminbase {
	...
	function control() {
		parent::__construct();
		if((getgpc('m') !='hg_user') && (getgpc('a') != 'login' && getgpc('a') != 'logout')) {
	...
	function onls() {
		...
		if(!empty($_REQUEST['addname'])) {
			$username = urldecode(getgpc('addname', 'P'));
			$password =  urldecode(getgpc('addpassword', 'P'));
			$email = urldecode(getgpc('addemail', 'P'));
			...
			{
				$status = $this->_check_username($username);
				echo json_encode($status);
				exit;
			}
		}
		...
		if(!empty($_REQUEST['delete'])) {
			$_REQUEST['delete'] = explode(',', urldecode($_REQUEST['delete']));
			$_ENV['user']->delete_user($_REQUEST['delete']);
			$status = 2;
			echo $status;exit;
		//	$this->writelog('user_delete', "uid=".implode(',', $_REQUEST['delete']));
		}
		$srchname = urldecode(getgpc('srchname', 'R'));
		$srchregdatestart = urldecode(getgpc('srchregdatestart', 'R'));
		$srchregdateend = urldecode(getgpc('srchregdateend', 'R'));
		$srchuid = intval(getgpc('srchuid', 'R'));
		$srchregip = trim(getgpc('srchregip', 'R'));
		$srchemail = urldecode(trim(getgpc('srchemail', 'R')));
		...
		foreach($userlist as $key => $user) {
			$user['smallavatar'] = UC_API . '/avatar.php?uid='.$user['uid'].'&size=small';
			$user['bigavatar'] = UC_API . '/avatar.php?uid='.$user['uid'].'&size=big';
			$user['bigavatarreal'] = UC_API . '/avatar.php?uid='.$user['uid'].'&size=big&type=real';
			$userlist[$key] = $user;
		}
	...
	function onedit() {
		...
		if($uid) {	//$this->submitcheck()
			$username = urldecode(getgpc('username', 'P'));
			$newusername = urldecode(getgpc('newusername', 'P'));
			$password = urldecode(getgpc('password', 'P'));
			$email = urldecode(getgpc('email', 'P'));
			$delavatar = urldecode(getgpc('delavatar', 'P'));
			$rmrecques = urldecode(getgpc('rmrecques', 'P'));
			$sqladd = '';
			...
			if(($status = $this->_check_email($email)) == -4)
			{
				echo $status;
				exit;
			}
		...
		$user['bigavatar'] = 'avatar.php?uid='.$uid.'&size=big';
		$user['bigavatarreal'] = 'avatar.php?uid='.$uid.'&size=big&type=real';
	...
	function onhg_check_username()
	{
		$username = urldecode(getgpc('username', 'P'));
		$ret = $this->_check_username($username);
		echo json_encode($ret);
	}

	function onhg_check_email()
	{
		$email = urldecode(getgpc('email', 'P'));
		$ret = $this->_check_email($email);
		echo json_encode($ret);
	}
