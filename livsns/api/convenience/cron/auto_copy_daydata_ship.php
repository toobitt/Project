<?php
require('global.php');
define('MOD_UNIQUEID','auto_copy_daydata_ship');//模块标识
...
class auto_copy_day extends cronBase
{
	...
	{
		$max_time= strtotime(date('Y-m-d',strtotime('+'.abs(intval(MAX_SEARCH_TIME_RANGE_SHIP)).' DAY')));//最大复制时间
		$run_time=strtotime(date('Y-m-d'));
		if(empty($max_date))
		{
			$sql="SELECT MAX(departDate) as max_date FROM ".DB_PREFIX."bus_query WHERE type = 1";
		...
		$condition .= " {$link} departDate < ".$run_time. " AND type = 1";
		if($condition)
		{
	...
	{
		$condition='';
		$condition .= " AND type = 1 AND departDate = ".($run_time-24*60*60);
		return $condition;
	}
}
