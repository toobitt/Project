<?php
define('MOD_UNIQUEID','shipmanage');
require_once('global.php');
require_once(CUR_CONF_PATH . 'lib/bus_manage.class.php');
class ship_types_update extends adminUpdateBase
{
	...
	public function update()
	{
		...
		{
			$this->errorOutput('上船站,到达站,终点站均不允许为空!');
		}
		...
		if($ret)
		{
			$this->addLogs('更新船次',$ret,'','更新' . $this->input['id']);
			$this->addItem($ret);
			$this->output();
		}
	}
	public function copy()
	{
		...
		if($ret)
		{
			$this->addLogs('添加船次',$ret,'','更新' . $this->input['id']);
			$this->addItem($ret);
			$this->output();
		}
	}
