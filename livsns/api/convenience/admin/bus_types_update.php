...
class bus_types_update extends adminUpdateBase
{
	...
	public function create()
	{
		...
		if($ret)
		{
			$this->addLogs('添加车次',$ret,'','添加' . $this->input['id']);
			$this->addItem($ret);
			$this->output();
		}
	}
	...
	public function copy()
	{
		...
		if($ret)
		{
			$this->addLogs('添加车次',$ret,'','复制' . $this->input['id']);
			$this->addItem($ret);
			$this->output();
		}
	}
	public function excel_update()
	{
		...
		if ((($extend == "xls")&&($_FILES["file"]["size"] < 200000)))
		{
			...
			{
				$isupload=$this->excel->show($uploadfile,$tmp_name);
				if($isupload)
				{
	...
	public function delete()
	{
		...
		if($ret)
		{
			$this->addLogs('删除车次',$ret,'','删除' . $this->input['id']);
	...
	public function get_condition()
	{
		...
		if($this->input['station'])
		{
			$condition .= ' AND  departStation  IN (\''.trim(urldecode($this->input['station'])).'\')';
		}
