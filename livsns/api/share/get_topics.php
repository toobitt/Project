...
class get_topicsAPI extends adminBase
{
	...
	public function get_topics()
	{
		...
		if($checktoken['msg'] !== 'new' && $checktoken['msg'])
		{
			...
			$action = $this->settings['share_plat'][$platdata['type']]['name'].'_topics';
			$ret = $this->$action($checktoken,$page,$count,$keyword);
			$this->addItem($ret);
			$this->output();
		}
		else
	...
	public function sinaweibo_topics($checktoken,$page,$count,$keyword)
	{
		if($checktoken)
		{
			...
			$result = $c->get_topic( $this->settings['share_plat'][$checktoken['data']['type']]['topicurl'],$keyword,$page,$count);
//			print_r($result);exit;
			$ret = array();
...
//			print_r($ret);exit;
	...
	public function renren_topics($tokendata,$platdata)
	{
		if($tokendata)
		{
