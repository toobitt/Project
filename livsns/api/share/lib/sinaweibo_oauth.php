...
//		print_r($params);exit;
    ...
    function get_home_timeline($url, $since_id, $page, $count)
	{
		if (!$url) {
		...
		$params['oauth_consumer_key'] = $this->oauth->client_id;
		$params['format'] = 'json';
		$params['page'] = $page ? intval($page) : 1;
		$params['count'] = $count ? intval($count) : 1;
		if($since_id)
		{
			$params['since_id'] = $since_id;
		}
		$params['access_token'] = $this->oauth->access_token;
		$params['trim_user'] = 0;
		return $this->oauth->get($url, $params , true);
	}	
	
	
	function get_topic($url = '',$keyword,$page,$count)
	{
		$params=array();
		$params['oauth_version'] = '2.a';
		$params['oauth_consumer_key'] = $this->oauth->client_id;
