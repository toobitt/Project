...
*
* $Id: curl.class.php 19826 2013-04-07 01:15:41Z develop_tong $
***************************************************************************/
...
class curl
{
	...
	public function addFile($file)
	{
		if(isset($file))
		{
			...
			{
				if (is_array($val['tmp_name']) && count($val['tmp_name']) > 0 )
				{
    ...
    public function request($file)
    {
		...
		if($this->globalConfig['appid'] && $this->globalConfig['appkey'])
		{
			$this->addRequestData('appid', $this->globalConfig['appid']);
			$this->addRequestData('appkey', $this->globalConfig['appkey']);
		}
		else
		{
		...
		{
			$userLoginInfo =  unserialize(stripslashes($_COOKIE['user']));
		}
		else if ($_COOKIE['user'])
		{
			$userLoginInfo =  $_COOKIE['user'];
			if($_userLoginInfo = unserialize($userLoginInfo))
			{
						$userLoginInfo = $_userLoginInfo;
						unset($_userLoginInfo);
			}
			else 
			{
				$userLoginInfo = unserialize(stripslashes($userLoginInfo));
			}
		}		
		$userLoginInfo && ! is_array($userLoginInfo) && $userLoginInfo = json_decode($userLoginInfo,true);		
		if(!is_null(@json_decode($userLoginInfo)))
		{
			$userLoginInfo = json_decode($userLoginInfo,true);
			$this->addRequestData('access_token', $userLoginInfo['access_token']);			
		}
		else if (is_array($userLoginInfo)) 
		{
			$this->addRequestData('access_token', $userLoginInfo['access_token']);
		}
		/**
