...
class grade extends adminReadBase
{
	private $mode;
	...
	public function show()
	{
		...
		if(!empty($ret))
		{
			foreach($ret as $k => $v)
			{
				if($v['people_num'])
				{
					$v['scores'] = round($v['total']/$v['people_num'],1);
				}
				$v['c_create_time'] = date('Y-m-d H:i',$v['c_create_time']);
				$index_pic =  unserialize($v['index_pic']);
				$v['index_pic'] = $index_pic['host'].$index_pic['dir'].$index_pic['filepath'].$index_pic['filename'];
				$this->addItem($v);
			}
			$this->output();
		}
	...
	public function detail()
	{
		if($this->input['id'])
		{
			...
			if($ret['people_num'])
			{
				$ret['scores'] = round($ret['total']/$ret['people_num'],1);
			}
			if($ret)
			{
				$this->addItem($ret);
