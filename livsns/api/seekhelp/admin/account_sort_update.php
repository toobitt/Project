...
define('MOD_UNIQUEID','account_sort');//模块标识
require_once(ROOT_PATH . 'frm/node_frm.php');
class account_sort_update extends nodeFrm
{
	public function __construct()
	{
		parent::__construct();
        $this->setNodeTable('account_sort');
        $this->setNodeVar('account_sort');
	...
	public function delete()
	{
	    ...
	    */
		...
		//查询分类
		$sql = 'SELECT * FROM '.DB_PREFIX.'account_sort WHERE id IN ('.$this->input['id'].')';
		...
		while($row = $this->db->fetch_array($q))
		{
			...
			$data2[$row['id']]['content']['account_sort'] = $row;
		}
		 $this->initNodeData();
		//判断是否成功删除
	...
	public function drag_order()
	{
		...
		if(!empty($sort))
		{
			foreach($sort as $key=>$val)
			{
				...
				{
					$sql ="UPDATE " . DB_PREFIX . "account_sort SET";
