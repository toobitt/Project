...
(function(window, doc) {
	var m = Math, dummyStyle = doc.createElement("div").style, vendor = (function() {
		...
		return false
			...
			&& !isTouchPad, hasTransform = vendor !== false, hasTransitionEnd = prefixStyle("transition") in dummyStyle, RESIZE_EV = "onorientationchange" in window
			? "orientationchange"
	...
	iScroll.prototype = {
		...
		_scrollbarPos : function(dir, hidden) {
			...
			if (pos < 0) {
				if (!that.options.fixedScrollbar) {
					...
					that[dir + "ScrollbarIndicator"].style[dir == "h"
							? "width"
							: "height"] = size + "px"
				}
				pos = 0
		...
		scrollToElement : function(el, time) {
			...
			pos.left += that.wrapperOffsetLeft;
			pos.top += that.wrapperOffsetTop;
			pos.left = pos.left > 0 ? 0 : pos.left < that.maxScrollX
					? that.maxScrollX
					: pos.left;
		...
		zoom : function(x, y, scale, time) {
			...
			that.scale = scale;
			that.refresh();
			that.x = that.x > 0 ? 0 : that.x < that.maxScrollX
					? that.maxScrollX
					: that.x;
