	...
	$.extend( Road.prototype, {
		...
		initMarker : function(marker, param){
			...
			marker.addEventListener('click', function(e){
				...
				var box = $('.getloc-wrap').find('.map_move'); 
				if( box.length > 1 ){
					current = box.last();
					current.siblings('.map_move').remove();
				}else{
					current = box;
				}
		...
		showmySquare : function( param, type ){
			var _this = this;
			var map = this.map;
			var mySquare = new SquareOverlay( param, 350, 100 );
		...
		detail : function( id ){
			...
			this.ajax( url, {road_id :id}, function(json){
				...
				_this.initBmap(data.longitude , data.latitude);/*实例化地图*/
				_this.closeLoading();
		...
		initBmap : function(lat , lng){
			...
			var i = 1;
			map.addEventListener("tilesloaded",function(){/*地图加载完的监听事件*/
				if(i==1)
				{
