	...
	$.extend( Bike.prototype, {
		...
		initMap : function(){
			...
			Widget.ready = function(){
				_this.map = new Widget.CMap.Map('divMap');
				_this.getgps( function( point ){
					_this.point = point || {lat : 31.490620682283, lng : 120.310796499252}
					_this.listAjax( {
						method : 'bikenearest',
						param : {
							baidu_latitude : _this.point.lat,
		...
		ajaxDetailInfo : function( info, box, type ){
			...
			this.ajax(url, info.param, function( json ){
				if( type ){
					...
					_this.initBmap( data.latitude ,data.longitude );/*实例化地图*/
