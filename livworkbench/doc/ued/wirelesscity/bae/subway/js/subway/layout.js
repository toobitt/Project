...
$.templete = { 
	...
	/*线路规划*/
	map_area_list : 
		'<area shape="circle" data-id="<%= id%>" data-line="<%=line%>" data-istoilet="<%= has_toilet%>" data-color="<%= sub_color%>" data-title="<%= title%>" href="#" coords="<%= site_x%>,<%= site_y%>,40" />',
	station_pop : 
		'<div class="station-pop station-btn" _id="<%=id%>" _title="<%=title%>">' + 
			'<div class="station-pop-head <% if(!istoilet){%>no-toilet<%}%>">' +
				'<span class="icons">' +
					'<% if( station_color.length ){for( var key in station_color ){%> ' +
						'<em class="fontsize16" style="background:<%= station_color[key]%>"><%=line[key]%></em>' +
					'<%}}%>' + 
				'</span>' +
