		...
		live_detail : function( self, info ){
			...
			tool.ajax( url, info, function( json ){
				...
				if( $.isArray( json ) && json[0] ){
					var data = json[0];
					data.poster = './images/loading_default_bg.png'
					data.type = (data.audio_only == '1') ? 'audio' : 'video';
					var html = tool.render( tool.tpl.live_tpl, data );
					...
					var currentDom = _this.initWeek( page, info.channel_id, data.type );
					_this.program( currentDom, info, data.type, 'detail' );
				}
			});
		},
		...
		program : function( page, info, type, style ){
			...
			tool.ajax( url, info, function( json ){
				if( $.isArray( json ) && json[0] ){
					...
					$( html ).appendTo( page );
					if( style == 'detail' ){
						var wrap = page.closest('.page-content');
