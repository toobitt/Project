(function(){
	...
	$.extend( hg_Submit.prototype, {
		...
		initVary : function(){
			var opt = this.opt;
			if( isAndroid ){
				opt.form.find('.file-set .view').hide();
			}
			
			var deviceLimit = opt.deviceLimit;		//设备限制信息
			var is_limit = false;
			if( deviceLimit ){
				!!hgClient && hgClient.getPlat(function( response ){
					if( response == 'other' || response == 'pc' ){
						...
						if( deviceLimit.is_login == 1 || deviceLimit.is_device == 1 ){
							is_limit = true;
							var limit = $('<div class="limitBox"/>').prependTo( 'body' )
								.css({
									width : '100%',
									color : '#dd4a68',
									'text-align' : 'center',
								...
								.html( '该表单需要设备登录信息才能操作' );
						}
					}
					$.isFunction( opt.deviceLoaded ) && opt.deviceLoaded( true );			//限制信息加载完全
				});
			}
			if( !is_limit ){
				opt.submitBtn.removeClass('btn-disable');
			}
		},
		...
		address : function(){
			var addressArea = this.opt.form.find('.liv_address');
			if( addressArea.length ){
				addressArea.address({
					together  : false,
					provId : '#A_prov',
					cityId : '#A_city',
