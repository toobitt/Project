...
$_INPUT['trans_status'] = $_INPUT['trans_status'] ? $_INPUT['trans_status'] : -2;
 ?>
<!--  状态搜索模版 -->
<?php 
$value = $_INPUT['trans_status'];
 ?>
<div class="new-search-item new-search-status" data-type="select">
	...
	<span class="condition-area">
		<span class="current-condition-show"><?php echo $_configs['video_upload_status'][$value];?></span>
		<?php if($_configs['video_upload_status']){ ?>
		<ul class="condition-area-list defer-hover-target">
			<?php foreach ($_configs['video_upload_status'] as $k => $v){ ?>
			<li>
		...
		<?php } ?>
		<input type="hidden" name="trans_status" value="<?php echo $value;?>" />
	</span>
</div><?php 
