<?php
			$this->curlNode = new curl('localhost', 'livsns/api/tuji/admin/', '');
			...
			$this->curlNode->addRequestData('trigger_mod_uniqueid', 'tuji');
			$hg_data = $this->curlNode->request('tuji_node.php');
			...
			if ($$s)
			{
				...
				$hg_selected_data = $this->curlNode->request('tuji_node.php');
				$this->tpl->addVar('hg_columns_selected', $hg_selected_data);
			}
			$hg_node_template = 'node';
			$extlink = '&amp;infrm=1';
			$hg_attr['nodeapi'] = 'fetch_node.php?nid=8&amp;node_en=tuji_node&amp;mid=' . $this->input['mid'] . $extlink;
