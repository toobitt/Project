<?php
		...
		$this->curl = new curl('localhost', 'livsns/api/outpush/admin/');
				...
				$this->curl->addRequestData('a', 'formdata');
				$this->curl->addRequestData('id', $id);$hg_data_return = $this->curl->request('outpush.php');
				$relate_menu = array();
					$nav = array(
						'name' => 'CRE表单',
						'link' => '#'
					...
					{
						$this->tpl->outTemplate('outpush_form', ",error");
					}
					else
					{
						$this->tpl->outTemplate('outpush_form', "");
					}
					?>