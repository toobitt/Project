<?php
			$api = array(
				'host' => 'localhost',
				'port' => '80',
				'dir' => 'livsns/api/program/admin/',
				);
			$this->tpl->addVar('__api', urlencode(json_encode($api)));
			$this->curl = new curl('localhost', 'livsns/api/program/admin/');
			...
			$_configs = $this->curl->request('program_library.php');
			$_configs = $_configs[0];
			$this->tpl->addVar('_configs', $_configs);
				...
				$total = $this->curl->request('program_library.php');
			...
			$op = array();$batch_op = array();
				...
				$datas = $this->curl->request('program_library.php');
...
');
				$this->tpl->setSoftVar('program'); //设置软件界面
				$this->tpl->addVar('list', $datas);
				$this->tpl->addVar('module_id', 609);
				$this->tpl->addVar('_cur_module_name', '节目库');
			...
			$this->tpl->outTemplate('program_library_list');
				?>
