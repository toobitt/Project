...
define('WITH_DB', true);
define('ROOT_DIR', './');
define('SCRIPT_NAME', 'mode');
require('./global.php');
require (ROOT_PATH . 'lib/class/curl.class.php');
class mode extends uiBaseFrm
{	
	private $modestore;
	...
	function __construct()
	{
		...
		{
			$this->ReportError('对不起，您没有权限进入样式商店');
		}
	}
	...
	{	
		...
		$mode_info = $curl->request('mode.php');
	...
	public function publish()
	{
		...
		$curl->setSubmitType('post');
		$curl->initPostData();
		$curl->addRequestData('a','export_modeinfo');
