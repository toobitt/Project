...
function InnovaEditor(oName)
  ...
  for(var i=0;i<oScripts.length;i++)
	...
	if(!InnovaEditor.new2012){
	  ...
	  this.iconHeight=25; //22;
		...
		this.iconPath="icons/2012/";
		this.iconWidth=27;
		this.iconHeight=22; //22;
	}
  ...
  */
  ...
  this.insert_pic=function(src)
  {
  	oUtil.obj.doCmd("InsertImage",src); 
  }
}
...
function RENDER(sPreloadHTML, dvId)
  ...
  if(!InnovaEditor.new2012){
	  sHTML+="<table id=idArea"+this.oName+" name=idArea"+this.oName+" border=0 "+
		"cellpadding=0 cellspacing=0 width='"+this.width+"' height='"+this.height+"' class=\"editortab\">"+
		"<tr><td colspan=2 style=\"position:relative;padding:0px;padding-left:0px;border:#cfcfcf 0px solid;border-bottom:0;background:url('"+this.scriptPath+"bg.gif')\">"+
	  ...
	  sHTML+="<input type=submit name=iwe_btnSubmit"+this.oName+" id=iwe_btnSubmit"+this.oName+" value=SUBMIT style='display:none' >";//hidden submit button
		...
		sHTML+="<table id=idArea"+this.oName+" name=idArea"+this.oName+" border='0px' "+
		 ...
		 sHTML+="<tr><td style='vertical-align:top;'>";
		 sHTML+="<div class=\"editor-iframe-box\">";

		if(this.IsSecurityRestricted)
		sHTML+="<iframe security='restricted' style=\"width:"+ (this.width - 2) +"px;height:"+ (this.height - 32) +"px;\" class=\"each-editor-iframe\" frameborder=\"0\" src='"+this.scriptPath+"blank.gif'"+
		  " name=idContent"+ this.oName + " id=idContent"+this.oName+
		  " contentEditable=true></iframe>";//prohibit running ActiveX controls
	  else
		sHTML+="<iframe style=\"width:"+ (this.width - 2) +"px;height:"+ (this.height - 32) +"px;\" class=\"each-editor-iframe\" frameborder=\"0\" src='"+this.scriptPath+"blank.gif'"+
		  " name=idContent"+ this.oName + " id=idContent"+this.oName+
		  " contentEditable=true></iframe>";
		
		sHTML+="<iframe style='width:1px;height:1px;overflow:auto;display:none;' src='"+this.scriptPath+"blank.gif'"+
		" name=idContentWord"+ this.oName +" id=idContentWord"+ this.oName+
		" contentEditable=true onfocus='"+this.oName+".hide()'></iframe>";

		sHTML+="<div id=idStyles"+this.oName+" style='display:none;padding:0px;background:#E9E8F2'></div>";
		sHTML+="</div>";
		sHTML+="</td></tr>";
		sHTML+="<tr id=idTagSelBottomRow"+this.oName+"><td colspan=2 id=idTagSelBottom"+this.oName+" style='padding:0px;'></td></tr>";
		sHTML+="</table>";
  ...
  if(confirm(getTxt("Are you sure you wish to delete all contents?"))==true)
    {
		...
		pagenum.value = intval(pagenum.value) - 1;
		alert(pagenum.value);
    }
  }
function applySpan()
...
function fullScreen()
  ...
  if(this.stateFullScreen)
	...
	if(InnovaEditor.new2012){
		var iframe = document.getElementById("idContent"+this.oName);
		iframe.style.width = (this.width - 2) + 'px';
		iframe.style.height = (this.height - 32) + 'px';
	}
	...
	if(InnovaEditor.new2012){
		...
		eval("idArea"+this.oName).style.zIndex=10000;
	}
 ...
 * @author kinch
 * @argument cur_page
 * @return show the selected page content
 */
function applypage(cur_page,ElementName)
{
	var oEditor = eval("idContent"+this.oName);
	var temcontent = document.getElementById(ElementName + cur_page);
	this.saveForUndo();
	oEditor.document.body.innerHTML = temcontent.value;
}

function is_content()
{
	var oEditor=eval("idContent"+this.oName);
	return oEditor.document.body.innerHTML ? true : false;
}
