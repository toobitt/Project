...
ISTbar.render=function() {
  ...
  if(!InnovaEditor.new2012){
	  ...
	  s[j++]="</td></tr></table></div>";
  	  ...
  	  s[j++]="<div id='"+this.id+"' class=\"all-editor-box\" style='"+(this.floating?"position:absolute;top:0px;left:0px;display:none":"")+"'>";
	  for(var it in this.btns) {
		s[j++]=this.btns[it].toHTML();
	  }
	  s[j++]="<br class=\"all-editor-br\"/>";
	  s[j++]="</div>";
  }
...
function ISButton(id, icon, text, width, height) {
  ...
  this.setState=function(s) {
	...
	if(!InnovaEditor.new2012){
		...
		if(this.id == 'btnFontName' + this.container.id || this.id == 'btnFontSize' + this.container.id){
			if(s == 3){
				s = 2;
			}
		}
		btn.style.top=-((this.height+1)*(s-1))+"px";
	}
  };
  ...
  this.toHTML=function() {
	...
	if(!InnovaEditor.new2012){
		...
		s[j++]="<div id=\""+id+"\" class=\"each-editor-btn "+ id.replace(tbId, '') +"\" style=\"clip:rect(0px "+this.width+"px "+this.height+"px 0px);\" onmouseover=\"$msOver(event, '"+tbId+"', '"+this.id+"')\" onmouseout=\"$msOut(event, '"+tbId+"', '"+this.id+"')\" onmousedown=\"$msDown(event, '"+tbId+"', '"+this.id+"')\" onmouseup=\"$msUp(event, '"+tbId+"', '"+this.id+"')\" >";
		s[j++]="<img src=\""+this.container.iconPath+this.icon+"\" style=\"position:absolute;top:0px;left:0px\" alt='"+this.text+"'/>"
		s[j++]="</div>";
	}
