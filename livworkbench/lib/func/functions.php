...
*
* $Id: functions.php 9169 2015-06-10 06:31:24Z repheal $
***************************************************************************/
...
function hg_encript_str($str, $en = true,$custom_appkey = CUSTOM_APPKEY)
{
	$salt = $custom_appkey;
	if ($en)
	{
...
function get_serv_file($server, $file)
{
	...
	$con = $socket->connect($ip, $port, $force_server);
	$socket->sendCmd($cmd);
...
class hgSocket
{
	...
	{
		if($this->connetced || $force)
		{
			@socket_close($this->socket);
		}
	}
	public function connect($ip, $port, $force_server = false)
	{
		if (!$force_server)
		{
			global $gGlobalConfig;
			if ($gGlobalConfig['localapp'])
			{
				$ip = $gGlobalConfig['mcphost'];
			}
		}
		//echo $ip . $port . '<br />';
...
* 加密通信
*/
function hoge_en($str, $salt = CUSTOM_APPKEY)
{
	$salt = md5(TIMENOW);
	$saltlen = strlen($salt);
	$str = base64_encode($str);
