...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("队列文件过多.\n" + (message === 0 ? "上传队列限制." : "你还可以上传 " + (message > 1 ? " " + message + " 文件." : "单个文件.")));
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			...
			this.debug("错误代码: 不能上传0字节文件, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			progress.setStatus("Invalid File Type.");
			this.debug("错误代码: Invalid File Type, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
		default:
			if (file !== null) {
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			...
			this.debug("错误代码: 上传受限, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
		case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
			progress.setStatus("验证失败，跳过.");
			this.debug("错误代码: 验证失败，跳过, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
...
function queueComplete(numFilesUploaded) {
	...
	status.innerHTML = numFilesUploaded + " file" + (numFilesUploaded === 1 ? "" : "s") + " 已上传.";*/
}
