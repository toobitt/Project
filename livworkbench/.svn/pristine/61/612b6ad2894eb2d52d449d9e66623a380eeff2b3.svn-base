(function($){
    ...
    var dataInfo = {
		...
		css : '' + 
			...
			'.search-button{border:0;cursor:pointer;width:40px;height:43px;float:right;margin-right:30px;border-left:1px solid #ccc;border-right:1px solid #ccc;background:url(' + RESOURCE_URL+'search.png) no-repeat center #fff;}' +
    ...
    $.widget('datasource.pubLib', $.datasource.pop, {
    	...
    	_getData : function( param ){
			...
			$('<img src=" ' + RESOURCE_URL + 'loading.gif" width="64" height="64" style="position:absolute;top:50%;left:50%;margin:-32px 0 0 -32px;"  />').appendTo( this.list_box );
    	...
    	_clickCall : function( ids ){
    		...
    		this._trigger( 'clickCall', event, [ info ] );
    	},
