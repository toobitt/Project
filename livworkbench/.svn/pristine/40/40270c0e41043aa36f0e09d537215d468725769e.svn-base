...
function I_InsertTable(nRow, nCol) {
    obj.setFocus();
    obj.saveForUndo();
    var sHTML = "<table style='border-collapse: collapse;min-width:300px'>";
...
function I_SetCss(oElement, cssRule, cssVal) {
    ...
    if (cssRule == 'fontSize') oElement.style.fontSize = cssVal;
...
function I_Size_saf(val) {
    if (!isiPad) obj.setFocus();
    obj.saveForUndo();
    I_FormatText_saf('font-size', val)
}
function I_FormatText_saf(cssRule, cssVal, bSys) {
    if (!isiPad) obj.setFocus();
    var oEditor = parent.oUtil.oEditor;
    var oSel;
...
function I_FormatText(cssRule, cssVal, bSys) {
    ...
    if (parent.oUtil.activeElement) {
        ...
        if (navigator.appName.indexOf('Microsoft') != -1) {
            ...
            (obj.myapplySpanStyle || obj.applySpanStyle).call(obj, [ [cssRule, cssVal] ]);
            oSel = oEditor.getSelection();
            oElement = parent.getSelectedElement(oSel);
            I_SetCss(oElement, cssRule, cssVal)
        }
