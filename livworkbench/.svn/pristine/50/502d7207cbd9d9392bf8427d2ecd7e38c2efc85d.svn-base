	...
	function News( el, options ){
		...
		this.template = {												//gmu的模版语法，$.parseTpl(tpl,data);来解析
			...
			list :  '<li data-id="<%= id%>" data-module="<%= module_id%>" class="list-item-li">' +
						'<div class="list-item <% if(module_id =="vod"){%>video<%}%>  m2o-flex m2o-flex-center">' +
							'<div class="info m2o-flex-one">' +
								'<p class="title"><%= title %><% if(module_id =="vod"){%><span class="flag">视频</span><%}%><% if(module_id =="tuji"){%><span class="flag">图集</span><%}%></p>' +
							'</div>' +
							'<span class="list-pic">' +
								'<img src="<%= src%>" /> ' +
							'</span>' +
			...
			news_detail : '<div class="content-box-word">' +
							...
							'<p></p>' +
							'<% if(+is_have_video){%>' +
								'<div><video src="<%= video_url%>" poster="<%= poster%>" style="width:100%;height:330px;margin:15px auto 0;padding:0;"/></div>' +
								'<div class="video-brief"><%= brief%></div>' +
							'<%}%>' +
							'<% if ( module_id == "news"){%>' +
								'<article>' +
								'</article>' +
		...
		detail : function( id ){										//取详情页
			...
			this.ajax( url, {id :id}, function(json){
				...
				if( data['is_have_video'] ){
					data['video_url'] = data['hostwork'] + '/' + data['video_path'] + data['video_filename'];
				...
				if( data['material'] ){
					...
					$.each( data['material'] , function( key ,value ){
						var src = _this.createImgsrc( value['pic'], {width : 400, height : 200});
						var picData = _this.createImgsrc( value['pic'], {width : wid, height : 1000});
						src_arr.push( src );
					...
					$.each( src_arr , function( key,value ){
						...
						img.on('load', function(){
							...
							if( number == src_arr.length ){
								_this.initDetailScroll();			//详情页如果有图片资源，要等到图片load完后在初始化详情页的scroll
							}
						});
					} );					
				}else{
				...
				content_box.on( 'tap','img',function(event){	//详情页大图查看
