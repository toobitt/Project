...
$.templete = { 
	...
	tab_tpl : '' + 
		'<div class="tabs" {{if style}}_channel_id="{{channel_id}}" _type="{{style}}"{{/if}}>' +
			'{{each column as value i}}' + 
	...
	live_tpl : '' + 
		'{{if type == "audio"}}' +
			'<div class="broadcast-play-wrap live-wrap">' +
			...
			'</div>' +
		'{{else}}' +
			'<div class="player-wrap live-wrap">' +
				'<video src="{{m3u8}}" poster="{{poster}}" autoplay="true" controls="true" style="width:100%; height:100%; "/>' +
			'</div>' +
		'{{/if}}' +
		'',
			...
			'<ul class="list detail-list">' +
				'{{each list as value ii}}' +
					'<li class="list-item m2o-flex m2o-flex-center{{if value.display}} live{{/if}}{{if value.zhi_play}} selected{{/if}}" _id={{ii + 1}}>' +
						'<div class="live-time">{{value.start}}</div>' +
						'<div class="state{{if value.zhi_play}} state_show{{/if}}">正在播放</div>' +
						'<div class="m2o-flex-one live-name m2o-overflow" _vedio_url="{{value.m3u8}}">{{value.theme}}</div>' +
