...
class Template
{
	...
	public function fetchCssFile($filename = 'head', $dir = 'head/')
	{
		...
		//file_put_contents("./temp.php", print_r($css, 1));
		//exit;
		return $css;
	}
	...
	private function parseCssCode($css)
	{
		...
		foreach ($css AS $v)
		{
			...
			{
				$csscode[$v] = 	'<link rel="stylesheet" type="text/css" href="<?php echo CSS_FILE_URL; ?>' . $v . '"/>';
			}
		}
		return implode("\n", $csscode);
	}
	...
	private function parseJsCsscode_normal($content, $type = 'js')
	{
		...
		foreach ($match[1] AS $v)
		{
			...
			if (!in_array($v, $jsscode))
			{
				if (is_file($jsfile))
				{
					if ($type == 'js')
					{
						$jsscode[$v] = 	'<script type="text/javascript" src="<?php echo SCRIPT_URL; ?>' . $v . '.js"></script>' . "\n";
					}
					else if ($type == 'css')
					{
						$jsscode[$v] = 	'<link rel="stylesheet" type="text/css" href="<?php echo CSS_FILE_URL; ?>' . $v . '.css" />' . "\n";
					}
				}
				else
				{
		...
		//file_put_contents("./temp.php", print_r($jsscode, 1));
		//exit;
			...
			if (!in_array($v, $jsscode))
			{
				if ($type == 'js')
				{
					if(in_array($v.'.js', $js_filter)){
						$js_filter_script[$v] = '<script type="text/javascript" src="<?php echo SCRIPT_URL; ?>' . $v . '.js"></script>' . "\n";
					}else{
						$jsscode[$v] = 	$v.'.js';
					}
				}
