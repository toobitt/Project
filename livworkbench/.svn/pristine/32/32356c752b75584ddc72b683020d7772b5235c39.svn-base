		...
		address_tpl : '' + 
			'{{if list}}' +
				'{{each list as value i}}' +
					'{{if label == "select"}}' +
					'<option value="{{value.kk}}" {{if selected == value.kk}}selected=""{{/if}}>{{value.vv}}</option>' +
					'{{/if}}' +
					'{{if label == "li"}}' +
					'<li _id="{{value.kk}}">{{value.vv}}</li>' +
					'{{/if}}' +
				'{{/each}}'	+
			'{{/if}}' +
			'',
		address_ul : '' + 
			'<ul class="Popaddress" id="{{className}}">' + 
				'{{if list}}' +
					'{{each list as value i}}' +
						'<li _id="{{value.kk}}">{{value.vv}}</li>' +
					'{{/each}}'	+
			...
			'</ul>' +
			'',
		default_tpl : '<option value="">请选择</option>',
		css : '' + 
			'.Popaddress{position:absolute; min-width:108px; max-height:360px; overflow:auto; border:1px solid #ccc; background-color:#fff; display:none; z-index:1002; }' + 
			'.Popaddress li{line-height:24px; padding:0 5px; border-bottom:1px dotted #ccc; cursor:pointer; }' +
			'.Popaddress li:last-child{border-bottom:none; }' +
			'.Popaddress li.selected{color:#23c880}' +
			'.Popaddress.show{display:block; }' +
			'',
		cssInited : false
	}
	
	function Address(element, options, e){
		if ( e ) {
            e.stopPropagation();
            e.preventDefault();
        }
        this.$element = $(element);
        var options = this.options = $.extend({}, $.fn.address.defaults, this.$element.data(), typeof options == 'object' && options);
        
        if( this.options.pop ){
        	this.options.label = 'ul';
        	this.initCss();
        }
        
        this.$province = $(element).find( options.province );
        this.$city = $(element).find( options.city );
        this.$region = $(element).find( options.region );
        this.init();
	}
	Address.prototype = {
		constructor: Address,
		init : function(){
			...
			$(document).on('click', function( event ){
				var target = widget.find('.Popaddress.show'),
					t_id  = target.attr('id');
				if( t_id ){
					if( $(event.target).is('span') ){
						var s_id = $(event.target).attr('id');
						if( s_id.indexOf( t_id.substring(4) ) > -1 ){
							return false;
						}
					}
					widget.find('.Popaddress.show').removeClass('show');
				}
			})
		},
		
		initCss : function(){
			if( !addressInfo.cssInited && addressInfo.css ){
        		addressInfo.cssInited = true;
        		 $('<style/>').attr('style', 'text/css').appendTo('head').html( addressInfo.css );
        	}
		},
		...
		initSelect : function(){
			var province = this.$province,
				...
				region = this.$region,
				r_val = region.val();
			var len = province[0].length;
			if( len < 3 ){
				this.changeAction("1", this.$province);
			}
			if( p_val && c_val){
				this.changeAction(p_val, this.$city);
				if( r_val ){
					this.changeAction(c_val, this.$region);
				}
			}
		},
		...
		bindEvent : function(){
			...
			var op = this.options,
				widget = this.$element;
			if( op.label == 'select' ){
				widget.on('change', 'select', function( e ){
					var id = this.id,
						label = id.substring(2),
						value = this.value;
					switch( label ){
						case 'province' : {
							_this.reset( _this.$city );
							_this.reset( _this.$region );
							_this.changeAction(value, _this.$city);
							break;
						} 
						case 'city' : {
							_this.reset( _this.$region );
							_this.changeAction(value, _this.$region);
							break;
						} 
						default : {
							break;
			...
			if( op.pop ){
				var domTag = [this.$province, this.$city, this.$region];
				domTag.forEach(function( dom, i ){
					dom.on('focus', function( event ){
						var id = this.id,
							label = id.substring(2);
						var val = dom.attr('_val');
						var offset = dom.offset(),
							hh = dom.height();
						
						_this.defer(150, function(){
							var popDom = widget.find( '#Pop_' + label );
							popDom.addClass('show').css({
								top : (offset.top + hh + 10) + 'px',
								left : offset.left
							});
						});
					});
				...
				widget.on('click', 'li', function( e ){
					var self = $(e.currentTarget),
						li_id = self.attr('_id'),
						li_val = self.html();
					var father = self.closest('ul[id^="Pop_"]'),
						id = father[0].id,
						label = id.substring(4);
					widget.find('#J_' + label).attr('_val', li_id).html( li_val );
					self.addClass('selected').siblings().removeClass('selected');
					father.removeClass('show');
					switch( label ){
						case 'province' : {
							_this.reset( widget.find('#Pop_city'), 'pop', _this.$city );
							_this.reset( widget.find('#Pop_region'), 'pop', _this.$region );
							_this.popAction(li_id, widget, 'Pop_city');
							break;
						} 
						case 'city' : {
							_this.reset( widget.find('#Pop_region'), 'pop', _this.$region );
							_this.popAction(li_id, widget, 'Pop_region');
							break;
						} 
						default : {
							break;
		...
		changeAction : function(value, father){
			var address = [];
			var selected = father.val();
			this.find(value, function( i, item ){
				address.push({
					kk : i,
					vv : item[0]
				});
			});
			var render = template.compile( addressInfo.address_tpl );
			var html = render({list : address, label : this.options.label, selected : selected});
			father.empty().append( addressInfo.default_tpl );
		...
		initPop : function(){
			var widget = this.$element;
			var province = this.$province,
				p_val = province.attr('_val'),
				city = this.$city,
				c_val = city.attr('_val'),
				region = this.$region,
				r_val = region.attr('_val');
			this.popAction("1", this.$element, 'Pop_province');
			if( p_val && province.html() ){
				widget.find('#Pop_province').find('li[_id="' + p_val + '"]').addClass('selected');
			}
			if( c_val && city.html() ){
				this.popAction( p_val, this.$element, 'Pop_city' );
				widget.find('#Pop_city').find('li[_id="' + c_val + '"]').addClass('selected');
			}
			if( r_val && city.html() ){
				this.popAction( c_val, this.$element, 'Pop_region' );
				widget.find('#Pop_region').find('li[_id="' + r_val + '"]').addClass('selected');
			}
		},
		
		popAction : function(value, father, className){
			var address = [];
			this.find(value, function( i, item ){
				address.push({
					kk : i,
			...
			var render = template.compile( addressInfo.address_ul );
			var html = render({list : address, label : 'ul', className : className});
			$( html ).appendTo( father );
		},
		...
		reset : function( elem, type, obj ){
			if( type == 'pop' ){
				obj.attr('_val', '').html('请选择');
				elem.remove();
				return;
			}
			if( elem[0].length > 2 ){
				elem.empty().append( addressInfo.default_tpl );
			}
		},
	}
	$.fn.address = function(option, event) {
       return this.each(function () {
	      var $this = $(this)
	        , data = $this.data('address')
	        , options = $.extend({}, typeof option == 'object' && option);
	      if (!data) $this.data('address', (data = new Address(this, options, event)));
   	 	})
    };
...
/*
 * 说明
 * <select id="J_province">			//options.label: 'select',id: 'J_province', 'J_city', 'J_region'
