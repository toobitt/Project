	...
	$.extend(Toast.prototype , {
		show : function( msg ){
			...
			if( !self.$toast || !self.$toast.length ){
				self.$toast = $('<div class="popBox fadeOut" id="' + self.op.target.substr(1) + '"><span class="inner"></span></div>').appendTo( self.op.appendTo );
			}
		...
		cssInit : function(){
			...
			$('<style />').attr('style', 'text/css').appendTo('head').html( 
				'.popBox{position:absolute; top:30%;left:0; height : 46px; width : 100%; z-index: 1001; text-align : center; transition:opacity 0.3s; }' +
