(function ($) {
	...
	var publis_li_tpl = 
		'<li _id="${id}" _fid="${$item.fid}" title="{{if $item.showName}}${$item.showName} > {{/if}}${name}" _name="${name}" _showName="{{if $item.showName}}${$item.showName} > {{/if}}${name}" class="one-column {{if +is_last}}no-child{{/if}}">'+
			'<input type="checkbox" class="publish-checkbox" {{if $data.is_auth == 2}}style="visibility: hidden;"{{/if}}  {{if can_select}}style="visibility: hidden;"{{/if}} /> '+
    ...
    var defaultOptions = {
    	...
    	getUrl: function(id) {
    		id = id || this.model.id;
    		return 'run.php?mid=' + gMid + '&a=show&fid=' + id;
    	}
    };
	...
	$.extend(Publish.prototype, {
		...
		getUrl: function(id1, id2, offset) {
			return this.options.php + '?a=get_node&mid=' + gMid + '&siteid=' + id1 + '&fid=' + id2 + (offset  ? 
