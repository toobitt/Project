...
$(function(){
    $('.add-black-item').click(function(){
        var target = $(this);
        var url = 'run.php?mid='+ gMid +'&a=black_device';
        var identifier = target.attr('_identifier');
        var member_id = target.attr('_member_id');
        var device_token = target.attr('_device_token');
        var black_device = target.attr('_black_device');
        var type = target.attr('_type');
        var deadline = target.attr('_deadline');

        $.getJSON(url, {identifier:identifier,member_id:member_id,device_token:device_token,black_device:black_device,deadline:deadline,type:type}, function( json ){
            var obj = json[0];
                   ...
                   <ul class="common-list">
                        <li class="common-list-head public-list-head clear">
                            ...
                            <div class="common-list-biaoti">
                                ...
                                <div class="common-list-item wd100">设备号</div>
					        </div>
                        </li>
                    </ul>
	                <ul class="common-list hg_sortable_list public-list" id="auth_form_list">
					    {if $list}
		       			    {foreach  $list  as $k => $v}
		                      {template:unit/device_blacklist_list_item}
		                    {/foreach}
		                {else}
