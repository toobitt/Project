...
{
	window.location = "./run.php?mid={$_INPUT['mid']}&a="+a+"&infrm=1&id="+id+"&site_id={$list['site_id']}&page_id={$list['page_id']}&page_data_id={$list['page_data_id']}&expand_name={$list['expand_name']}" ;
}
</script>
<!-- 
...
<li class="common-list-data clear"  id="r_{$v['rid']}" _id="{$v['id']}" name="{$v['rid']}"   orderid="{$v['order_id']}">
	...
	<div class="common-list-item opration">
		...
		<div class="common-list-item last-update">
			<div class="common-list-cell">
				{code}echo date('Y-m-d H:i:s',$v['last_update_time']);{/code}
			</div>
		</div>
	</div>
