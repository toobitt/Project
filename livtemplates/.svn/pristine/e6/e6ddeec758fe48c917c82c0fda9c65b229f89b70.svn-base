...
<li class="clear"  id="r_{$v['id']}"    name="{$v['id']}"   orderid="{$v['id']}"  onmouseout="hg_row_interactive(this, 'out');" onmouseover="hg_row_interactive(this, 'on');">
	<span class="left">
		<input type="checkbox" style='margin-left: 15px; float: left'  name="infolist[]" value="{$v['id']}" title="{$v['id']}" >
		<a class="shareslt"  >&nbsp;{$v['name']}</a>
		<a class="shareslt"  href="./run.php?mid={$_INPUT['mid']}&a=lang_detail&id={$v['id']}">编辑</a>
	</span>
</li>   
-->
<li class="common-list-data clear" id="r_{$v['id']}" name="{$v['id']}">
	<div class="common-list-left">
		...
		<div class="common-list-item option">
			<div class="common-list-cell">
				<a class="shareslt"  href="./run.php?mid={$_INPUT['mid']}&a=lang_detail&id={$v['id']}&infrm=1&nav=1">编辑</a>
			</div>
		</div>
	</div>
</li>