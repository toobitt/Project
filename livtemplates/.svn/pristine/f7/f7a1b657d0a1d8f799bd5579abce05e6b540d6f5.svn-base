<li class="clear"  id="r_{$v['id']}"    name="{$v['id']}"   orderid="{$v['order_id']}"  onmouseout="hg_row_interactive(this, 'out');" onmouseover="hg_row_interactive(this, 'on');">
	<span class="left">
		...
		<a class="fb overflow"  style="width:120px;"  id="contribute_sort_name_{$v['id']}"><span class="m2o-common-title">{$v['config_name']}</span></a>
	</span>
	<span class="right" onclick="hg_row_interactive('#r_{$v[id]}', 'click', 'cur');"  style="width:400px;">
		...
		<a class="tjr" style="width:150px;"><em>{$v['user_name']}</em><span>{$v['create_time']}<a class="fb"  style="width:145px;">{$v['ip']}</a></span></a>
	</span>
	<span class="title overflow"  style="cursor:pointer;"><a id="contribute_sort_desc_{$v['id']}">{if $v['type']}图片水印{else}文字水印{/if}</a></span>
</li>