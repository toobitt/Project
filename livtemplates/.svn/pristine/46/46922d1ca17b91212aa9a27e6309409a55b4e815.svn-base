jQuery(function($){
    var url = './run.php?mid='+ gMid +'&a=resume_form&infrm=1&id=';
    var ajax = 'fetch_column_node.php?siteid={{siteid}}&fid={{fid}}&ban=1';
    var createUrl = 'fetch_column_node.php?a=create';
    var sortUrl = './run.php?mid='+ gMid +'&a=column_sort';
    var deleteUrl = './run.php?mid='+ gMid +'&a=delete&id=';
                            ...
                            $.getJSON(replaceTpl(ajax, {
                                ...
                                fid : id
                                    ...
                                    if(json){
                                        ...
                                        next.find('.column-ul').html(lis).find('.column-edit-button[data-auth="2"]').hide();
                                        $('.column-box').triggerHandler('size.column');
                                    }
                                });
                        }
    ...
    box.on('click', '.column-submit', function(){
        ...
        setTimeout(function(){
            $.post(
                createUrl,
                {column_name : val, siteid : $('#default-site').val(), fid : fid},
                function(data){
