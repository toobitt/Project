...
{/code}
<div class="set-area-title" data-max="{$transcode_server[0]['max_size']}">设置</div>
<ul class="set-area-nav">
	<li class="server_id">转码服务器(<span class="select-item">空闲</span>)</li>
	<li class="water_id">水印(<span class="select-item">{if $default_water}{$default_water_name}{else}无{/if}</span>)</li>
...
<div class="set-item water-area clear" data-name="water_id">
		<div class="watermark">
			<div class="title">水印列表</div>
			<ul>
				{foreach $water_pic[0]  as $k => $v}
		...
		<div class="water-position set-item" data-name="water_pos">
	...
	<div class="set-item mosaic clear" data-name="mosaic_id" >
			<ul>
				{foreach $mosaic[0]  as $k => $v}
	...
	<div class="set-item vod_config mosaic clear" data-name="vod_config_id" >
			<ul>
		...
		<input type="hidden" class="fast-set-hidden"  name="no_water" value="{if !$default_water}1{/if}"/>
