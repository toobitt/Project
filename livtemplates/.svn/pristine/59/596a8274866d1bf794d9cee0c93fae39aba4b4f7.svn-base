jQuery(function($){
    var url = './run.php?mid='+ gMid +'&a=form_out_variable&infrm=1&id=';
    var ajax = '_fetch_node.php?multi=mode&fid={{fid}}&ban=1';
    var createUrl = './run.php?mid='+ gMid +'&a=create_out_variable';
    var sortUrl = './run.php?mid='+ gMid +'&a=sort_out_variable';
    var deleteUrl = './run.php?mid='+ gMid +'&a=delete_out_variable&id=';
    ...
    $(window).on('resize', function(){
        var max = Math.floor( parseInt($('.m2o-f').width(), 10) / constWidth );
    ...
    box.on('click', '.column-add-box', function(){
        ...
        if(len){
            if(!each.prev().find('.column-li-selected').length){
                errorTip(each.prev(), '请在这一列中选择父级模板分类...', [5, -5, 3, -3, 1, -1]);
                return;
            }
        }
            ...
            $.post(
                ...
                function(data){
                    ...
                    if(data && parseInt(data, 10) > 0){
                        ...
                        var el = $('#out_variable_ids');
                        el.val( 
                        	el.val() + (el.val() ? ',' + data : data) 
                        );
                        input.remove();
                        $('.column-box').triggerHandler('size.column');
    ...
    box.on('click', '.column-sort-button', function(){
        ...
        if(length < 2){
            var mi = $(this);
            errorTip(mi, !length ? '这一列没有模板分类' : '只有一个模板分类无法排序');
            return;
        }
