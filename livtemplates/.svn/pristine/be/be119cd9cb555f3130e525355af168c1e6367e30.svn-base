...
<div id="hg_page_menu" class="head_op_program" {if $_INPUT['infrm']}style="display:none"{/if}>
   <a class="blue mr10" href="./run.php?mid={$_INPUT['mid']}&a=form{$_ext_link}" target="nodeFrame">
		...
		<span class="middle"><em class="add">新增小店</em></span>
								        ...
								        <div class="common-list-cell">
											 {code}
												...
												if($v['pic'][0])
												{
													...
													{
														$log_img = $v['pic'][0]['host'] . $v['pic'][0]['dir'] . $v['picsize'][0]['thumbnail'] . $v['pic'][0]['filepath'] . $v['pic'][0]['filename'];									
													}					
												}
											 {/code}	
								        	 ...
								        	 <span id="title_{$v['id']}"  class="common-list-overflow" style="max-width:350px;{if $v['address']}display:block;{/if}"><a title="编辑" href="./run.php?mid={$_INPUT['mid']}&a=form&id={$v['id']}&infrm=1"><span class="m2o-common-title">{$v['address']}</span></a></span>
								        	 </span>						      
								        </div> 
							        </div>         
							    </div>	
