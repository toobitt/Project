...
{if !$val['new']}
	<li class="day_default" id="li_{$val['id']}" onmouseover="hg_move_index({$val['id']},true);" onmouseout="hg_move_index({$val['id']},false);">
		<div class="single_item" id="update_item_{$val['id']}" style="{code} echo $bcolor?'background:'.$bcolor.';':'';{/code}{if $val['is_plan']}border-left-color: {$pcolor};border-left-width: 2px;{/if}">
			...
			<span class="lb-btn{if $val['item']} cur{/if}" id="lb-btn_{$val['id']}" style="display:{if $val['item']}block{else}{if $val['outdate']}none{else}block{/if} {/if};" onclick="hg_request_source({$val['id']});"></span>
			{if $val['item']}
			{code}
				...
				$item_source = array(
					...
					'extra_li'=>'onmouseover="hg_stop_source(1);"  onmouseout="hg_stop_source(0);"',
					'extra_over'=>'hg_stop_source(1);',
					'extra_out'=>'hg_stop_source(0);',
					'extra_div' => 'hg_show_item(' . $val['id'] . ')',
					'more' => $val['id'],
				);
				$program_item[-1] = '录播分类';
			...
			{/code}
			<span class="down_list" style="display:{if $val['item']}block{else}none{/if};" id="lb_column_{$val['id']}">{template:form/search_source,item,$default,$program_item,$item_source}
			{else}
