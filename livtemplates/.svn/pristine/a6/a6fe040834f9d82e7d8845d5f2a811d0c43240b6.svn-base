jQuery(function($){
    ...
    var ajax = '_fetch_node.php?multi={{setNode}}&fid={{fid}}&ban=1';
    var createUrl = './run.php?mid='+ gMid +'&a=create';
    ...
    box.on('click', '.column-add-box', function(){
        ...
        if(len){
            if(!each.prev().find('.column-li-selected').length){
                errorTip(each.prev(), '请在这一列中选择父级相册分类...', [5, -5, 3, -3, 1, -1]);
                return;
            }
        }
    ...
    box.on('click', '.column-sort-button', function(){
        ...
        if(length < 2){
            var mi = $(this);
            errorTip(mi, !length ? '这一列没有相册分类' : '只有一个相册分类无法排序');
            return;
        }
    ...
    box.on('click', '.column-delete-button', function(){
        var deleteId = $(this).closest( 'li' ).attr( '_id' );
