...
<div style="display:none">
	{template:unit/identifierUserSystem_search}
	<div class="controll-area fr mt5" id="hg_page_menu" style="display:none">
		<a href="?mid={$_INPUT['mid']}&a=form{$_ext_link}" class="button_6">新增用户系统</a>
	</div>
</div>
<!-- 记录列表 -->
<div class="common-list-content" style="min-height:auto;min-width:auto;">
	<form action="" method="post">
	 <div class="m2o-list">
            ...
            <div class="m2o-item m2o-flex-one m2o-bt" title="名称">系统名称</div>
            <div class="m2o-item m2o-mark" title="标识">系统标识</div>
            <div class="m2o-item m2o-brief" title="描述备注">描述</div>
            <div class="m2o-item m2o-switch" title="是否启用此系统">启用</div>
            <div class="m2o-item m2o-time" title="添加人/时间">添加人/修改时间</div>
        </div>
        <div class="m2o-each-list">
        	{if $list}
	        {foreach $list as $k => $v}
	        	{template:unit/identifierUserSystemList}
	        {/foreach}
	        {else}
...
$(function(){
	var onOff = function(id, obj, is_on){
		...
		$.getJSON( url, {id : id, is_on : is_on,ajax:1} ,function( data ){
			if( data['callback'] ){
				eval( data['callback'] );
				return;
			}
