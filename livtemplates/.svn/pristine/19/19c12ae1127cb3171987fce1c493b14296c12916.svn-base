	...
	(function($){
		$.widget('tv.tv_list',{
			...
			_rePlay : function( event ){
				...
				var self = $(event.currentTarget),
					item = self.closest( op['tv-each'] ),
					astroid = item.attr('_astroid');
				id = item.attr('_id');
				fun = item.attr('_fun');
				this._re( id,astroid,fun,item );
				event.stopPropagation();
			},
			...
			_re : function( id,astroid,fun,item ){
				var method = function(){
					var url = './run.php?mid=' + gMid + '&a=update&fun='+ fun +'&id='+id;
					$.get( url, {astroid : astroid } ,function(){
						window.location.reload();
					});
				};
				this._remind( '是否要更新此内容?', '更新提醒' , method );
			},
			_del : function( id,fun,item ){
				var method = function(){
					var url = './run.php?mid=' + gMid + '&a=delete&fun='+fun;
