...
function hg_download_material(url)
{
	if(url)
	{
		url = eval(url);
...
<div class="info clear vider_s"  id="vodplayer_{$formdata['data']['id']}">
  <div id="contribute_pics_show" class="tuji_pics_show">
  	 ...
  	 <object id="vodPlayer" type="application/x-shockwave-flash" data="{code}echo RESOURCE_URL{/code}swf/vodPlayer.swf?11122713" width="400" height="330">
		...
		<param name="flashvars" value="startTime={$formdata['data']['start']}&duration={$formdata['data']['duration']}&videoUrl={$formdata['data']['video_url'][0]['m3u8']}&videoId={$formdata['data']['video_url'][0]['vodid']}&snap=false&aspect={$formdata['data']['aspect']}&autoPlay=false&snapUrl={$formdata['data']['snapUrl']}">
	  </object>
...
<div class="info clear cz"  >
	<ul id="video_opration" class="clear" style="border:0;">
		...
		<li><a class="button_6 recommend" _href="./run.php?mid={$_INPUT['mid']}&a=recommend&id={$formdata['data']['id']}" data-id="{$formdata['data']['id']}" _onclick="return hg_ajax_post(this, '推荐', 0);">发布至网站</a></li>
		...
		{/if}
		{if $formdata['data']['pic']}
		<li>
			<a class="button_6" onclick="return hg_ajax_post(this, '打包下载图片', 1, function( data ){ if(data){window.location.href=data[0]}});"  title=""  href="./run.php?mid={$_INPUT['mid']}&a=download&id={$formdata['data']['id']}">打包下载图片</a>
		</li>
		{/if}
	</ul>
	<ul>
...
<div class="info clear vo">
	...
	<ul id="con_userinfo" class="clear">	    
		...
		{/if}
		<li class="h"><span>设备标识：{$formdata['data']['device_token']}</span></li>
	</ul>
</div>
{if intval($formdata['data']['baidu_latitude']) || intval($formdata['data']['baidu_longitude'])}
...
{code}
	$hg_bmap = array(
		'height' => 350,
		'width'  => 380,
		'longitude' => isset($formdata['data']['baidu_longitude']) ? $formdata['data']['baidu_longitude'] : '0', 
		'latitude'  => isset($formdata['data']['baidu_latitude']) ? $formdata['data']['baidu_latitude'] : '0',
		'zoomsize'  => 13,
