jQuery(function($){
    var url = './run.php?mid='+ gMid +'&a=form&infrm=1&trigger_action=manage&trigger_mod_uniqueid=contribute_node&id=';
    var ajax = '_fetch_node.php?multi=contribute_node&fid={{fid}}&ban=1&trigger_action=show&trigger_mod_uniqueid=contribute_node';
    var createUrl = './run.php?mid='+ gMid +'&a=create&trigger_action=manage&trigger_mod_uniqueid=contribute_node';
    var sortUrl = './run.php?mid='+ gMid +'&a=sort&trigger_action=manage&trigger_mod_uniqueid=contribute_node';
    var deleteUrl = './run.php?mid='+ gMid +'&a=delete&ajax=1&trigger_action=manage&trigger_mod_uniqueid=contribute_node&id=';
    ...
    box.on('click', '.column-add-box', function(){
        ...
        if(len){
            if(!each.prev().find('.column-li-selected').length){
                errorTip(each.prev(), '请在这一列中选择父级栏目...', [5, -5, 3, -3, 1, -1]);
                return;
            }
        }
    ...
    box.on('click', '.column-sort-button', function(){
        ...
        if(length < 2){
            var mi = $(this);
            errorTip(mi, !length ? '这一列没有栏目' : '只有一个栏目无法排序');
            return;
        }
    ...
    $('.column-delete-button').on('click', function(){
        ...
        jConfirm('确定删除？', '删除提示', function(result){
            if(result){
                $.post(deleteUrl + deleteId, function( data ){
                	if( data['callback'] ){
                		eval( data['callback'] );
                		return;
                	}
