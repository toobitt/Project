...
$(function($) {
    ...
    function getIds() {
        return el.find(".m2o-each").map(function() {
            return $(this).data("id") || this.id.slice(2);
        }).get();
    }
    ...
    function openDragView() {
        ...
        el.find(".m2o-each").find("a[name='alist[]']").addClass('pic_logo').find("input").css('visibility', 'hidden');
        el.addClass("gDragMode").sortable("option", "disabled", false);
    }
    function closeDragView(msg) {
        el.find(".m2o-each").find("a[name='alist[]']").removeClass('pic_logo').find("input").css('visibility', 'visible');
    ...
    $.destructState = function(){destructState()}
    $.closeDragView = function(msg){closeDragView(msg)};
