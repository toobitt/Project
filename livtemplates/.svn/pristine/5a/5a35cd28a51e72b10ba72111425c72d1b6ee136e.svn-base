...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData) {
			...
			if(serverData.ErrorCode == 'AD_TOKEN_ERROR')
			{
				alert('表单TOKEN错误');
				return;
			}
			url2preview(serverData.murl,serverData.mtype);
			$('#material_url').val(serverData.mhidden);
...
FileProgress.prototype.setProgress = function (percentage) {
	/*
	this.fileProgressElement.className = "adv_progressContainer";
