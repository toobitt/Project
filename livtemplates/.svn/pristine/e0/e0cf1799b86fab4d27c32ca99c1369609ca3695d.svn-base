...
<div id="hg_page_menu" class="head_op_program" {if $_INPUT['infrm']}style="display:none"{/if}>
	<a href="./run.php?mid={$_INPUT['mid']}&a=form{$_ext_link}" class="button_6" style="font-weight:bold;">添加标签</a>
</div>
          ...
          <div class="right v_list_show">
			...
			<div class="list_first clear"  id="list_head">
                    ...
                    <span class="right" style="width:350px;">   
                    	 <a class="fl" style="width:100px;">操作</a>              
						 <a class="fl" style="width:100px;">系统分类名</a>
						 <a class="fl" style="width:120px;">创建时间</a>
                   </span>
                   <a class="title" style="margin-left:53px;margin-top:8px;">标签</a>  
           </div>

           <form method="post" action="" name="listform">
               <ul class="list" id="vodlist">
					 {if is_array($list['data']) && !empty($list['data']) && count($list['data'])>0}
						{foreach $list['data'] as $k => $v}		
		                     {template:unit/marklist}
		                {/foreach}
					{else}
