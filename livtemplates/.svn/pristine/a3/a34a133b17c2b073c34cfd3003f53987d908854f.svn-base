<?php 
/* $Id: member_list.php 32137 2014-05-26 09:08:24Z youzhenghuan $ */
?>
{template:head}
{css:2013/list}
...
<div style="display:none">
    {template:unit/member_extensionsort_search}
	<div class="controll-area fr mt5" id="hg_page_menu" style="display:none">
		<a class="blue mr10" href="?mid={$_INPUT['mid']}&a=form{$_ext_link}" target="nodeFrame">
			...
			<span class="middle"><em class="add">新增会员扩展分类</em></span>
...
<div id="add_question"  class="single_upload">
	...
	<form action="" method="post">
	 <div class="m2o-list">
            ...
            <div class="m2o-item m2o-flex-one m2o-bt" title="头像/会员名">分类名称</div>
            <div class="m2o-item m2o-state w120" title="总积分">字段</div>
        </div>
        <div class="m2o-each-list">
        	{if is_array($list) && count($list)>0}
				{foreach $list as $k => $v}	
		           <div class="m2o-each m2o-flex m2o-flex-center m2o-list-data" data-id="{$v['extension_sort_id']}" orderid="{$v['order_id']}">
					    <div class="m2o-item m2o-paixu">
							<input type="checkbox" value="{$v['extension_sort_id']}" title="{$v['extension_sort_id']}" name="infolist[]" class="m2o-check" style="visibility: visible;">
						</div>
						<div class="m2o-item m2o-flex-one m2o-bt" title="{$v['name']}">
							<a class="common-title" href = "./run.php?mid={$_INPUT['mid']}&a=form&id={$v['extension_sort_id']}&infrm=1"  target="nodeFrame"  need-back>
								<span class="m2o-item-bt">
									<span>{$v['extension_sort_name']}</span>
								</span>
							</a>
						</div>
						<div class="m2o-item m2o-style w120 color">{$v['extension_sort']}</div>
    ...
    $.extend($.geach || ($.geach = {}), {
        data : function(id , status){
            ...
            $.each(data, function(i, n){
               if(n['extension_sort_id'] == id){
                   info = {
                       id : n['extension_sort_id'],
                       status : n['status']
                   }
                   return false;
               }
