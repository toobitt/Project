...
{css:edit_video_list}
{code}
//print_r($memmodule[0]['server_count']);
{/code}
...
<div id="hg_page_menu" class="head_op"{if $_INPUT['infrm']} style="display:none"{/if}>
			<a class="add-button mr10" href="run.php?mid={$_INPUT['mid']}&a=memcache_form&infrm={$_INPUT['infrm']}">新增服务器</a>
</div>
          ...
          <div class="right v_list_show">
                ...
                <form method="post" action="" name="listform">
	                <ul class="list" id="tujilist">
					    {if $memmodule[0]['module']}
		       			    {foreach $memmodule[0]['module'] as $k => $v} 
		                      {template:unit/memmodule_list}
		                    {/foreach}
		  				{/if}
