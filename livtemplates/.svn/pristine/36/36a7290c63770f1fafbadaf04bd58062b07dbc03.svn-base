<li order_id="{$v['order_id']}" _id="{$v[$primary_key]}" class="common-list-data clear"  id="r_{$v[$primary_key]}" name="{$v['order_id']}" >
    ...
    <div class="common-list-right">
        <div class="common-list-item wd70 overflow news-fenlei open-close" style="min-width: 70px;">
            <span>{$v['name']}</span>
        </div>
        <div class="common-list-item wd100 overflow news-fenlei open-close" style="width:300px;">
            <span>{$v['start_date_show']} 至 {$v['end_date_show']}</span>
        </div>
        <div class="common-list-item wd70 overflow news-fenlei open-close">
            <span>{$v['selled_num']}</span>
        </div>
