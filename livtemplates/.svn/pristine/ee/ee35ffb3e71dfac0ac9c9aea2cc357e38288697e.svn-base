$(document).ready(function(){	
	...
	endUploads = function(json){
		$("#sta_logo").attr('style','display:block;padding-left: 50%;');
		var obj = new Function("return" + json)();
		if(!obj)
		{
			tipsport('尺寸大于235*235');
			logo_exit();
		}
		else
		{
	...
	uploads = function (){
		$("#sta_logo").attr('src',RESOURCE_DIR+'img/loading.gif');
		$("#sta_logo").attr('style','display:block;padding-left: 50%;');
		$("#form1").submit();
...
//	        		$("#get_logo img").attr('style','');
	...
	stationEdit = function(){
		...
		$("#tags").val(get_tags);
		$("#brief").val(get_brief);
		$("#set_station").attr('style','display:inline');
		$("#get_station").attr('style','display:none');	
	}
	
	logoEdit = function(){
		var get_logo = $("#get_logo img").attr('src');
		$("#sta_logo").attr('style','display:inline');
		$("#sta_logo").attr('src',get_logo);	
		$("#set_logo").attr('style','display:inline');
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					$("#s_time").html(toff_time(parseInt(starttime),parseInt(total)));
				}
				$.ajax({
					url:"my_program.php",
					type:"POST",
				...
				$.ajax({
			        ...
			        success: function(json) {
			        	...
			        	$.each(obj,function(k,v){
							...
							'<div class="program_name"><a target="_blank" title="'+v.programe_name+'" id="program_name_'+v.id+'" href="station_play.php?sta_id='+v.sta_id+'&user_id='+v.user_id+'">'+v.programe_name.substr(0,10)+'</a></div>'+
							'<div class="program_manage" id="p_'+v.id+'">'+
							'<a href="javascript:void(0);" onclick="program('+v.sta_id+','+v.id+',1);">编辑</a>'+
							' <a href="javascript:void(0);" onclick="program('+v.sta_id+','+v.id+',2);">删除</a>';
							if(k!=0)
							{
								li += ' <a href="javascript:void(0);" onclick="program('+v.sta_id+','+v.id+',3);">上移</a>';
							}
							if(k!=(obj.length-1))
							{
								li += ' <a href="javascript:void(0);" onclick="program('+v.sta_id+','+v.id+',4);">下移</a>';
							}
