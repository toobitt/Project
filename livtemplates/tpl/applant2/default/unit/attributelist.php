        ...
        <div class="common-list-right">
                ...
                <div class="group-cz2 common-list-item open-close">
                       <div class="common-list-cell">
                              <span>
                              {if $v['type'] == 'range'}
                                  {if isset($v['def_val']['def_value'])}
                                      value : {$v['def_val']['def_value']}<br />
                                  {/if}
                                  {code}
                                  $str = 'range : ';
                                  if (!isset($v['def_val']['from']))
                                  {
                                      $str .= '( ∞ , ';
                                  }
                                  else
                                  {
                                      $str .= (!$v['def_val']['from']['include']) ? '( ' : '[ ';
                                      $str .= $v['def_val']['from']['value'].' , ';
                                  }
                                  if (!isset($v['def_val']['to']))
                                  {
                                      $str .= '∞ )';
                                  }
                                  else
                                  {
                                      $str .= $v['def_val']['to']['value'];
                                      $str .= (!$v['def_val']['to']['include']) ? ' )' : ' ]';
                                  }
                                  echo $str;
                                  {/code}
                              {else}
