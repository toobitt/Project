$(function(){
	...
	var tpl = {
			modal : '' +
			'<div class="modal fade modal-pop in" id="upload-video" style="width:675px;  height: 440px;display:none;position:fixed;top:0px;left: 60%;z-index: 999;">'+
				'<div class="modal-header">'+
					'<a class="videoclose"></a>'+
					'<h3 class="overhidden">上传云视频</h3>'+
				'</div>'+
				'<div class="inner">'+
	 ...
	 $.widget('upyun.upyun', {
        	...
        	if(!modalBox.find('object')[0]){
	   			$.getScript( SCRIPT_URL + 'jqueryfn/jqueryfn_custom/uploadify/jquery.uploadify.js', function(){
	   				var target = modalBox.find('#YunPlace'),
		   		 	 	url = 'run.php?mid='+ gMid +'&a=get_form_api&client_id=' + client_id;
		   			 _this._ajax( target , url , null , function( json ){
		   				 if(json){
		   					 _this._flashReady( json[0] ); 
		   				 }
		   			 })
	   			}); 
   		 	}
        ...
        _flashReady : function( data ){
        	...
        	modalBox.find('#YunPlace').uploadify({
				...
				uploader      : data.action,															/*后台接受地址*/
				fileObjName : 'file',
				fileSizeLimit : data.allow_max_size,													/*视频尺寸限制*/
				fileTypeExts : data.allow_file_type,												    /*视频类型限制*/
				formData : {},
				removeTimeout : 30000,
				...
				onSelect : function( file ){
					 var target = modalBox.find('#YunPlace'),
				 	 	 url = 'run.php?mid='+ gMid +'&a=get_form_api&client_id=' + client_id;
					 _this._ajax( target , url , { title : file.name } , function( json ){
						 if(json){
							var data = json[0];
							policy[file.name] = data.policy ;
							signature[file.name] = data.signature ;
	 ...
	 var yunBtn = $('.add-yunvideo-btn', parent.document),
