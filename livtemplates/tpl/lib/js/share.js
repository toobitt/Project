var gisEditId = 0;
function hg_showAddShare(id,show_pic)
{
	if(!id)
	{	
		$('#tuji_title').text('新增分享');
		id = 0;
		gisEditId = 0;
	}
	else
	{
		$('#tuji_title').text('编辑分享');
		gisEditId = id;//编辑的标志位
	}
...
function hg_OverUpdateshareapp(json)
{
	var obj = eval('('+json+')');
	$('#shareplat_'+obj.id).text(obj.plats);
	$('#share_status_'+obj.id).text(obj.status);
	$('#updatetimef_'+obj.id).text(obj.updatetimef);
	$('#updatetimea_'+obj.id).text(obj.updatetimea);
	hg_success_tip();
	hg_closeTuJiTpl();
}
function hg_success_tip(){
	$('<span class="result-tip"></span>').text('保存成功').appendTo('body');
	setTimeout(function(){
		$('.result-tip').remove();
	},2000);
}
...
{
	var obj = eval('('+obj+')');
	var url = "./run.php?mid="+gMid+"&a=add_share_new";//插入一行列表
	hg_ajax_post(url);
	hg_closeTuJiTpl();
}
...
function show_typename()
{
	if($('#plat_type').val()!='127')
	{
		$("li[name='platdata']").remove();
	}
	else
	{
		var platpara_param = Array();
		var platpara_name = Array();
		platpara_param = $('#platpara_param').val().split(',');
		platpara_name = $('#platpara_name').val().split(',');
		var tota = platpara_param.length;
		for(i=tota-1;i>=0;i--){
			$("#add_sign").after("<li class='i'name='platdata'><div class='form_ul_div clear' id='add_sign'><span  class='title'>"+platpara_name[i]+": </span><input type='text' name='"+platpara_param[i]+"' id='"+platpara_name[i]+"' size='50' value='' /></div></li>"); 
		}
	}
