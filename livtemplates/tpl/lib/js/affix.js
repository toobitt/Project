var gisEditId = 0;
function hg_edit_affix(id)
{
	
	$('#tuji_title').text('编辑图片附件');
	gisEditId = id;//编辑的标志位
...
//将图片表单模板放入页面中
function hg_put_edit_form_tpl(html)
{
	$('#tuji_contents_form').html(html);
	top.livUpload.SWF.setButtonText("<span class='white'>编辑图片附件</span>");
}
...
{
	if(parseInt($('#tuji_sort_id').val()) == -1)
	{
		alert('请选择图集类别');
		return;
	}
