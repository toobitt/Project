jQuery(function($){
	...
	//		program_pm_slider:program_pm_slider,
		...
		_create:function(){
			...
			this.movie_id = $('input[name="movie_id"]').val();
		},
	...
	$('.movie_show .overflow').on('click' , function( event ){
		var self = $( event.currentTarget ),
			id = self.attr('attrid'),
			box = $('.movie-box');
		$.each( $.globalpmMovie , function( key , value ){
			if( id == value['id']){
				var src = value['index_pic']['host'] + value['index_pic']['dir'] + value['index_pic']['filepath'] + value['index_pic']['filename'];
				box.find('img').attr('src' , src );
				box.find('.movie-name').text(value['title']);
				box.attr('title' , value['title']);
			}
		} )
	});
    ...
    $(".save").click(function( event ){
	    ...
	    $(box).each(function(index,event){
			var tmp_data = '{"project_time":"'+$(event).attr("_start")
			+'","hall":"' + $(event).find("input[class*=position]").val()
			+'","ticket_price":"' + $(event).find("input[class*=price]").val()
			+'","language":"' + $(event).find("input[class*=lan]").val()
			+'","dimension":"' + $(event).find("input[class*=dim]").val()
			+ '","id":"' + $(event).attr("_id")
		    + '"}';
			data += space + tmp_data;
			space = ',';
		...
		data = '[' + data + ']';
		var Ndate = $('input[name="create_time"]').val();
		var info = {
			data : data, 
			cinema_id: parseInt($(this).attr('_cinema_id')), 
			movie_id : $('input[name="movie_id"]').val(),
			movie_name : $.trim($('#display_movie_show').text()),
			dates: Ndate ? Ndate : today
		}
		if( info.movie_id == 'other'){
			myTip($(this) , '请先选择影片' , 100, -80);
			return false;
		}
		var type = $('input[name="type"]').val();
		if( type == 0 ){
			var url = './run.php?'+'mid=' + gMid + '&a=create&ajax=1';
		}else{
			var url = './run.php?'+'mid=' + gMid + '&a=update&ajax=1';
		}
		$.post(url, info, function( data ){
			if( data.callback ){
				...
				saveprogram_callback( info.cinema_id );
			}
		}, 'JSON');
   ...
   function  myTip( self , tip , wid , left){
		self.myTip({
			string : tip,
			delay: 1000,
			width : wid,
...
function saveprogram_callback( cinema_id ){
	gTasks = {};
	window.location.href="./run.php?mid="+ gMid +"&a=frame&cinema_id=" + cinema_id;
}
