...
function wateruploadSuccess(file, serverData) {
	try {
		if (serverData.replace(/\s+/g,""))
		{
			...
			var str='<img src="' + obj[0].url + '" alt="" onclick="hg_select_water(\'' + obj[0].url + '\',\'' + obj[0].filename + '\',0);"/>';
			$(".awb").css("display","block");
			$(".awb").html(str);
...
function hg_swf_water()
{
	var vod_swfu_water;
	var url = "./run.php?mid=" + gMid + "&a=upload_water&admin_id=" + gAdmin.admin_id + "&admin_pass=" + gAdmin.admin_pass;
	//post_params: {},
	vod_swfu_water = new SWFUpload({
		upload_url: url,
		file_size_limit : "10 MB",
		file_types : "*.png;",
		file_types_description : "选择水印",
		file_upload_limit : "0",
		...
		button_placeholder_id : 'image_water',
		button_width: 100,
		button_height: 75,
		button_text : '',
		...
		custom_settings : {
			...
			progressTarget : "article_water_box"
		},
		debug: false
	});
}
