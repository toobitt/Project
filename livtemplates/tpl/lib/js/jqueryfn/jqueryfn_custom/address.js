 ...
 * Jquery省市区联动代码
 * $('.address-box').address({
		label : 'select',	//默认为select下拉框(目前只支持select)
		
		together : true,	//默认为true 选择省市区在不同的dom结构中，如果为false，表示在同一dom结构下
							//在不同的结构下需this中存在对应的select结构
		isSelect : true, 	// 默认为true 选择第一项 如果页面有指定省市区项时，就显示指定项。如果false，就选中 "请选择"
		
		provId : '#J_prov',	// 省下拉框id容器 格式是 #XXX
		cityId : '#J_city',
		areaId : '#J_area',
		
		provName : 'prov',	//select name值
		cityName : 'city',
		areaName : 'area',
	...
	默认值绑定在dom结构上：data-prov="220000" data-city="220100" data-dist="220104"
	或 _prov="220000" _city="220100" _dist="220104"
 */
(function( $ ){
	...
	Plugin.prototype =  {
		...
		init : function(){
			...
			this.provCode = container.attr("data-prov") || container.attr("_prov") || -1;
			this.cityCode = container.attr("data-city") || container.attr("_city") || -1;
			this.areaCode = container.attr("data-dist") || container.attr("_dist") || -1;
			
			// 使省市区id (#XX --> XX)
			this.provid = op.provId.replace(/^#/g,'');
			this.cityid = op.cityId.replace(/^#/g,'');
			this.areaid = op.areaId.replace(/^#/g,'');
			...
			var provFirstOpt = this.$( op.provId ).val();
			if(provFirstOpt == -1) {
				this.$( op.provId ).attr("data-code","-1");
				this.$( op.cityId ).attr("data-code","-1");
				this.$( op.areaId ).attr("data-code","-1");
			}
			
			self._change();		//绑定change事件
		},
		...
		_drawFunc : function(){
			...
			if( op.label == 'select' ){
				...
				if( op.provName ){
					elemHtml += '<select name="' + op.provName + '" id="' + this.provid + '" data-code="' + this.provCode + '"></select>';
				}
				if( op.cityName ){
					elemHtml += '<select name="' + op.cityName + '" id="' + this.cityid + '" data-code="' + this.cityCode + '"></select>';
				}
				if( op.cityName ){
					elemHtml += '<select name="' + op.areaName + '" id="' + this.areaid + '" data-code="' + this.areaCode + '"></select>';
				}
			}
			this.el[0].innerHTML = elemHtml;
		},
		...
		_change : function(){
			...
			this.$( op.provId ).change(function(e){		// change事件 省切换 自动切换市 and 区 
				self._cityFunc();
				self._areaFunc();
			});
			
			// 市 change事件 自动切换区
			this.$( op.cityId ).change(function(e){
				self._areaFunc();
			});
			
			// 区change事件 
			this.$( op.areaId ).change(function(){
		...
		_provFunc: function(){
			...
			this.el.find( op.provId )[0].innerHTML = this.createHtml(1);
			
			if( op.isSelect ) {
				this.$( $(op.provId + " option" )[1]).attr("selected",'true');
				if( this.provCode > -1 ){
					self._setSelectCode( op.provId, this.provCode );
				}
			} 

			op.provCallBack && $.isFunction(op.provCallBack) && op.provCallBack();
		},
		...
		_cityFunc: function(){
			...
			var provIndex = this.$( op.provId ).get(0).selectedIndex,   // 获取省份的索引index
				provValue;
			var opthtml = '<option value="-1" data-name="请选择" selected>请选择</option>';
			
			this.$( op.cityId ).empty();
			this.$( op.areaId ).empty();
			/*
			...
			if( provIndex < 0 ){
				...
				this.$( op.cityId ).html( opthtml );
				this.$( op.areaId ).html( opthtml );
				return;
			}
			
			provValue = this.$( op.provId ).val();
			this.el.find( op.cityId )[0].innerHTML = this.createHtml( provValue );
			
			if( op.isSelect ) {
				this.$( $(op.cityId + " option" )[1] ).attr( "selected",'true' );
				if( this.cityCode > -1) {
					self._setSelectCode( op.cityId,this.cityCode );
				}
			}
			
			self._selectChangeCode();
			
			op.cityCallBack && $.isFunction(op.cityCallBack) && op.cityCallBack();
		},
		...
		_areaFunc: function(){
			...
			var provIndex = this.$( op.provId ).get(0).selectedIndex,
				cityIndex = this.$( op.cityId ).get(0).selectedIndex,
				cityValue;
			...
			cityValue = this.$( op.cityId ).val();
			this.el.find( op.areaId )[0].innerHTML = this.createHtml( cityValue );	
			
			if( op.isSelect ) {
				this.$( $( op.areaId + " option" )[1] ).attr("selected",'true');
				if( this.areaCode > -1 ) {
					self._setSelectCode( op.areaId,this.areaCode );
				}
			}
			self._selectChangeCode();
	
			op.areaCallBack && $.isFunction( op.areaCallBack ) && op.areaCallBack();
		},
	    ...
	    _setSelectCode: function( selectId, code ){
	    	...
	    	var optionElems = this.$( selectId + " option");
			
	    	this.$( optionElems ).each(function( index,item ){
	    ...
	    _changeSetCode: function(selectId){
	    	...
	    	var idx = selectId.selectedIndex, //获取选中的option的索引
	            value,
				dataValue = this.$( op.provId ).val();
			
			var opthtml = "<option data-name='请选择' value='-1' selected>请选择</option>",
				provOpt = this.$( $( op.provId + ' option' )[0] ).attr("value");
	        ...
	        if(idx > -1) {
				if(dataValue == -1) {
					...
					value = this.$( selectId ).val();  //获取选中的option元素value值
					this.$( selectId ).attr("data-code",value);
				}
	        }
	    },
		...
		createHtml : function( value ){
			...
			this.find(value, function(ii, item){
				elemHtml += "<option value='"+ ii +"' data-name='"+ item[0] +"'>"+ item[0] +"</option>";
			});
			return elemHtml;
		},

		find: function(name, fn){
		    var data = this.op.data;
