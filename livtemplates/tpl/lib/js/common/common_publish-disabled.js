(function($){
    var defaultOptions = {
        url : 'fetch_column.php?siteid={{siteid}}&fid={{fid}}',
        maxcolumn : 2,
        column : 2,
        guding : true,
        height : 0,
    ...
    $.fn.commonPublish = function(options){
        ...
        return this.each(function(){
            ...
            if(options['hiddenName']){
                hiddenInput.attr('name', options['hiddenName']);
            }
                ...
                'mask.publish' : function(){
                    ...
                    mask.one('click', function(){
                        ...
                        $(button).html(function(){
                            return names.length ? ($(this).attr('_prev') + '<span style="color:#000;">' + names.join(',&nbsp;') + '</span>') : $(this).attr('_default');
                        });
                    });
                },
            ...
            innerList.on({
                ...
                'ajax.publish' : function(event, type, id, myurl){
                    var oldCache = cache.get(type, id);
                    if(oldCache){
                ...
                'add.publish' : function(event, data){
                    ...
                    $.each(data, function(i, n){
                        ...
                        if(!n['is_auth']){
                            ul.find('li[_id="'+ n.id +'"] input').css('visibility', 'hidden');
                        }
                    });
            ...
            innerList
                ...
                .on('mouseup', 'input', function(event){
                    ...
                    if(state){
                        ...
                        result.triggerHandler('remove.publish', [id]);
                    }
                });
            ...
            result
                .on('mouseup', 'input', function(){
                    var id = $(this).closest('li').attr('_id');
                    result.triggerHandler('remove.publish', [id]);
                    innerList.find('li[_id="'+ id +'"] input').prop('checked', false);
                ...
                .on({
                    ...
                    'remove.publish' : function(event, id){
                        $(this).find('li[_id="'+ id +'"]').remove();
                    ...
                    'check.publish' : function(){
                        $(this).find('li').each(function(){
                            innerList.find('li[_id="'+ $(this).attr('_id') +'"] input').prop('checked', true);
                        });
                    }
                });
