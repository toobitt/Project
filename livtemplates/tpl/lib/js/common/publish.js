(function ($) {
	...
	var publis_li_tpl = 
		'<li _id="${id}" _fid="${$item.fid}" title="{{if $item.showName}}${$item.showName} > {{/if}}${name}" _name="${name}" _showName="{{if $item.showName}}${$item.showName} > {{/if}}${name}" class="one-column {{if +is_last}}no-child{{/if}}">'+
			'<input type="checkbox" class="publish-checkbox" {{if $data.is_auth == 2}}style="visibility: hidden;"{{/if}} /> '+
			'<span class="publish-name">${name}</span>'+
			'<span class="publish-child">&gt;</span>'+
		'</li>';
	var result_tpl =
		'<li _id="${id}" _name="${name}" data-auth="${is_auth}" _siteid="${$item.siteid}" title="${showName}" class="result_show" {{if $data.is_auth == 2}}style="visibility: hidden;"{{/if}}>'+
			'<input type="checkbox" checked="checked" class="publish-checkbox" {{if !is_auth}}style="visibility: hidden;"{{/if}}/>'+
			'<span>${showName}</span>'+
		'</li>';
	var site_tpl = 
		'{{each sites}}' +
		'<li class="publish-site-item {{if $index == $item.default_site.key}}publish-site-select{{/if}}" _siteid="${$index}" _name="${$value}">' +
		 	'<label><input type="radio" name="publish-sites-${$item.unique}" {{if $index == $item.default_site.key}}checked="checked"{{/if}} />' +
		 	'${$value}</label>' +
		 '</li>' +
		 '{{/each}}';
	var load_more_tpl = 
    ...
    var defaultOptions = {
    	...
    	getUrl: function(id) {
    		id = id || this.model.id;
    		return 'run.php?mid=' + gMid + '&a=recommend&id=' + id;
    	}
    };
	function Publish(el, options) {
		...
		//时间选择
		//el.find('input[name="pub_time"]').datetimepicker();
	...
	$.extend(Publish.prototype, {
		...
		reinit: function(ids) {
			...
			this.cache.get(this.options.getUrl.call(this, ids), {
				after: function(data) {
					if (_this.model !== model) return;
					_this.addResult(data.selected_items || [], { reset: true });
					_this.addChild(data.items || [], '', { reset: true });
					_this.el.find('.publish-site-current').attr('_siteid', data.default_site.key);
					_this.el.find('.publish-site-current').text(  data.default_site.value  );
					_this.site.find('ul').empty().append($.tmpl(site_tpl, {
						sites: data.sites
					}, {
						default_site: data.default_site,
						unique: ++_this.unique
					}));
					_this.el.find('[name=pub_time]').val(data.pub_time);
					_this.el.find('[name=custom_filename]').val(data.custom_filename);
				},
				type: 'json'
			});
		},
		...
		loadMore: function(e) {
			...
			this.cache.get(this.getUrl(this.siteid, fid, offset), {
				type: 'json',
				after: after
			});
		...
		getUrl: function(id1, id2, offset) {
			return this.options.php + '?siteid=' + id1 + '&fid=' + id2 + (offset  ? 
					'&offset=' + offset + '&count=' + this.options.count : '');
		},
		adjustView: function () {
