    ...
    $.widget('m2o.hotarea', $.ui.mouse, {
        ...
        _addCss : function(){
            if(!$('style#m2o-hotarea-css').length){
                var css = '' +
                    '.m2o-hotarea .hot-item{position:absolute;border:2px dashed red;background-color:rgba(255,0,0,0.3);}' +
                    '.m2o-hotarea .hot-item.on{border:2px red solid;}' +
        ...
        _mouseStart : function(event){
            ...
            this.currentHot = this.createHot({
                left : this.startLT[0],
                top : this.startLT[1],
                width : 0,
                height : 0
        ...
        _mouseDrag : function(event){
            ...
            this._refreshMaxHeight( this.currentHot );
            return false;
        },
        ...
        createHot : function(info){
            ...
            this._insertLinkChooseArea( flag );
            this._refreshMaxHeight( hot );
            return hot;
        },
        ...
        _insertLinkChooseArea : function( flag ){
        	var	pos = $('.hot-item').eq(flag-1),
        		items = $('.link-news-link').find('li')
        		len = items.length;
    		$('#link-choose-area-tpl').tmpl().appendTo(pos).attr('_flag',flag);
    		var ul = pos.find('.link-choose');
    		$.each(items,function(k,v){
    			var content = $(this).find('p').text();
    			var html = '<li class="option" _id=' + $(this).attr('_id') + '>' + 
    						'<a title="' + content + '">' + content + '</a></li>';
    			$(html).appendTo(ul);
    		});
    		$('.link-choose-area').show(300);
        },
        _refreshMaxHeight : function( hotarea ){
        	var maxHeight = hotarea.css('height'),
        		ul = hotarea.find('.link-choose');
        	ul.css({'max-height':maxHeight,'overflow-y':'scroll'});
        }
