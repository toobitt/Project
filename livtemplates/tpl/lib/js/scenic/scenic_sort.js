jQuery(function($){
    ...
    var ajax = '_fetch_node.php?multi=scenic_sort&fid={{fid}}&ban=1';
    ...
    box.on('click', '.column-add-box', function(){
        ...
        if(len){
            if(!each.prev().find('.column-li-selected').length){
                errorTip(each.prev(), '请在这一列中选择父级景区分类...', [5, -5, 3, -3, 1, -1]);
                return;
            }
        }
    ...
    box.on('click', '.column-sort-button', function(){
        ...
        if(length < 2){
            var mi = $(this);
            errorTip(mi, !length ? '这一列没有景区分类' : '只有一个景区分类无法排序');
            return;
        }
