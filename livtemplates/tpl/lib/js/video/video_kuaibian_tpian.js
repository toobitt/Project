    ...
    $.widget('video.pian', {
        ...
        _init : function(){
            ...
            if(initInfo){
                ...
                initInfo['start'] = parseFloat(initInfo['start']);
                initInfo['end'] = parseFloat(initInfo['end']);
                initInfo['time'] = parseFloat(initInfo['time']);
                start = initInfo['start'] / initInfo['time'] * 100;
                end = (initInfo['end'] && initInfo['end'] > 0) ? (initInfo['end'] / initInfo['time'] * 100) : (start + this.options.distance);
                this.slider.myslider('option', 'values', [start, end]).trigger('_hide');
                this.slider.myslider('createHandlePart');
                this.slider.trigger('_time', [0, initInfo['start']]);
                this.slider.trigger('_time', [1, initInfo['end']]);
        ...
        _slider : function(){
            ...
            this.slider = $('<div/>', {
                ...
                _change : function(event, value){
                    ...
                    $(this).trigger('_time', [index]);
                    me._img(index, 'show');
                    me._refresh();
                },

                _time : function(event, index, time){
                    if(typeof time == 'undefined'){
                        time = me.video.video('getTime') * 1000;
                    }
