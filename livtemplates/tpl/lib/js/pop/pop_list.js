(function($){
    var dataInfo = {
					...
					'<form method="post" id="custom-form" enctype="multipart/form-data">' + 
						'<ul>' + 
							'<li>' + 
								...
								'<input type="text" name="title" class="content-input" value="${title}" />' + 
							'</li>' + 
							'<li class="i">' + 
								'<span class="title">描述</span>' + 
								'<textarea name="brief" class="descr-area">${brief}</textarea>' + 
							'</li>' +
							'<li class="i">' + 
								'<span class="title">链接</span>' + 
								'<input type="text" name="outlink" class="link-input" value="${outlink}" />' + 
							'</li>' + 
							'<li>' + 
							...
							'</li>' + 
							'<li>' +
							   '<input type="button" class="save" value="提交">' +
							'</li>' +
						'</ul>' + 
					...
					'<div class="search-select-area">' +
						'<div class="search-select-item">' +
							...
							'<ul class="select-list">' +
								'<li _type="publish_lib" class="select-each-item" data-type="navcontroll">发布库</li>' +
								'<li _type="publish_custom" class="select-each-item" data-type="navcontroll">自定义添加</li>' +
							'</ul>' + 
						'</div>' +
		...
		css : '' + 
			...
			'.pop-search .search-button{position:absolute;top:0px;right: 0px;border:0;cursor:pointer;width:40px;height:43px;margin-right:30px;border-left:1px solid #ccc;border-right:1px solid #ccc;background:url(' + RESOURCE_URL+'datasource/search.png) no-repeat center #fff;}' +
			'.pop-search .search-button:hover{border:0;border-left:1px solid #ccc;border-right:1px solid #ccc;}'+ 
			'.search-k{position:absolute;top:9px;line-height:20px;right:90px;}' +
			...
			'.custom-form-area{padding:15px 25px;position:relative;}' +
			'.custom-form-area li{margin-bottom:14px;}' +
			'.custom-form-area li.i{border-bottom:1px dotted #cfcfcf;padding-bottom: 14px;}' + 
			'.custom-form-area .title{display: block;float: left;width: 35px;height: 30px;text-indent: 3px;line-height: 30px;color: #7d7d7d;}' +
			'.custom-form-area .content-input{width:590px;height:26px;}'+
			'.custom-form-area .descr-area{width:590px;height:90px;}'+
			'.custom-form-area .link-input{width:780px;height:26px;}'+
			'.custom-form-area .index-pic{position: absolute;top: 15px;right: 32px;width: 175px;height: 140px;border: 1px solid #c4c4c4;background: url(' + RESOURCE_URL + 'special/default-pic.png) no-repeat center #f9f9f9;cursor: pointer;line-height: 140px;text-align: center;font-size:0;}' +
			'.custom-form-area .index-pic img{vertical-align: middle;}'+
			'.custom-form-area .save {cursor: pointer;display: block;margin-top: 10px;width: 98px;height: 34px;margin-left: 35px;color: #fff;text-align: center;font-size: 14px;border: 0;border-radius: 2px;background: url(' + RESOURCE_URL + 'menu2013/form/form-button-blue.png) repeat-x;}' +
			'',
		cssInited : false
    };
    ...
    $.widget('pop.pubLib', $.pop.pop, {
    	options : {
    		...
    		upload_url : './run.php?mid=' + gMid + '&a=upload',
    		phpkey : 'Filedata'
    	...
    	_create : function(){
    		this._super();
    		this._template( 'list_box', dataInfo.list_box, dataInfo, this.publish_lib_wrap );
    	...
    	_initCustomForm : function(){
    		var	_this = this,
    			op = this.options;
    		this._initUnit( this.publish_custom_wrap, 'custom', {
    			submitCall : op['clickCall'],
    			uploadUrl : op['upload_url'],			//自定义form索引图接口
    			phpkey : op['phpkey'], 					//文件name
    			publib_widget_this :  _this
    		} );
    	},
    	
    	onoff : function( isonoff ){
    		if( isonoff ){
    			this.publish_lib_wrap.hide();
    			this.publish_custom_wrap.custom('refresh');
    		}else{
    			this.publish_custom_wrap.hide();
    			this.publish_lib_wrap.show();
    		}
    	}
    });
    
    $.widget('pop.custom', $.pop.base, {
    	options : {
...
//    		this.element.find('.index-pic img').attr('src' , src );
    		
    	},
    	
    	_uploadAfter : function( json ){
    		var box = this.element.find('.index-pic'),
    			src = json.data.host + json.data.dir + '175x140/' + json.data.filepath + json.data.filename;
    		box.find('img').attr( 'src' , src );
    		box.find('input[name="indexpic_id"]').val( json.data.material_id );
    		box.find('input[name="pic"]').val( JSON.stringify(json.data) );
    	},
    	
    	_submit : function( event ){
    		var publib_widget_this = this.options.publib_widget_this,
    			self = $( event.currentTarget ),
    			title = this.element.find('input[name="title"]').val(),
    			outlink = this.element.find('input[name="outlink').val();
    		if( !title ){
    			this.myTip( self , '标题不能为空！');
    			return false;
    		}
    		if( !outlink ){
    			this.myTip( self , '链接不能为空！');
    			return false;
    		}
//    			id = this.element.find('input[name="id"]').val(),
//    			a = id ? 'update' : 'create',
//	    		form = this.element.find('#custom-form'),
    		var data = this._getData();
    		this._trigger( 'submitCall', event, [ data, publib_widget_this ] );
			this._clearData();
...
//				},
//				type : 'POST',
//				url : window.SITE_URL +  'index.php/topic/' + a + '?ajax=1', 
//				dataType : 'json',
...
//			});
    	...
    	_getData : function(){
			var title = this.element.find('input[name="title"]').val(),
    			brief = this.element.find('textarea[name="brief').val(),
    			outlink = this.element.find('input[name="outlink').val(),
    			src = this.element.find('input[name="pic"]').val();
			var data = [],
				info = {};
			info.title = title;
			info.brief = brief;
			info.content_url = outlink;
			info.indexpic = src && JSON.parse( src ) ;
			info.module_id = '';
			info.bundle_id = '';
			info.id = '';
			data.push( info );
    	...
    	_clearData : function(){
    		this.element.find('input[type="text"] , input[type="hidden"] , textarea').val('');
			this.element.find('.index-pic img').attr('src' , '');
    	},
    	
    	getData : function( option ){
    		this._template( 'form_tpl', dataInfo.form_tpl,dataInfo, this.element );
    		this._initFile();
    	},
    	
    	refresh : function( option ){
    		this.element.show().find( '.custom-form-area' ).remove();
    		this.getData( option );
    	},
    ...
    $.widget('pop.search', $.pop.base, {
    	...
    	_select : function( event ){
			...
			if( type == 'navcontroll' ){
				var _type = self.attr('_type'),
					isonoff = ( _type == 'publish_custom' ) ? true : false;
				this.options.publib_widget.pubLib('onoff',isonoff);
				this._onoffSearch( isonoff );
				return;
			}
			if( type == '1' ){
				if( id == 'other' ){
    	...
    	_onoffSearch: function( isonoff ){
    		var select_area = this.element.find('.search-select-area'),
    			weight_area = this.element.find('.search-weight-area'),
    			key_area = this.element.find('.search-key-area');
    		weight_area[isonoff ? 'hide' : 'show']();
    		key_area[isonoff ? 'hide' : 'show']();
    		select_area.find('.search-select-item').not('.search-select-item:first')[isonoff ? 'hide' : 'show']();
    	},
