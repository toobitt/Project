<li class="common-list-data clear"  id="r_{$v['id']}"    name="{$v['id']}"   orderid="{$v['order_id']}">
    ...
    <div class="common-list-right">
        ...
        <div class="common-list-item vote-tjr wd120">
            <div class="common-list-cell">
			    <span class="common-user"> {if $v['fortuneinfostart']!=$v['fortuneinfoend']}从{/if}{$v['fortuneinfostart']}</span>
			  {if $v['fortuneinfostart']!=$v['fortuneinfoend']}  <span class="common-time">至{$v['fortuneinfoend']}</span>{/if}
            </div>
        </div>
   </div>
   <div class="common-list-biaoti">
	    <div class="common-list-item biaoti-transition">
			   ...
			   <a href="./run.php?mid={$_INPUT['mid']}&a=form&id={$v['id']}&infrm=1">
                <span class="common-list-overflow m2o-common-title" title="{$v['astrocn']}">{$v['astrofuncn']}</span>
               </a>
            </div>  
	    </div>
   </div>
