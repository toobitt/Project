		...
		{foreach $list AS $k => $v}
			...
			<tr onmouseover="hg_row_interactive(this, 'on');" onmouseout="hg_row_interactive(this, 'out');" onclick="hg_row_interactive(this, 'click', 'cur');" id="r{$v[$primary_key]}"  class="h" align="left" valign="middle">
				...
				{if !$vv['group_op']}
					{if($kk == 'authorize')}
						{if ($v['admin_group_id'])}
							<a href="{$vv['link']}&amp;{$vv['pre']}{$primary_key}={$v[$primary_key]}&amp;gid={$v['admin_group_id']}" title="{$vv['brief']}"{$vv['attr']}>{$vv['name']}</a>
						{else}
							<a href="{$vv['link']}&amp;{$vv['pre']}{$primary_key}={$v[$primary_key]}" title="{$vv['brief']}"{$vv['attr']}>{$vv['name']}</a>
						{/if}
					{else if($kk == 'authorize_node')}
						{if ($v['admin_group_id'])}
							<a href="{$vv['link']}&amp;{$vv['pre']}{$primary_key}={$v[$primary_key]}&amp;gid={$v['admin_group_id']}&amp;name={$v['user_name']}" title="{$vv['brief']}"{$vv['attr']}>{$vv['name']}</a>
						{else}
							<a href="{$vv['link']}&amp;{$vv['pre']}{$primary_key}={$v[$primary_key]}&amp;name={$v['name']}" title="{$vv['brief']}"{$vv['attr']}>{$vv['name']}</a>
						{/if}
					{else}
