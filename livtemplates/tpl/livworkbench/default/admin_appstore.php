...
<div style="position:absolute;top:250px;left:150px;border:1px #999 solid;background:#eee;padding:10px;width:600px;margin:auto;z-index:999">
	<h2>{$appinfo['name']}</h2>
	<div>预发布版本：{$appinfo['pre_version']} 
		<a href="admin_appstore.php?a=pre_release&app={$appinfo['app_uniqueid']}" style="margin-left:10px;color:red;">预发布新版本</a>
		{if $appinfo['pre_release_url'] != 'NO_VERSION'}
			<a href="{$appinfo['pre_release_url']}" style="margin-left:10px;">程序下载</a>
		{else}
			<a href="admin_appstore.php?a=pre_release&app={$appinfo['app_uniqueid']}" style="margin-left:10px;">版本未发布</a>
		{/if}
	</div>
	<div>发布版本：{$appinfo['version']} 
		<a href="admin_appstore.php?a=release&app={$appinfo['app_uniqueid']}" style="margin-left:10px;color:red;">发布新版本</a>
		{if $appinfo['release_url'] != 'NO_VERSION'}
			<a href="{$appinfo['release_url']}" style="margin-left:10px;">程序下载</a>
		{else}
			<a href="admin_appstore.php?a=pre_release&app={$appinfo['app_uniqueid']}" style="margin-left:10px;">版本未发布</a>
		{/if}
	</div>
	<div>{$appinfo['brief']}</div>
</div>
{/if}
