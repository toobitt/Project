...
/*
$(function($){
    $('.news-status-button').on('click', function(){
        if($(this).data('ajax')) return;
        var state = parseInt($(this).attr('_state'), 10),
            id = $(this).attr('_id');
        var me = $(this);
        $(this).data('ajax', true);
        $.post(
            './run.php?mid=' + gMid + '&a='+ (state == 1 ? 'back' : 'audit') ,
            {team_id : id},
            function(){
                me.data('ajax', false);
                me.html(state == 1 ? '未审核' : '已审核').attr('_state', state == 1 ? 0 : 1);
            }
        );
    })
});
          ...
          <div class="right v_list_show">
			...
			<div class="list_first clear"  id="list_head">
                    ...
                    <span class="right" style="width:850px;">                 
						 <a class="fl" style="width:400px;">小组描述</a>
						 <a class="fl" style="width:90px;">小组类型</a>
						 <a class="fl" style="width:90px;">小组分类</a>
						 <a class="fl" style="width:90px;">审核</a>
						 <a class="fl" style="width:120px;">添加人/添加时间</a>
                   </span>
                   <a class="title" style="margin-left:53px;margin-top:8px;">小组名称</a>  
           </div>
...
function hg_team_audit(id)
{
	var ids=id.split(",");
	for(var i=0;i<ids.length;i++)
	{
		//$("#statusLabelOf"+ids[i]).text('已审核');
		$("#r_"+ids[i]).remove();
	}
}

function hg_team_back(id)
{
	var ids=id.split(",");
	for(var i=0;i<ids.length;i++)
	{
		$("#statusLabelOf"+ids[i]).text('未审核');
	}
}
</script>
{template:foot}