<?php 
/* $Id: index.php 12369 2012-09-25 07:20:56Z repheal $ */
?>
{template:head}
			...
			<div class="layermedia">
	        	...
	        	<div class="video_list">
					...
					{code}
						$space = "";
					{/code}
					{foreach $_settings['video_type'] as $key => $value}
						{code}
							$space;
						{/code}
						<a target="_blank" href="http://{$value}">{$key}</a>
						{code}
							$space = "、";
						{/code}
					{/foreach}
...
{if !empty($statusline)&&is_array($statusline)}
	{if SCRIPTNAME == 'index' && $_user['id']==0}
		{template:unit/statusline_index}
	{else}
...
{/if}
	...
	<div class="user">
		...
		<a href="{code} echo hg_build_link(SNS_UCENTER.'user.php');{/code}"><img src="{$user_info['middle_avatar']}" title="{$user_info['username']}" /></a>
	</div>
