 <?php
/*$Id: mytemplate.php 390 2011-07-26 05:35:00Z lijiaying $*/
?>
			...
			<div>
			 ...
			 <?php //foreach($style_list as $kk => $vv){?>
					...
					<div class="set_setBackground lf">
						<div id="use_bg" class="set_bg set_border" onclick="changeClass(this)">
							<div class="set_isUsing">
								<div class="imgBox">
									<img id="tips_img" height="60px" width="109px" src="{$save_file_path}" alt="背景图" onload="changeBG();" onreload="changeBG();">
								</div>
								<p>使用背景图</p>
							</div>
						</div>
					...
					<div class="set_otherSettings lf">
						...
						<span class="set_gray" id="upload_process" style="display:none;">上传中...<img src="<?php echo RESOURCE_DIR; ?>img/spinner.gif" alt="上传中" /></span>
							<form target="Upfiler_iframe" name="upload_form" method="post" action="mytemplate.php?a=uploadImg" enctype="multipart/form-data" >
								...
								<strong style="display: none;">{$_lang['success']}</strong> 
							</form>
							<p class="set_gray">
							...
							<input type="hidden" value="{$result}" name="re" id="up_result" />
						</div>
				...
				<div class="set_changeColor" style="display: none;" id="mainColor_div">
					<ul>
					<!-- 以下要循环输出配置的选项 -->
					{foreach $_mSetting as $key => $val}
					{code}
						$vv = explode("_",$key);
					{/code}
						<li class=""  onclick="cpinit({$vv[1]})" id="cSli_{$vv[1]}">
							<input type="text" disabled="disabled" class="set_pageBack" id="mainD_{$vv[1]}" style="border:0;background-color:{$val['color']}" >
							<input type="hidden" id="mainF_{$vv[1]}" value="{$val['color']}" class="colorSet"/> 
							<div  id="mainCP{$vv[1]}" ></div>
							<p>{$val['name']}</p>
						</li> 
					{/foreach}
					</ul>
