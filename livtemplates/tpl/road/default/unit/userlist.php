 <li class="common-list-data clear"  id="r_{$v['id']}"    name="{$v['id']}"   orderid="{$v['order_id']}"   cname="{$v['cid']}"    corderid="{$v['order_id']}">
	    ...
	    <div class="common-list-biaoti" style="cursor:pointer;">
	    	<div class="common-list-item">
		        <div class="common-list-cell">
		           <span>
		           	{if $v['avatar']}<img src="{$v['avatar']}" style="vertical-align:middle;width:40px;height:30px;margin-right:10px;" />{else}{/if}<a title="{$v['name']}"><span id="title_{$v['id']}" class="m2o-common-title">{$v['name']}</span></a>
		           </span>
		        </div> 
	        </div>         
	    </div>						    		    
