<div {if $v['is_plan']} style="background:#E5EEFF;" {/if} {if $v['record_status']} style="background:#FCF9EA;" {/if} class="input_div{$hg_attr['disabled_cls']} {if !$v['plan_status']}edit_flag{/if}" onchange="hg_content_change(this);"  onclick="hg_input_div_bgcolor(this);">
	...
	{else}
		<span class="chg_type_fir">信号</span>
		<span class="chg_type_sec">{$v['channel2_name']}</span>	
	{/if}
	...
	<input type="hidden" class="source_id" name="source_id[]" value="{$v['source_id']}"{$hg_attr['disabled']} />
	<input type="hidden" class="hidden_flag" value="{$v['is_plan']}" />
	<input type="hidden" name="record_status[]" value="{$v['record_status']}" />
</div>