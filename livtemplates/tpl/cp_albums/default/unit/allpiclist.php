<li class="clear" style="z-index:45;border:1px solid #fff;height:169px;width:186px;"  id="r_{$v['material_id']}"    name="{$v['material_id']}"   orderid="{$v['order_id']}"  onmouseout="hg_row_interactive(this, 'out');hg_show_tips({$v['material_id']},0);" onmouseover="hg_row_interactive(this, 'on');hg_show_tips({$v['material_id']},1);"   onclick="hg_row_interactive(this, 'click', 'cur');hg_check_boxok({$v['material_id']});">
		<span class="left">
			<a class="lb" name="alist[]" ><input type="checkbox" name="infolist[]"  value="{$v['material_id']}" title="{$v['material_id']}" id="primary_key_{$v['material_id']}"  /></a>
			<a class="slt" style="left:1px;top:1px;" ><img src="{$v['file_path']}"   width="40" height="30" style="width:184px;height:137px;" /></a>
		</span>
		<span class="right">
			<a class="fb" id="delete_{$v['material_id']}" href="run.php?mid={$_INPUT['mid']}&a=delete&id={$v['material_id']}" onclick="return hg_ajax_post(this, '删除', 1);" style="bottom:4px;display:none;right:9px;"><em class="b3" ></em></a>
			...
			<a class="fl_text overflow" id="tips_{$v['material_id']}" style="top:118px;left:38px;width:147px;">
			<span class="m2o-common-title">{$v['user_name']}</span></a>
		</span>
