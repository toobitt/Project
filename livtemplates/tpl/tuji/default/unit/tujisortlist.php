                 <li class="clear"  id="r_{$v['id']}"    name="{$v['id']}"   orderid="{$v['order_id']}"  onmouseout="hg_row_interactive(this, 'out');" onmouseover="hg_row_interactive(this, 'on');">
                    	<span class="left">
							...
							<a class="fb overflow"  style="width:120px;{if $v['color']}color:{$v['color']}{/if}"  id="tuji_sort_name_{$v['id']}" href="run.php?mid={$_INPUT['mid']}&fid={$v['id']}&infrm=1">{$v['name']}</a>
						</span>
	                        <span class="right" onclick="hg_row_interactive('#r_{$v[id]}', 'click', 'cur');"  style="width:239px;">
								<a class="fb" title="编辑" href="javascript:void(0)" onclick="return hg_showAddpicsort({$v['id']})"><em class="b2" ></em></a>
						...
						<span class="title overflow"  style="cursor:pointer;"><a id="tuji_sort_desc_{$v['id']}">{$v['brief']}</a></span>
                </li>