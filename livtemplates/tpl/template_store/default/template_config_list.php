...
<form>
	<table  border="0" cellpadding="0" cellspacing="0" width="100%"  id="channel_table" class="form_table">
		<tr id="item_th" class="h" align="left" valign="middle" >
			...
			<th class="text_indent">配置＃ID</th>
			<th class="text_indent">分组</th>
			<th class="text_indent">标识</th>
			<th class="text_indent">值</th>
			<th class="text_indent" width="220">创建时间</th>
			<th class="text_indent" width="250">管理</th>
		</tr>	
		<tbody id="status">
		{if $template_config_list}
		   {foreach $template_config_list as $k => $v}
			<tr orderid="{$v['id']}"  id="r_{$v['id']}" class="h"   name="{$v['id']}"  align="left"  valign="middle"  onmouseover="hg_row_interactive(this, 'on');" onmouseout="hg_row_interactive(this, 'out');" onclick="hg_row_interactive(this, 'click', 'cur');">
				...
				<td class="text_indent"><span onclick="hg_backup_flv({$v['id']});" style="cursor:pointer;" title="预览">{$_configs['config_type'][$v['type']]}</span></td>
				<td class="text_indent" >{$v['key']}</td>
				<td class="text_indent">{$v['value']}</td>
				<td class="text_indent">{code} echo date('Y-m-d H:i', $v['create_time']);{/code}/{$v['user_name']}</td>
