...
{/code}
<script>
function scenic_survey_form(id)
{
	window.location.href="modify.php?app_uniqueid={$_configs['appmod']['news']['app']}&mod_uniqueid={$_configs['appmod']['news']['mod']}&app={$scenic_survey_list[0]['app']}&mod={$scenic_survey_list[0]['mod']}&para={$_INPUT['para']}";
}
</script>
<style type="text/css">
...
<form action="" method="POST" name="add_survey" id="add_survey">
	<span type="button" class="button_6"  onclick="scenic_survey_form()">新增概况</span>
</form>
</div>
                   ...
                   <ul class="common-list" id="list_head">
                        <li class="common-list-head clear">
                            <div class="common-list-left">
                                ...
                                <div class="special-slt common-list-item">概况图</div>
                            </div>
                            <div class="common-list-right">
                                <div class="common-list-item open-close">操作</div>
                                <div class="common-list-item open-close">ID</div>
                                <div class="common-list-item open-close">概况状态</div>
                                <div class="common-list-item open-close">更新时间</div>
                            </div>
                            <div class="common-list-biaoti ">
						        <div class="common-list-item open-close special-biaoti">概况标题</div>
					        </div>
                        </li>
                    </ul>
	                <ul class="common-list" id="tujilist">
					    {if $scenic_survey_list[0]['data']}
		       			    {foreach $scenic_survey_list[0]['data'] as $k => $v} 
		                      {template:unit/scenicsurveylist}
		                    {/foreach}
						{else}
						<p style="color:#da2d2d;text-align:center;font-size:20px;line-height:50px;font-family:Microsoft YaHei;">没有概况！</p>
						<script>hg_error_html(scenicsurveylist,1);</script>
		  				{/if}
	                </ul>
