<?php
/* $Id: register.php 634 2011-09-09 01:04:57Z repheal $ */
?>
...
reg = function(curobj){
	...
	if ($("#username").val() != ""&&$("#email").val() != ""&&$("#password").val() != ""&&$("#passwords").val() == $("#password").val()&&$("#verifycode").val() != ""&&$(".con1 #checkbox:checked").val() == 1 )
	{
		...
		$("#submitTips").html('<span class="ok"></span><strong>{$_lang["submit_tips_one"]}</strong>');
		$.ajax({
			url: $("#to").val(),
			type: 'POST',
			dataType: 'html',
	...
	else
	{
		...
		{
			$("#emailTips").attr('class','failing');
			$("#emailTips").html('<span class="error"></span><strong>{$_lang["email_tips_one"]}</strong>');
		}
		if(!$("#username").val())
		{
			$("#usernameTips").attr('class','failing');
			$("#usernameTips").html('<span class="error"></span><strong>{$_lang["username_tips_one"]}</strong>');
		}
		if(!$("#password").val())
		{
			$("#passwordTips").attr('class','failing');
			$("#passwordTips").html('<span class="error"></span><strong>{$_lang["password_tips_one"]}</strong>');
		}
		if(!$("#passwords").val())
		{
			$("#passwordsTips").attr('class','failing');
			$("#passwordsTips").html('<span class="error"></span><strong>{$_lang["passwords_tips_one"]}</strong>');
		}
		if($(".con1 #checkbox:checked").val() != 1)
		{
			$("#agreeTips").attr('class','failing');
			$("#agreeTips").html('<span class="error"></span><strong>{$_lang["agree_tips"]}</strong>');
...
<p style="text-align:right;padding-right:20px;">已有葫芦网账号，<a href="<?php echo hg_build_link('login.php'); ?>" >登录</a></p>
	<table id = "register" width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  height="450">
	  <tr class="txts">
	    <td width="200" align="right" valign="middle">{$_lang['email']}：</td>
...
<input type="hidden" value="{$invite_code}" id="invite_code"/>
