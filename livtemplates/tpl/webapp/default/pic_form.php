...
function del_pic(id)
{
		...
		var url = "run.php?mid="+gMid+"&a=del_pic&id="+id;
		hg_ajax_post(url);
				...
				<ul class="form_ul">
					...
					<li class="i">
						<div class="form_ul_div">
							...
							<input type="text" value="{$formdata['title']}" name='title' style="width:180px;">
					...
					<li class="i">
						<div class="form_ul_div">
							...
							<input type="text" value="{$formdata['pic_icon']}" {if $formdata['pic_icon']}disabled="disabled"{/if} name='pic_icon' style="width:180px;">
							<font class="important" style="color:red">*创建和无法修改</font>
						</div>
					</li>
					<li class="i">
						<div class="form_ul_div">
							<span  class="title">基础值：</span>
							<input type="text" value="{$formdata['pic_value']}" name='pic_value' style="width:180px;">
							<font class="important" style="color:red">*</font>
						</div>
					</li>
					<li class="i">
						<div class="form_ul_div">
							<span  class="title">步进值：</span>
							<input type="text" value="{if $formdata['step_value']}{$formdata['step_value']}{else}1{/if}" name='step_value' style="width:180px;">
							<font class="important" style="color:red">*</font>
						</div>
					</li>
					<li class="i">
						<div class="form_ul_div">
							...
							<input type="radio" {if !empty($formdata['is_on']) && $formdata['is_on']==1} checked {/if} name="is_on" value="1" /> 是 <input type="radio" {if empty($formdata['is_on']) || $formdata['is_on']==0} checked {/if}  name="is_on" value="0" /> 否
							<font class="important" style="color:red">*</font>
						</div>
					</li>
					{if $formdata['id']}
					<!-- <li class="i">
						...
						{if $formdata['system_img'] && !in_array('',array_values($formdata['system_img']))}
								...
								{/code}
							<a  href="{$org}"><img alt="分类图" src="{$url}" onclick="preview_system_image({$formdata['id']})"/></a>
						{/if}	
						</div>
					</li> -->
					{/if}
						...
						<div class="form_ul_div" >
							...
							{if $formdata['user_img'] && !empty($formdata['user_img'])}				
								<div class="weather-img-a" id = "user_image_{$formdata['id']}" >
									...
									{/code}
									<a  href="{$org}" ><img alt="分类图" src="{$url}" onclick="preview_user_image({$formdata['id']})"/></a>
						...
						<div class="form_ul_div" >
							...
							{if !in_array('',array_values($vv['app_user_image']))}
								...
								<div class="weather-img-a" id = "app_user_image_{$val['appid']}" >
									<a  href="{$org}" ><img alt="分类图" src="{$url}" onclick="preview_app_user_image({$kk})"/></a>
