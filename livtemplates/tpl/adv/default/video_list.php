...
{foreach $formdata['video_info'] as $k => $v}
  <span id="v_{$v['id']}" onclick="hg_deleteSelf($(this));" class="li" >
	 ...
	 <div id="s_{$v['id']}" class="show_item">
		...
		{code}
			$data = serialize($formdata['video_info'])
		{/code}
		<input type="hidden" id="s_data_{$v['id']}" value='{$data}'/>		
	 </div>
  </span>
{/foreach}
</div>
...
<div class="page">
    {if $formdata['total_num'] > $formdata['page_num']}
		...
		<a href="javascript:void(0);" onclick="hg_getVideo({$formdata['first_page']});" class="p">|<</a>
		<a href="javascript:void(0);" onclick="hg_getVideo({$formdata['prev_page']});" class="p"><</a>
		{/if}
		{if $formdata['total_page'] != $formdata['current_page']}
		<a href="javascript:void(0);" onclick="hg_getVideo({$formdata['next_page']});" class="p" >></a>
		<a href="javascript:void(0);" onclick="hg_getVideo({$formdata['last_page']});" class="p">>|</a>
		{/if}
	{/if}
