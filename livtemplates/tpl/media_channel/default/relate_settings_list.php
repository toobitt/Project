...
<script type="text/javascript">
	...
	function hg_get_cfg(bundle_id,obj)
	{
		$('#nav_left div').css('background','#fcf4ea');
		$(obj).css('background','#70a9de');
		var url = "run.php?mid="+gMid+"&a=get_app_config&bundle_id="+bundle_id;
		hg_ajax_post(url);
	}
</script>
<style type="text/css">
...
<div class="relate_c">
	<div class="nav_left" id="nav_left">
		{foreach $list AS $k => $v}
			<div onclick="hg_get_cfg('{$v[bundle_id]}',this)">{$v['name']}</div>
		{/foreach}
	</div>
