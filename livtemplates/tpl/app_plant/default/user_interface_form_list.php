		...
		<div class="ad_middle">
			<form action="run.php?mid={$_INPUT['mid']}" method="post" enctype="multipart/form-data" class="ad_form h_l">
					...
					<ul class="form_ul">
						...
						<li class="i">
							<div class="form_ul_div">
								<span  class="title">组件示意图：</span>
								<input type="file" name='comp_img' />
							</div>
							{code}
								$comp_img_url = '';
								if($comp_img && is_array($comp_img))
								{
									$comp_img_url = $comp_img['host'] . $comp_img['dir'] . $comp_img['filepath'] . $comp_img['filename'];
								}
							{/code}
							{if $comp_img_url}
							<div class="form_ul_div" style="margin-left:84px;">
								<img src="{$comp_img_url}" width="320" height="480" />
							</div>
							{/if}
						</li>
