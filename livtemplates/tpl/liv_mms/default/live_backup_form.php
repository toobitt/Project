...
<div class="ad_middle">
<form action="./run.php?mid={$_INPUT['mid']}" method="post" enctype="multipart/form-data" id="backup" name="backup" onsubmit="return hg_submit_backup_video();" class="ad_form h_l">
<h2>增加备播文件</h2>
<ul class="form_ul">
	<li class="i">
		<div class="form_ul_div">
			<span class="title">标题：</span><input type="text" name="title" value="{$formdata['title']}" style="width:192px"  id="required_1" >
			<!-- onblur="input_content_color(1),checkCode();" onfocus="input_content_color(1);"  -->
			<font class="important" id="important_1">必填</font>
		</div>
		<div class="form_ul_div">
			<span class="title">描述：</span><input type="text" name="brief" value="{$formdata['brief']}" style="width:192px">
		</div>
		<div class="form_ul_div clear">
			<span class="title">备播文件：</span>
			<span class="file_input" id="vidoes_show" onclick="hg_showBackUp(),hg_getManyVideos(0);" style="cursor:pointer;margin-right:10px;float:left">选择视频</span>
			<!--屏蔽本地上传
			<span class="file_input" id="file_input" style="float:left">本地上传</span>
			...
			<!--////-->
			<span id="img_span" style="display:{if $a == 'create' || !$formdata['vodinfo_id']}none{else}block{/if};margin-left:10px;color:#333;float:left;line-height:44px;">
				
				<div style="float:right;line-height:22px;margin-left:5px;width:250px;">
					<span id="vod_name" style="width:250px;display:block;" class="overflow">名称:{$formdata['filename']}</span>
					<span id="vod_toff" style="clear:both;display:block">时长:{$formdata['toff']}</span>
				</div>
				<img id="vodid_img" width=52 src="{$formdata['img']}" style="vertical-align: middle;padding:2px;background:white;border:1px solid #CCC;margin:0 2px 0 5px;"/>
			</span>
...
{if $a == 'update'}
	<input type="hidden" name="a" value="backup_update" />
{else}
	<input type="hidden" name="a" value="{$a}" />
{/if}
	<input type="hidden" name="{$primary_key}" value="{$$primary_key}" />
	<input type="hidden" name="referto" value="{$_INPUT['referto']}" />
	<input type="hidden" name="back_up_video_id" id="back_up_video_id" value="{$formdata['vodinfo_id']}" />
	<input type="hidden" id="temp_video_id" value="{$formdata['vodinfo_id']}" />
	</br>
