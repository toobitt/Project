<?php 
/* $Id: program_plan_list.php 6571 2011-12-21 06:09:23Z repheal $ */
?>
{code}
if(isset($plan))
{
				...
				{else}
					{if $mArr[$i]}
						{if $single['end'] == $value}
							{if !empty($next) && $next['start'] == $value}
								<td onclick="hg_plan_form(this,{$next['id']});"><div class="item_start" style="{if empty($prev) || $prev['end'] != $single['start']}
								...
								{else}
									...
									{/if}
								{/if}"><span class="t">{code} echo date('H:i',$next['start']);{/code}-{code} echo date('H:i',$next['end']);{/code}</span><span class="n overflow">{$next['program_name']}</span></div></td>
								{code}
									$mArr[$i] = 1;
									$mKey[$i]++;
									continue;
							...
							{/code}{/if}
						{else}
								<td onclick="hg_plan_form(this,{$single['id']});"><div class="item_none"></div></td>
								{code}
									$mArr[$i] = 1;
									continue;
								{/code}	
					...
					{else}					
						{if $single['start'] == $value}
							<td onclick="hg_plan_form(this,{$single['id']});"><div class="item_start"><span class="t">{code} echo date('H:i',$single['start']);{/code}-{code} echo date('H:i',$single['end']);{/code}</span><span class="n overflow">{$single['program_name']}</span></div></td>
							{code}
								$mArr[$i] = 1;
								continue;
							{/code}
...
{else}
	<tr class="default" align="left" valign="top">
		<td class="none" onclick="hg_plan_form(this);" onmouseover="hg_plan_none(this,1);" onmouseout="hg_plan_none(this,0);"></td>
		<td class="none" onclick="hg_plan_form(this);" onmouseover="hg_plan_none(this,1);" onmouseout="hg_plan_none(this,0);"></td>
		<td class="none" onclick="hg_plan_form(this);" onmouseover="hg_plan_none(this,1);" onmouseout="hg_plan_none(this,0);"></td>
		<td class="none" onclick="hg_plan_form(this);" onmouseover="hg_plan_none(this,1);" onmouseout="hg_plan_none(this,0);"></td>
		<td class="none" onclick="hg_plan_form(this);" onmouseover="hg_plan_none(this,1);" onmouseout="hg_plan_none(this,0);"></td>
