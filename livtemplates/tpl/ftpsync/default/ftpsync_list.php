		...
		{if $ftpsync_list}
		   {foreach $ftpsync_list as $k => $v}
			<tr orderid="{$v['id']}"  id="r_{$v['id']}" class="h"   name="{$v['id']}"  align="left"  valign="middle"  onmouseover="hg_row_interactive(this, 'on');" onmouseout="hg_row_interactive(this, 'out');" onclick="hg_row_interactive(this, 'click', 'cur');">
				...
				<td class="text_indent"><span id="name_{$v['id']}">{$v['syncname']}</span></td>
				<td class="text_indent">{if !$v['status']}待审核{else}已运行{/if}</td>
				...
				<td class="text_indent">
					...
					<a onclick="return hg_ajax_post(this, '审核', 1);" href="./run.php?mid={$_INPUT['mid']}&a=audit&id={$v['id']}&infrm=1">{if !$v['status']}执行任务{else}暂停任务{/if}</a>
				</td>
