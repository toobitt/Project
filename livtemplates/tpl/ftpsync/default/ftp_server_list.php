...
<a href="?mid={$_INPUT['mid']}&a=form{$_ext_link}" class="button_6"><strong>新增FTP服务器</strong></a>
</div>
<div class="wrap">
<form>
	<table  border="0" cellpadding="0" cellspacing="0" width="100%"  id="channel_table" class="form_table">
		<tr id="item_th" class="h" align="left" valign="middle" >
			...
			<th class="text_indent" width="120">#ID</th>
			<th class="text_indent">主机名称</th>
			<th class="text_indent" width="100">端口</th>
		...
		<tbody id="status">
		{if $ftp_server_list}
		   {foreach $ftp_server_list as $k => $v}
			<tr orderid="{$v['id']}"  id="r_{$v['id']}" class="h"   name="{$v['id']}"  align="left"  valign="middle"  onmouseover="hg_row_interactive(this, 'on');" onmouseout="hg_row_interactive(this, 'out');" onclick="hg_row_interactive(this, 'click', 'cur');">
				...
				<td class="text_indent"><span id="name_{$v['id']}">{$v['hostname']}</span></td>
				<td class="text_indent"><span onclick="hg_backup_flv({$v['id']});" style="cursor:pointer;" title="预览">{$v['port']}</span></td>
				<td class="text_indent" >{$v['user']}</td>
				<td class="text_indent">{if !$v['status']}<font color='red'>异常</font>{else}<font color='green'>正常</font>{/if}</td>
