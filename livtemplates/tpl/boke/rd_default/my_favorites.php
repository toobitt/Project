<?php
/* $Id: my_favorites.php 5351 2011-12-06 05:49:24Z lixuguang $ */
?>
{template:head}
<div class="main_div vui">
	<div class="right_window con-left">
	<div class="station_content">	
	<h3 class="con_top"><a href="<?php echo hg_build_link("upload.php")?>">+上传视频</a>我的收藏</h3>
		<div class="show_info">
		<div class="clear">
		{code}
		switch ($type)
		...
		{/code}
			{if $stationInfo}
							...
							<ul id="collect_{$v['collect_id']}" style="float:left;padding:10px 15px;line-height:23px;height:230px;">
								...
								<li><span>播放:</span><strong>{$v['play_count']}</strong><span class="collect_commets"><span>评论:</span><strong>{$v['comment_count']}</strong></span></li>
								<li>
			                     <a href="javascript:void(0);" onclick="del_collect({$v['collect_id']},{$v['id']},{$type});">取消收藏</a>
			                   	</li>
							</ul>
							{/foreach}
						{else}
