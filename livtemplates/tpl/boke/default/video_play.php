<?php 
/* $Id: video_play.php 9469 2012-05-29 05:57:57Z repheal $ */
?>
{template:head/head2}
<div class="gareas clear">
  <div class="gtop" onmouseover="report_show({$single_video_info['id']},{$single_video_info['user']['id']});" onmouseout="report_hide({$single_video_info['id']},{$single_video_info['user']['id']});">
  	...
  	<div class="lc">
  	<span class="subleft"><a href="{$_nav['main']['index']}">{$_nav['lang']['sitename']}</a></span><span class="subcolor">&gt;</span>
	...
	{/if}
  	<h1 class="subsub" id="vedio_name_container">{$single_video_info['title']}</h1></div>
  	<input type="hidden" id="video_briefs" value="{$single_video_info['brief']}"/>
    <div class="rc">
  {if $_user['id']}<span class="rsubcolor2" onclick="report_play({$single_video_info['id']},{$single_video_info['user']['id']});" style="display:none;cursor:pointer;" id="re_{$single_video_info['id']}_{$single_video_info['user']['id']}">{$_lang['report']}</span>{/if}  
  ...
  <div class="gtv">
   <div class="gtg">
    ...
    <div class="tv">
      <div class="tv_video"><div id="container"><div id="player">加载中</div></div></div>
    </div>
    <div class="tv_vidtt">
        <div class="tv1">
            	<div><img src="<?php echo RESOURCE_DIR;?>img/adico1.gif"  /></div>
                <div id="status_share"><a href="javascript:void(0);" onclick="show_status_share({$single_video_info['id']});">分享到点滴</a></div>
...
<div class="garea">
    ...
    <div class="gsub2"><h3>相关视频</h3></div>
	...
	{else}
		...
		{foreach $video as $key => $value}
			{if $i ==1}
				<div class="ottv"><a title="{$value['title']}" href="<?php echo hg_build_link("video_play.php", array('id'=>$value['id']));?>"><img src="{$value['schematic']}" width="122" height="91" /></a><a style="height: 14px;*height: 12px; overflow: hidden; width: 120px;" title="{$value['title']}" href="<?php echo hg_build_link("video_play.php", array('id'=>$value['id']));?>"><?php echo hg_cutchars($value['title'],19," ");?></a>
    ...
    <div class="gright">
	    <div class="gsub1"><img src="<?php echo RESOURCE_DIR;?>img/hcar_1.gif" width="19" height="28" /></div>
		<div class="gsub2"><h3>视频信息</h3></div>
		<div class="gsub3"><img src="<?php echo RESOURCE_DIR;?>img/hcar_4.gif" width="7" height="28" /></div>
		<div class="gsub5">
			...
			<div class="img_txt">
				<div><span class="lc"><h2 style="font-size:12px;">{$single_video_info['title']}</h2></span><span class="rc"><!--<a class="sline" href="#">发站内站</a><a target="_blank" href="<?php echo hg_build_link(SNS_UCENTER."user.php", array('user_id'=>$single_video_info['user']['id']));?>">{$single_video_info['user']['username']}</a>-->
		 		{if is_array($station)}
					{if !$station['myself']}
						{if $station['relation']}
							{code}
				...
				<ul class="img_t">
				<li>发布时间:{code} echo date("Y-m-d H:i:s",$single_video_info['create_time']);{/code}</li>
				<li>标签：
				{if !empty($single_video_info['tags'])}
				{code}
					$tmp_tags =  explode(',',$single_video_info['tags']);
					if(count($tmp_tags)>5)
					{
		...
		{if count($visit)>1}
	        ...
	        <div class="gsub4">
				...
				{foreach $visit as $key => $value}
					{if $i==1||$i==6}
						<li><a href="<?php echo hg_build_link(SNS_UCENTER."user.php", array("user_id"=>$value['user']['id']));?>"><img src="{$value['user']['middle_avatar']}" width="50" height="50" /></a><a href="<?php echo hg_build_link(SNS_UCENTER."user.php", array("user_id"=>$value['user']['id']));?>" class="titles">{$value['user']['username']}</a></li>
					{else}
						<li class="mars"><a href="<?php echo hg_build_link(SNS_UCENTER."user.php", array("user_id"=>$value['user']['id']));?>"><img src="{$value['user']['middle_avatar']}" width="50" height="50" /></a><a href="<?php echo hg_build_link(SNS_UCENTER."user.php", array("user_id"=>$value['user']['id']));?>" class="titles">{$value['user']['username']}</a></li>
					{/if}
					{code}
						$i++;
					{/code}
