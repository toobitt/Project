<?php 
/* $Id: upload.php 9761 2012-06-13 01:35:09Z repheal $ */
?>
{template:head}
...
$(document).ready(function() {
	...
	swfu = new SWFUpload( {
		...
		use_query_string : false, /*以POST方式提交数据(true 则为GET)*/
	...
	{	
		if(server_data == 1)
		{
			...
			message_notice('视频上传成功！<a style="padding:3px;" href="my_video.php" target="_blank">查看我上传的视频</a>' , 1);
			...
			setTimeout("upload_reset()",3000);
		}
		else
		{
			$('#file_deal').css('display' , 'none');
			$('#waiting_notice').css('display' , 'none');
			/*var json_obj = eval('(' + server_data + ')'); //将json串转化为json对象*/
			/*var message = json_obj.error;*/
...
function upload_reset()
{
	...
	$("#video_brief").val('');
	$("#video_tags").val('');
	$("#video_sort_19").attr("checked","checked");
	$("#video_copyright_1").attr("checked","checked");
	$("#progress_show").attr("style","background-color: rgb(99, 223, 45); width: 0px; ");
	$('#upload_button').attr('disabled',"false");
	$('#upload_button').show();
	$('#upload_progress').attr("style","display:none;");
...
function message_notice(notice , type)
{
	...
	else	/*错误提示*/
	{
		...
		/*$('#error_message').animate({opacity:'show'},3000).animate({opacity:'toggle'},3000);*/
	}
	$('#error_message').show();
}
	...
	<div class="right_window">	
		<div class="show_head">
				<ul class="show_head_ul">
					<li><span class="show_head_cursor">上传视频</span></li>
					{if ALLOW_PROGRAME}
		...
		<div class="show_info">
			...
			<ul>
				...
				<li class="up_load" style="font-family:Arial,Verdana,Tahoma,Simsun,sans-serif;">
					...
					<input style="vertical-align:middle;margin-left:5px;" onFocus="this.blur();" type="button" id="upload_button" onclick="upload_video();" value="" />
				</li>
			</ul>		
			</div>
			...
			<div class="info">
				<div class="help_info" style="font-family:Arial,Verdana,Tahoma,Simsun,sans-serif;">
					...
					<ol>
						...
						<li>2 .支持格式：葫芦网支持绝大多数的视频格式。</li>
						<li>   
							<ul>
								<li>* 微软视频：.wmv .avi .dat .asf </li>
								<li>* Real Player：.rm .rmvb .ram </li>
								<li>* MPEG视频：.mpg .mpeg </li>
								<li>* 手机视频：.3gp </li>
								<li>* Apple视频：.mov </li>
								<li>* Sony视频：.mp4 .m4v </li>
								<li>* DV视频：.dvix .dv </li>
								<li>* 其他常见视频：.dat .mkv .flv .vob .ram .qt .divx .cpk .fli .flc .mod</li>
							</ul>								
						</li>
