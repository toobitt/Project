...
<div id="comment_note" class="clear">
	...
	<ul class="comment_notes clear">
		...
		<?php 
		$checked = $_settings['default_sync']['comm']?' checked':'';
		?>
		<input type="checkbox" id="syn" {$checked}/>
...
{/if}
			...
			<div class="bar-right">
				...
				{if $_user['id']}<a onclick="report_play({$value['id']},{$value['user']['id']});" href="javascript:void(0);" style="display:none;" id="re_{$value['id']}_{$value['user']['id']}">{$_lang['report']}</a>{/if}
...
{/if}
			...
			<div style="display:none;" id="type_{$v['id']}_{$v['user']['id']}">{$type}</div>					
					<div class="bar-right">
						...
						{if $_user['id']}<a onclick="report_play({$v['id']},{$v['user']['id']});" href="javascript:void(0);" style="display:none;" id="re_{$v['id']}_{$v['user']['id']}">{$_lang['report']}</a>{/if}
						<a href="javascript:void(0);" onclick="reply_comment({$value['cid']},{$value['id']},{$value['user']['id']});">回复</a>
						{if $v['relation']}
