...
function hg_audit_publisher(id)
{
...
/*审核回调*/
function hg_audit_publisher_callback(obj)
{
	var obj = eval('('+obj+')');
	$('#status_'+obj.id).text(obj.status_format);
}
</script>
...
<div id="hg_page_menu" class="head_op"{if $_INPUT['infrm']} style="display:none"{/if}>
	<a class="blue mr10"  href="?mid={$_INPUT['mid']}&a=form{$_ext_link}">
		...
		<span class="middle"><em class="add">新增版权商</em></span>
                ...
                <form method="post" action="" name="vod_sort_listform">
                   ...
                   <ul class="common-list">
	                ...
	                <ul class="common-list hg_sortable_list public-list" id="auth_form_list">
					    {if $list}
		       			    {foreach  $list  as $k => $v}
		                      {template:unit/publisher_list_item}
		                    {/foreach}
		                {else}
