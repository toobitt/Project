<li order_id="{$v['order_id']}" _id="{$v[$primary_key]}" class="common-list-data clear"  id="r_{$v[$primary_key]}" name="{$v['order_id']}" >
    ...
    <div class="common-list-right">
        <div class="common-list-item wd100 overflow open-close" >
            <span>{$v['channel_name']}</span>
        </div>
        <div class="common-list-item overflow open-close" style="width:300px;">
            <span>{$v['start_time_show']} 至 {$v['end_time_show']}</span>
        </div>
        <div class="common-list-item wd70 overflow open-close">
            <span style="color:{$_configs['status_color'][$v['time_status']]};">{$v['time_status_text']}</span>
        </div>

        <div class="common-list-item wd70 news-zhuangtai open-close">
            <div class="common-switch-status">
                <span _id="{$v['id']}" _state="{$v['status']}" id="statusLabelOf{$v['id']}" style="color:{$_configs['status_color'][$v['status']]};">{$v['status_text']}</span>
            </div>
        </div>
