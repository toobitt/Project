...
$publish['selected_ids'] = $item['column_id'] ? $item['column_id'] : $item['publish_prms'];
...
$item['siteid'] = isset($item['siteid']) ? $item['siteid'] : $item['site_prms'];

$publish['siteid'] = explode(',', $item['siteid']);


$hg_print_selected = array();
foreach ($publish['selected_items'] as $index => $value) {
...
<div class="common-form-pop" id="form_publish" _type="publish" style="top:-450px;position:fixed;z-index:99999;-webkit-transition:top .5s;left:50%;margin-left:-312px;">   
    <div style="margin-bottom: 10px;">
        ...
        <div class="publish-site-mulitselect" style="left:160px;top:0;position:absolute;">
	        <input type="hidden" name="siteid" value="{$selected_site_id}" id="hiddenSiteid" />
	...
	jQuery(function($){
	    var timeid = setInterval(function(){
	        if($.fn.hg_publish){
	            ...
	            var publish = $('.common-hg-publish').data('publish');
	            if (!publish) { 
		           publish = $('.common-hg-publish').hg_publish({
		            	maxColumn: 3
		           }).data('publish');
		        }
	            /*重写一些方法*/
